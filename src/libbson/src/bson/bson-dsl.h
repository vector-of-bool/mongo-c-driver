...
_bson_thread_local _bson_comdat const struct _bsonVisitContext_t *_bsonVisitContextThreadLocalPtr = NULL;
...
static BSON_INLINE bool _bson_dsl_test_strequal(const char *string, bool case_sensitive) {
...
static BSON_INLINE bool _bson_dsl_key_is_anyof(const char *key, const size_t keylen, int case_sensitive, ...) {
    va_list va;
...
static BSON_INLINE void _bson_dsl_iter_as_doc(bson_t *into, const bson_iter_t *it) {
    uint32_t len = 0;
...
static BSON_INLINE bool _bson_dsl_is_empty_bson(const bson_iter_t *it) {
    bson_t d = BSON_INITIALIZER;
    _bson_dsl_iter_as_doc(&d, it);
    return d.len == 5; // Empty documents/arrays have byte-size of five
}

static BSON_INLINE bool _bson_dsl_iter_is_last_element(const bson_iter_t *it) {
    bson_iter_t dup = *it;
...
static BSON_INLINE void _bson_dsl_dupPath(char **into) {
    if (*into) {
        bson_free(*into);
        *into = NULL;
    }
...
static BSON_INLINE const char *_bsonVisitIterAs_cstr(void) {
    return bson_iter_utf8(&bsonVisitIter, NULL);
}

static BSON_INLINE int32_t _bsonVisitIterAs_int32(void) {
    return bson_iter_int32(&bsonVisitIter);
}

static BSON_INLINE bool _bsonVisitIterAs_bool(void) {
    return bson_iter_as_bool(&bsonVisitIter);
}

static BSON_INLINE bool _bsonVisitIterAs__Bool(void) {
