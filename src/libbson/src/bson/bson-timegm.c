...
static const int64_t time_t_min = INT64_MIN;
static const int64_t time_t_max = INT64_MAX;
...
static struct bson_tm *gmtsub(const int64_t *const timep, const int_fast32_t offset, struct bson_tm *const tmp);
static int64_t increment_overflow(int64_t *const ip, int64_t j);
static int64_t leaps_thru_end_of(const int64_t y) ATTRIBUTE_PURE;
static int64_t increment_overflow32(int_fast32_t *const lp, const int64_t m);
static int64_t normalize_overflow32(int_fast32_t *const tensptr, int64_t *const unitsptr, const int64_t base);
static int64_t normalize_overflow(int64_t *const tensptr, int64_t *const unitsptr, const int64_t base);
...
static int64_t tmcomp(const struct bson_tm *const atmp, const struct bson_tm *const btmp);
...
static int64_t leaps_thru_end_of(const int64_t y) {
    return (y >= 0) ? (y / 4 - y / 100 + y / 400) : -(leaps_thru_end_of(-(y + 1)) + 1);
}

static struct bson_tm *timesub(const int64_t *const timep,
                               const int_fast32_t offset,
