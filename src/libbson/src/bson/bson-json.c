...
bson_json_opts_t *bson_json_opts_new(bson_json_mode_t mode, int32_t max_len) {
...
static void _bson_json_read_set_error(bson_json_reader_t *reader, const char *fmt, ...) BSON_GNUC_PRINTF(2, 3);
...
static void _bson_json_read_corrupt(bson_json_reader_t *reader, const char *fmt, ...) BSON_GNUC_PRINTF(2, 3);
...
static const char *_bson_json_type_name(bson_type_t type) {
    switch (type) {
    case BSON_TYPE_EOD: return "end of document";
    case BSON_TYPE_DOUBLE: return "double";
    case BSON_TYPE_UTF8: return "utf-8";
    ...
    case BSON_TYPE_CODEWSCOPE: return "code with scope";
...
#pragma warning(suppress : 4146)
        ...
        _bson_json_read_set_error(reader,
                                  "Unexpected integer %s%" PRIu64 " in state \"%s\"",
                                  sign == -1 ? "-" : "",
                                  val,
                                  read_state_names[rs]);
    }
...
static bool _bson_json_parse_double(bson_json_reader_t *reader, const char *val, size_t vlen, double *d) {
    errno = 0;
    *d = strtod(val, NULL);
...
static bool _unhexlify_uuid(const char *uuid, uint8_t *out, size_t max) {
    unsigned int byte;
    size_t x = 0;
    int i = 0;
 ...
 * "$uuid" */
static void _bson_json_parse_binary_elem(bson_json_reader_t *reader, const char *val_w_null, size_t vlen) {
    bson_json_read_bson_state_t bs;
...
static bool _bson_json_allow_embedded_nulls(const bson_json_reader_t *reader) {
                                   ...
                                   size_t vlen)                /* IN */
{
    ...
    if (rs == BSON_JSON_REGULAR) {
        ...
        switch (bs) {
        case BSON_JSON_LF_REGEX: bson->bson_type_data.regex.is_legacy = true;
        /* FALL THROUGH */
        case BSON_JSON_LF_REGULAR_EXPRESSION_PATTERN:
            bson->bson_type_data.regex.has_pattern = true;
            _bson_json_buf_set(&bson->bson_type_buf[0], val, vlen);
            break;
        case BSON_JSON_LF_OPTIONS: bson->bson_type_data.regex.is_legacy = true;
        /* FALL THROUGH */
        case BSON_JSON_LF_REGULAR_EXPRESSION_OPTIONS:
        ...
        case BSON_JSON_LF_TYPE:
            ...
            /* FALL THROUGH */
        case BSON_JSON_LF_UUID: _bson_json_parse_binary_elem(reader, val_w_null, vlen); break;
        case BSON_JSON_LF_INT32: {
            int64_t v64;
        ...
        case BSON_JSON_LF_CODE: _bson_json_buf_set(&bson->code_data.code_buf, val, vlen); break;
        case BSON_JSON_LF_SYMBOL:
...
static void _bson_json_read_start_map(bson_json_reader_t *reader) /* IN */
{
    ...
    if (bson->read_state == BSON_JSON_IN_BSON_TYPE) {
        ...
        case BSON_JSON_LF_DATE: bson->read_state = BSON_JSON_IN_BSON_TYPE_DATE_NUMBERLONG; break;
        case BSON_JSON_LF_BINARY: bson->read_state = BSON_JSON_IN_BSON_TYPE_BINARY_VALUES; break;
        case BSON_JSON_LF_TYPE:
...
#define IS_KEY(k) (len == strlen(k) && (0 == memcmp(k, key, len)))
    ...
    ret = (IS_KEY("$regularExpression") || IS_KEY("$regex") || IS_KEY("$options") || IS_KEY("$code") || IS_KEY("$scope")
           || IS_KEY("$oid") || IS_KEY("$binary") || IS_KEY("$type") || IS_KEY("$date") || IS_KEY("$undefined")
           || IS_KEY("$maxKey") || IS_KEY("$minKey") || IS_KEY("$timestamp") || IS_KEY("$numberInt")
           || IS_KEY("$numberLong") || IS_KEY("$numberDouble") || IS_KEY("$numberDecimal") || IS_KEY("$numberInt")
           || IS_KEY("$numberLong") || IS_KEY("$numberDouble") || IS_KEY("$numberDecimal") || IS_KEY("$dbPointer")
           || IS_KEY("$symbol") || IS_KEY("$uuid"));
...
static void _bson_json_read_end_map(bson_json_reader_t *reader) /* IN */
{
    ...
    if (bson->read_state == BSON_JSON_IN_BSON_TYPE) {
        ...
        case BSON_TYPE_REGEX: _bson_json_read_append_regex(reader, bson); break;
        case BSON_TYPE_CODE:
        case BSON_TYPE_CODEWSCOPE:
            /* we've read the closing "}" in "{$code: ..., $scope: ...}" */
            _bson_json_read_append_code(reader, bson);
            break;
        case BSON_TYPE_OID: _bson_json_read_append_oid(reader, bson); break;
        case BSON_TYPE_BINARY: _bson_json_read_append_binary(reader, bson); break;
        case BSON_TYPE_DATE_TIME: _bson_json_read_append_date_time(reader, bson); break;
        case BSON_TYPE_UNDEFINED: r = bson_append_undefined(STACK_BSON_CHILD, bson->key, (int)bson->key_buf.len); break;
        case BSON_TYPE_MINKEY: r = bson_append_minkey(STACK_BSON_CHILD, bson->key, (int)bson->key_buf.len); break;
        case BSON_TYPE_MAXKEY: r = bson_append_maxkey(STACK_BSON_CHILD, bson->key, (int)bson->key_buf.len); break;
...
static void _push_callback(jsonsl_t json, jsonsl_action_t action, struct jsonsl_state_st *state, const char *buf) {
    ...
    case JSONSL_T_UESCAPE: reader->json_text_pos = state->pos_begin; break;
    case JSONSL_T_OBJECT: _bson_json_read_start_map(reader); break;
    case JSONSL_T_LIST: _bson_json_read_start_array(reader); break;
    default: break;
    }
}

static void _pop_callback(jsonsl_t json, jsonsl_action_t action, struct jsonsl_state_st *state, const char *buf) {
    bson_json_reader_t *reader;
    bson_json_reader_bson_t *reader_bson;
    ssize_t len;
    double d;
    ...
    case JSONSL_T_STRING:
        ...
        break;
    case JSONSL_T_OBJECT: _bson_json_read_end_map(reader); break;
    case JSONSL_T_LIST: _bson_json_read_end_array(reader); break;
    case JSONSL_T_SPECIAL:
        obj_text = _get_json_text(json, state, buf, &len);
...
static int _error_callback(jsonsl_t json, jsonsl_error_t err, struct jsonsl_state_st *state, char *errat) {
    bson_json_reader_t *reader = (bson_json_reader_t *)json->data;
