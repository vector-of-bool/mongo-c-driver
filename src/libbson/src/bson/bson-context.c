...
void _bson_context_set_oid_seq32(bson_context_t *context, /* IN */
                                 bson_oid_t *oid)         /* OUT */
{
...
void _bson_context_set_oid_seq64(bson_context_t *context, /* IN */
                                 bson_oid_t *oid)         /* OUT */
{
 ...
 * --------------------------------------------------------------------------
 */
static void _bson_context_get_hostname(char out[HOST_NAME_MAX]) {
...
void _sip_round(uint64_t *v0, uint64_t *v1, uint64_t *v2, uint64_t *v3) {
    *v0 += *v1;
...
void _siphash(const void *in, const size_t inlen, const uint64_t key[2], uint64_t digest[2]) {
    ...
    default: BSON_UNREACHABLE("Invalid remainder during SipHash");
    case 0: break;
    }
...
static void _bson_context_init_random(bson_context_t *context, bool init_seq) {
...
static void _bson_context_init(bson_context_t *context, bson_context_flags_t flags) {
    context->flags = (int)flags;
    _bson_context_init_random(context, true /* Init counters */);
}

void _bson_context_set_oid_rand(bson_context_t *context, bson_oid_t *oid) {
    BSON_ASSERT(context);
    BSON_ASSERT(oid);
