...
#define DEF_ATOMIC_OP(MSVC_Intrinsic, GNU_Intrinsic, GNU_Legacy_Intrinsic, Order, ...)                                 \
    do {                                                                                                               \
        ...
        case bson_memory_order_relaxed:                                                                                \
            ...
            BSON_IF_GNU_LEGACY_ATOMICS(return GNU_Legacy_Intrinsic(__VA_ARGS__);)                                      \
        default: BSON_UNREACHABLE("Invalid bson_memory_order value");                                                  \
        }                                                                                                              \
    } while (0)
...
#define DEF_ATOMIC_CMPEXCH_STRONG(VCSuffix1, VCSuffix2, GNU_MemOrder, Ptr, ExpectActualVar, NewValue)                  \
    do {                                                                                                               \
        BSON_IF_MSVC(                                                                                                  \
            ExpectActualVar =                                                                                          \
                BSON_CONCAT3(_InterlockedCompareExchange, VCSuffix1, VCSuffix2)(Ptr, NewValue, ExpectActualVar);)      \
...
#define DECL_ATOMIC_INTEGRAL(NamePart, Type, VCIntrinSuffix)                                                           \
    static BSON_INLINE Type bson_atomic_##NamePart##_fetch_add(Type volatile *a,                                       \
                                                               Type addend,                                            \
        ...
        DEF_ATOMIC_OP(BSON_CONCAT(_InterlockedExchangeAdd, VCIntrinSuffix),                                            \
                      ...
                      addend);                                                                                         \
                                                                                                                       ...
                                                                                                                       \
    static BSON_INLINE Type bson_atomic_##NamePart##_fetch_sub(Type volatile *a,                                       \
                                                               Type subtrahend,                                        \
                                                               enum bson_memory_order ord) {                           \
    ...
    static BSON_INLINE Type bson_atomic_##NamePart##_fetch(Type volatile const *a, enum bson_memory_order order) {     \
        ...
        BSON_IF_GNU_LIKE(switch (order) {                                                                              \
            ...
            case bson_memory_order_seq_cst: return __atomic_load_n(a, __ATOMIC_SEQ_CST);                               \
            case bson_memory_order_acquire: return __atomic_load_n(a, __ATOMIC_ACQUIRE);                               \
            case bson_memory_order_consume: return __atomic_load_n(a, __ATOMIC_CONSUME);                               \
            case bson_memory_order_relaxed: return __atomic_load_n(a, __ATOMIC_RELAXED);                               \
            default: BSON_UNREACHABLE("Invalid bson_memory_order value");                                              \
        })                                                                                                             \
        BSON_IF_GNU_LEGACY_ATOMICS({                                                                                   \
            ...
            return *a;                                                                                                 \
                                                                                                                       ...
                                                                                                                       \
    static BSON_INLINE Type bson_atomic_##NamePart##_exchange(Type volatile *a,                                        \
                                                              Type value,                                              \
                                                              enum bson_memory_order ord) {                            \
        ...
        BSON_IF_GNU_LIKE(switch (ord) {                                                                                \
            case bson_memory_order_acq_rel: return __atomic_exchange_n(a, value, __ATOMIC_ACQ_REL);                    \
            case bson_memory_order_release: return __atomic_exchange_n(a, value, __ATOMIC_RELEASE);                    \
            case bson_memory_order_seq_cst: return __atomic_exchange_n(a, value, __ATOMIC_SEQ_CST);                    \
            case bson_memory_order_consume: /* Fall back to acquire */                                                 \
            case bson_memory_order_acquire: return __atomic_exchange_n(a, value, __ATOMIC_ACQUIRE);                    \
            case bson_memory_order_relaxed: return __atomic_exchange_n(a, value, __ATOMIC_RELAXED);                    \
            default: BSON_UNREACHABLE("Invalid bson_memory_order value");                                              \
        })                                                                                                             \
        BSON_IF_GNU_LEGACY_ATOMICS(return __sync_val_compare_and_swap(a, *a, value);)                                  \
    }                                                                                                                  \
                                                                                                                       \
    static BSON_INLINE Type bson_atomic_##NamePart##_compare_exchange_strong(Type volatile *a,                         \
                                                                             Type expect,                              \
                                                                             Type new_value,                           \
                                                                             enum bson_memory_order ord) {             \
        Type actual = expect;                                                                                          \
        ...
        return actual;                                                                                                 \
                                                                                                                       ...
                                                                                                                       \
    static BSON_INLINE Type bson_atomic_##NamePart##_compare_exchange_weak(Type volatile *a,                           \
                                                                           Type expect,                                \
                                                                           Type new_value,                             \
                                                                           enum bson_memory_order ord) {               \
        Type actual = expect;                                                                                          \
...
BSON_EXPORT(int64_t)
_bson_emul_atomic_int64_fetch_add(volatile int64_t *val, int64_t v, enum bson_memory_order);
BSON_EXPORT(int64_t)
_bson_emul_atomic_int64_exchange(volatile int64_t *val, int64_t v, enum bson_memory_order);
BSON_EXPORT(int64_t)
_bson_emul_atomic_int64_compare_exchange_strong(volatile int64_t *val,
                                                int64_t expect_value,
                                                int64_t new_value,
                                                enum bson_memory_order);

BSON_EXPORT(int64_t)
_bson_emul_atomic_int64_compare_exchange_weak(volatile int64_t *val,
                                              int64_t expect_value,
                                              int64_t new_value,
                                              enum bson_memory_order);

BSON_EXPORT(int32_t)
_bson_emul_atomic_int32_fetch_add(volatile int32_t *val, int32_t v, enum bson_memory_order);
BSON_EXPORT(int32_t)
_bson_emul_atomic_int32_exchange(volatile int32_t *val, int32_t v, enum bson_memory_order);
BSON_EXPORT(int32_t)
_bson_emul_atomic_int32_compare_exchange_strong(volatile int32_t *val,
                                                int32_t expect_value,
                                                int32_t new_value,
                                                enum bson_memory_order);

BSON_EXPORT(int32_t)
_bson_emul_atomic_int32_compare_exchange_weak(volatile int32_t *val,
                                              int32_t expect_value,
                                              int32_t new_value,
                                              enum bson_memory_order);

BSON_EXPORT(int)
_bson_emul_atomic_int_fetch_add(volatile int *val, int v, enum bson_memory_order);
BSON_EXPORT(int)
_bson_emul_atomic_int_exchange(volatile int *val, int v, enum bson_memory_order);
BSON_EXPORT(int)
_bson_emul_atomic_int_compare_exchange_strong(volatile int *val,
                                              int expect_value,
                                              int new_value,
                                              enum bson_memory_order);

BSON_EXPORT(int)
_bson_emul_atomic_int_compare_exchange_weak(volatile int *val, int expect_value, int new_value, enum bson_memory_order);
...
#endif
#else
static BSON_INLINE int64_t bson_atomic_int64_fetch(const volatile int64_t *val, enum bson_memory_order order) {
    return _bson_emul_atomic_int64_fetch_add((volatile int64_t *)val, 0, order);
}

static BSON_INLINE int64_t bson_atomic_int64_fetch_add(volatile int64_t *val, int64_t v, enum bson_memory_order order) {
    return _bson_emul_atomic_int64_fetch_add(val, v, order);
}

static BSON_INLINE int64_t bson_atomic_int64_fetch_sub(volatile int64_t *val, int64_t v, enum bson_memory_order order) {
    return _bson_emul_atomic_int64_fetch_add(val, -v, order);
}

static BSON_INLINE int64_t bson_atomic_int64_exchange(volatile int64_t *val, int64_t v, enum bson_memory_order order) {
    return _bson_emul_atomic_int64_exchange(val, v, order);
}

static BSON_INLINE int64_t bson_atomic_int64_compare_exchange_strong(volatile int64_t *val,
                                                                     int64_t expect_value,
                                                                     int64_t new_value,
...
static BSON_INLINE int64_t bson_atomic_int64_compare_exchange_weak(volatile int64_t *val,
                                                                   int64_t expect_value,
                                                                   int64_t new_value,
                                                                   enum bson_memory_order order) {
...
#if defined(BSON_EMULATE_INT32)
static BSON_INLINE int32_t bson_atomic_int32_fetch(const volatile int32_t *val, enum bson_memory_order order) {
    return _bson_emul_atomic_int32_fetch_add((volatile int32_t *)val, 0, order);
}

static BSON_INLINE int32_t bson_atomic_int32_fetch_add(volatile int32_t *val, int32_t v, enum bson_memory_order order) {
    return _bson_emul_atomic_int32_fetch_add(val, v, order);
}

static BSON_INLINE int32_t bson_atomic_int32_fetch_sub(volatile int32_t *val, int32_t v, enum bson_memory_order order) {
    return _bson_emul_atomic_int32_fetch_add(val, -v, order);
}

static BSON_INLINE int32_t bson_atomic_int32_exchange(volatile int32_t *val, int32_t v, enum bson_memory_order order) {
    return _bson_emul_atomic_int32_exchange(val, v, order);
}

static BSON_INLINE int32_t bson_atomic_int32_compare_exchange_strong(volatile int32_t *val,
                                                                     int32_t expect_value,
                                                                     int32_t new_value,
                                                                     enum bson_memory_order order) {
...
static BSON_INLINE int32_t bson_atomic_int32_compare_exchange_weak(volatile int32_t *val,
                                                                   int32_t expect_value,
                                                                   int32_t new_value,
                                                                   enum bson_memory_order order) {
...
#if defined(BSON_EMULATE_INT)
static BSON_INLINE int bson_atomic_int_fetch(const volatile int *val, enum bson_memory_order order) {
    return _bson_emul_atomic_int_fetch_add((volatile int *)val, 0, order);
}

static BSON_INLINE int bson_atomic_int_fetch_add(volatile int *val, int v, enum bson_memory_order order) {
    return _bson_emul_atomic_int_fetch_add(val, v, order);
}

static BSON_INLINE int bson_atomic_int_fetch_sub(volatile int *val, int v, enum bson_memory_order order) {
    return _bson_emul_atomic_int_fetch_add(val, -v, order);
}

static BSON_INLINE int bson_atomic_int_exchange(volatile int *val, int v, enum bson_memory_order order) {
    return _bson_emul_atomic_int_exchange(val, v, order);
}

static BSON_INLINE int bson_atomic_int_compare_exchange_strong(volatile int *val,
                                                               int expect_value,
                                                               int new_value,
                                                               enum bson_memory_order order) {
...
static BSON_INLINE int bson_atomic_int_compare_exchange_weak(volatile int *val,
                                                             int expect_value,
                                                             int new_value,
                                                             enum bson_memory_order order) {
...
static BSON_INLINE void *bson_atomic_ptr_exchange(void *volatile *ptr, void *new_value, enum bson_memory_order ord) {
...
static BSON_INLINE void *bson_atomic_ptr_compare_exchange_strong(void *volatile *ptr,
                                                                 void *expect,
                                                                 void *new_value,
                                                                 enum bson_memory_order ord) {
    switch (ord) {
...
static BSON_INLINE void *bson_atomic_ptr_fetch(void *const volatile *ptr, enum bson_memory_order ord) {
 ...
 */
static BSON_INLINE void bson_atomic_thread_fence(void) {
