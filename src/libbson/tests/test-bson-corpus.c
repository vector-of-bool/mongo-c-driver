...
static void compare_data(const uint8_t *a, uint32_t a_len, const uint8_t *b, uint32_t b_len) {
    bson_string_t *a_str;
    bson_string_t *b_str;
    uint32_t i;
...
static bool is_test_skipped(const char *scenario, const char *description) {
 ...
 */
static void test_bson_corpus_valid(test_bson_valid_type_t *test) {
    bson_t cB;
    bson_t dB;
    bson_t *decode_cE;
    bson_t *decode_dE;
 ...
 */
static void test_bson_corpus_decode_error(test_bson_decode_error_type_t *test) {
    bson_t invalid_bson;
 ...
 */
static void test_bson_corpus_parse_error(test_bson_parse_error_type_t *test) {
    ...
    case BSON_TYPE_BINARY: ASSERT(!bson_new_from_json((uint8_t *)test->str, test->str_len, NULL)); break;
    ...
    default: fprintf(stderr, "Unsupported parseError type: %#x\n", test->bson_type); abort();
    }
}

static void test_bson_corpus_cb(bson_t *scenario) {
    corpus_test(scenario, test_bson_corpus_valid, test_bson_corpus_decode_error, test_bson_corpus_parse_error);
}

static void test_bson_corpus_prose_1(void) {
    bson_t *bson;
    bool ok;
    bson_t subdoc;
...
void test_bson_corpus_install(TestSuite *suite) {
