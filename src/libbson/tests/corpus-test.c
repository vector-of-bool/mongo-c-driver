...
void corpus_test_print_description(const char *description) {
...
uint8_t *corpus_test_unhexlify(bson_iter_t *iter, uint32_t *bson_str_len) {
    const char *hex_str;
    uint8_t *data = NULL;
    unsigned int byte;
    uint32_t tmp;
...
void corpus_test(bson_t *scenario,
