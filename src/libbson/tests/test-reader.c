...
static void test_reader_from_data(void) {
...
static void test_reader_from_data_overflow(void) {
    bson_reader_t *reader;
    uint8_t *buffer;
    const bson_t *b;
    uint32_t i;
...
static void test_reader_from_data_document_length_too_large(void) {
    bson_reader_t *reader;
    uint8_t *buffer;
    bool eof = false;
...
static void test_reader_from_data_document_length_too_small(void) {
    bson_reader_t *reader;
    uint8_t *buffer;
    bool eof = false;
...
static ssize_t test_reader_from_handle_read(void *handle, void *buf, size_t len) {
    return bson_read(*(int *)handle, buf, len);
}

static void test_reader_from_handle_destroy(void *handle) {
    bson_close(*(int *)handle);
}

static void test_reader_from_handle(void) {
    bson_reader_t *reader;
    const bson_t *b;
    bson_iter_t iter;
    uint32_t i;
...
static void test_reader_tell(void) {
    bson_reader_t *reader;
    const bson_t *b;
    uint32_t i;
    bson_iter_t iter;
...
static void test_reader_from_handle_corrupt(void) {
    bson_reader_t *reader;
    const bson_t *b;
    uint32_t i;
    bson_iter_t iter;
...
static void test_reader_grow_buffer(void) {
    bson_reader_t *reader;
    const bson_t *b;
    bool eof = false;
    int fd;
...
static void test_reader_reset(void) {
    uint8_t buffer[10];
    bson_reader_t *reader;
    bool eof;
...
void test_reader_install(TestSuite *suite) {
