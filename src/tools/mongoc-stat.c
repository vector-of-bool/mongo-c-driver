...
static void mongoc_counters_destroy(mongoc_counters_t *counters) {
    BSON_ASSERT(counters);
    munmap((void *)counters, counters->size);
}

static mongoc_counters_t *mongoc_counters_new_from_pid(unsigned pid) {
...
static mongoc_counter_info_t *mongoc_counters_get_infos(mongoc_counters_t *counters, uint32_t *n_infos) {
...
static void mongoc_counters_print_info(mongoc_counters_t *counters, mongoc_counter_info_t *info, FILE *file) {
