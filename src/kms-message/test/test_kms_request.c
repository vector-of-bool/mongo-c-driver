...
bool skip_aws_test(const char *test_name) {
    size_t i;
...
char *last_segment(const char *str) {
...
char *test_file_path(const char *path, const char *suffix) {
    char *r;
...
void realloc_buffer(char **buffer, size_t *n, size_t len) {
    if (*buffer == NULL) {
        *buffer = malloc(len);
        KMS_ASSERT(*buffer);
...
char *read_test(const char *path, const char *suffix) {
...
void set_test_date(kms_request_t *request) {
    struct tm tm;
...
kms_request_t *read_req(const char *path) {
...
ssize_t first_non_matching(const char *x, const char *y) {
...
void test_compare(kms_request_t *request, char *(*func)(kms_request_t *), const char *dir_path, const char *suffix) {
...
void test_compare_creq(kms_request_t *request, const char *dir_path) {
    test_compare(request, kms_request_get_canonical, dir_path, "creq");
}

void test_compare_sts(kms_request_t *request, const char *dir_path) {
    test_compare(request, kms_request_get_string_to_sign, dir_path, "sts");
}

void test_compare_authz(kms_request_t *request, const char *dir_path) {
    test_compare(request, kms_request_get_signature, dir_path, "authz");
}

void test_compare_sreq(kms_request_t *request, const char *dir_path) {
    test_compare(request, kms_request_get_signed, dir_path, "sreq");
}

void aws_sig_v4_test(const char *dir_path) {
    kms_request_t *request;
...
bool all_aws_sig_v4_tests(const char *path, const char *selected) {
...
void example_signature_test(void) {
...
void path_normalization_test(void) {
    const char *tests[][2] = {
        {"", "/"},
        {"/", "/"},
        {"/..", "/"},
...
kms_request_t *make_test_request(void) {
...
void connection_close_test(void) {
...
void decrypt_request_test(void) {
...
void encrypt_request_test(void) {
...
void kms_response_parser_test(void) {
    FILE *response_file;
...
void kms_request_validate_test(void) {
