...
static void _parser_destroy(kms_response_parser_t *parser) {
...
static void _parser_init(kms_response_parser_t *parser) {
...
kms_response_parser_t *kms_response_parser_new(void) {
...
int kms_response_parser_wants_bytes(kms_response_parser_t *parser, int32_t max) {
    switch (parser->state) {
    case PARSING_DONE: return 0;
    case PARSING_STATUS_LINE:
    case PARSING_HEADER: return max;
    case PARSING_BODY:
...
static bool _parse_int(const char *str, int *result) {
    char *endptr = NULL;
    int64_t long_result;
...
static bool _parse_int_from_view(const char *str, int start, int end, int *result) {
...
static bool _is_lwsp(char c) {
    return c == ' ' || c == 0x09 /* HTAB */;
}

/* parse a header line or status line. */
static kms_response_parser_state_t _parse_line(kms_response_parser_t *parser, int end) {
    int i = parser->start;
...
bool kms_response_parser_feed(kms_response_parser_t *parser, uint8_t *buf, uint32_t len) {
        ...
        case PARSING_BODY:
            ...
            if (body_read == parser->content_length) {
                parser->response->body =
                    kms_request_str_new_from_chars(raw->str + parser->start, parser->content_length);
                parser->state = PARSING_DONE;
            }

            curr = (int)raw->len;
            break;
        case PARSING_DONE: KMS_ERROR(parser, "Unexpected extra HTTP content"); return false;
        }
    }
...
kms_response_t *kms_response_parser_get_response(kms_response_parser_t *parser) {
    kms_response_t *response = parser->response;
...
int kms_response_parser_status(kms_response_parser_t *parser) {
...
const char *kms_response_parser_error(kms_response_parser_t *parser) {
    if (!parser) {
        return NULL;
    }
...
void kms_response_parser_destroy(kms_response_parser_t *parser) {
    _parser_destroy(parser);
    free(parser);
}
