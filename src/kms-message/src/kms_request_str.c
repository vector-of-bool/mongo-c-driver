...
kms_request_str_t *kms_request_str_new(void) {
...
kms_request_str_t *kms_request_str_new_from_chars(const char *chars, ssize_t len) {
    kms_request_str_t *s = malloc(sizeof(kms_request_str_t));
    KMS_ASSERT(s);
...
kms_request_str_t *kms_request_str_wrap(char *chars, ssize_t len) {
    kms_request_str_t *s;
...
void kms_request_str_destroy(kms_request_str_t *str) {
    if (!str) {
        return;
    }
...
char *kms_request_str_detach(kms_request_str_t *str) {
    if (!str) {
        return NULL;
    }
    char *r = str->str;
...
bool kms_request_str_reserve(kms_request_str_t *str, size_t size) {
...
kms_request_str_t *kms_request_str_dup(kms_request_str_t *str) {
...
void kms_request_str_set_chars(kms_request_str_t *str, const char *chars, ssize_t len) {
...
bool kms_request_str_ends_with(kms_request_str_t *str, kms_request_str_t *suffix) {
...
void kms_request_str_append(kms_request_str_t *str, kms_request_str_t *appended) {
...
void kms_request_str_append_char(kms_request_str_t *str, char c) {
...
void kms_request_str_append_chars(kms_request_str_t *str, const char *appended, ssize_t len) {
    if (len < 0) {
        len = strlen(appended);
    }
...
void kms_request_str_append_newline(kms_request_str_t *str) {
    kms_request_str_append_char(str, '\n');
}

void kms_request_str_append_lowercase(kms_request_str_t *str, kms_request_str_t *appended) {
    size_t i;
    char *p;
...
void kms_request_str_appendf(kms_request_str_t *str, const char *format, ...) {
    va_list args;
    size_t remaining;
    int n;
...
void kms_request_str_append_escaped(kms_request_str_t *str, kms_request_str_t *appended, bool escape_slash) {
    uint8_t *in;
    uint8_t *out;
    size_t i;
...
void kms_request_str_append_stripped(kms_request_str_t *str, kms_request_str_t *appended) {
...
bool kms_request_str_append_hashed(_kms_crypto_t *crypto, kms_request_str_t *str, kms_request_str_t *appended) {
    uint8_t hash[32];
    char *hex_chars;
...
bool kms_request_str_append_hex(kms_request_str_t *str, unsigned char *data, size_t len) {
    char *hex_chars;
...
static bool starts_with(char *s, const char *prefix) {
...
static void delete_last_segment(kms_request_str_t *str, bool is_absolute) {
    ssize_t i;
...
kms_request_str_t *kms_request_str_path_normalized(kms_request_str_t *str) {
