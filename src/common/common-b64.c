...
int mcommon_b64_ntop(const uint8_t *src, size_t srclength, char *target, size_t targsize) {
    size_t datalength = 0;
    uint8_t input[3];
    uint8_t output[4];
    size_t i;
...
static int mongoc_b64_pton_do(const char *src, uint8_t *target, size_t targsize) {
    int tarindex, state;
    uint8_t ch, ofs;
    ...
    if (ch == Pad64) { /* We got a pad char. */
        ...
        case 1: /* Invalid = in second position */ return (-1);
...
static int mongoc_b64_pton_len(const char *src) {
    int tarindex, state;
    uint8_t ch, ofs;
...
int mcommon_b64_pton(const char *src, uint8_t *target, size_t targsize) {
...
size_t mcommon_b64_ntop_calculate_target_size(size_t raw_size) {
