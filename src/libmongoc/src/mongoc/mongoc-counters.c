 ...
 */
static bool mongoc_counters_use_shm(void) {
 ...
 */
static size_t mongoc_counters_calc_size(void) {
    size_t n_cpu;
    size_t n_groups;
    size_t size;
 ...
 */
static void *mongoc_counters_alloc(size_t size) {
#if defined(BSON_OS_UNIX) && defined(MONGOC_ENABLE_SHM_COUNTERS)
    void *mem;
    char name[32];
    int pid;
 ...
 */
static size_t mongoc_counters_register(mongoc_counters_t *counters,
                                       uint32_t num,
                                       const char *category,
                                       const char *name,
