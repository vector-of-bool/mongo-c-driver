...
static void _mongoc_topology_server_dtor(void *server_, void *ctx_) {
    BSON_UNUSED(ctx_);
 ...
 *--------------------------------------------------------------------------
 */
void mongoc_topology_description_init(mongoc_topology_description_t *description, int64_t heartbeat_msec) {
    ENTRY;
 ...
 *--------------------------------------------------------------------------
 */
void _mongoc_topology_description_copy_to(const mongoc_topology_description_t *src,
 ...
 *-------------------------------------------------------------------------
 */
mongoc_topology_description_t *mongoc_topology_description_new_copy(const mongoc_topology_description_t *description) {
 ...
 *--------------------------------------------------------------------------
 */
void mongoc_topology_description_cleanup(mongoc_topology_description_t *description) {
    ENTRY;
...
static bool _mongoc_topology_description_has_primary_cb(const void *item, void *ctx /* OUT */) {
 ...
 *--------------------------------------------------------------------------
 */
static bool _mongoc_server_description_primary_is_not_stale(mongoc_topology_description_t *td,
                                                            const mongoc_server_description_t *sd) {
 ...
 *--------------------------------------------------------------------------
 */
static bool _mongoc_topology_description_later_election(mongoc_topology_description_t *td,
                                                        const mongoc_server_description_t *sd) {
    /* initially max_set_version is -1 and max_election_id is zeroed */
 ...
 *--------------------------------------------------------------------------
 */
static void _mongoc_topology_description_set_max_set_version(mongoc_topology_description_t *td,
                                                             const mongoc_server_description_t *sd) {
 ...
 *--------------------------------------------------------------------------
 */
static void _mongoc_topology_description_set_max_election_id(mongoc_topology_description_t *td,
                                                             const mongoc_server_description_t *sd) {
    bson_oid_copy(&sd->election_id, &td->max_election_id);
}

static bool _mongoc_topology_description_server_is_candidate(mongoc_server_description_type_t desc_type,
                                                             mongoc_read_mode_t read_mode,
                                                             mongoc_topology_description_type_t topology_type) {
    switch ((int)topology_type) {
    case MONGOC_TOPOLOGY_SINGLE:
        switch ((int)desc_type) {
        case MONGOC_SERVER_STANDALONE: return true;
        default: return false;
        }
    ...
    case MONGOC_TOPOLOGY_RS_WITH_PRIMARY:
        ...
        case MONGOC_READ_PRIMARY:
            switch ((int)desc_type) {
            case MONGOC_SERVER_RS_PRIMARY: return true;
            default: return false;
            }
        case MONGOC_READ_SECONDARY:
            switch ((int)desc_type) {
            case MONGOC_SERVER_RS_SECONDARY: return true;
            default: return false;
            }
        default:
            switch ((int)desc_type) {
    ...
    case MONGOC_TOPOLOGY_SHARDED:
        switch ((int)desc_type) {
        case MONGOC_SERVER_MONGOS: return true;
        default: return false;
        }
    ...
    case MONGOC_TOPOLOGY_LOAD_BALANCED: return desc_type == MONGOC_SERVER_LOAD_BALANCER;
...
static bool _mongoc_replica_set_read_suitable_cb(const void *item, void *ctx) {
    const mongoc_server_description_t *server = item;
...
static void _mongoc_try_mode_secondary(mongoc_array_t *set, /* OUT */
...
/* if any mongos are candidates, add them to the candidates array */
static bool _mongoc_find_suitable_mongos_cb(const void *item, void *ctx) {
    const mongoc_server_description_t *server = item;
    mongoc_suitable_data_t *data = (mongoc_suitable_data_t *)ctx;
 ...
 *-------------------------------------------------------------------------
 */
int32_t mongoc_topology_description_lowest_max_wire_version(const mongoc_topology_description_t *td) {
 ...
 *-------------------------------------------------------------------------
 */
bool mongoc_topology_description_all_sds_have_write_date(const mongoc_topology_description_t *td) {
 ...
 *-------------------------------------------------------------------------
 */
bool _mongoc_topology_description_validate_max_staleness(const mongoc_topology_description_t *td,
...
static bool _check_any_server_less_than_wire_version_13(const void *sd_, void *any_too_old_) {
 ...
 * (i.e. '$out' and '$merge').
 */
static bool _must_use_primary(const mongoc_topology_description_t *td,
    ...
    default: BSON_UNREACHABLE("Invalid mongoc_ss_optype_t for _must_use_primary()");
    }
}
...
void mongoc_topology_description_suitable_servers(mongoc_array_t *set, /* OUT */
                                                  mongoc_ss_optype_t optype,
                                                  const mongoc_topology_description_t *topology,
                                                  const mongoc_read_prefs_t *read_pref,
                                                  bool *must_use_primary,
    ...
    if (topology->type == MONGOC_TOPOLOGY_RS_NO_PRIMARY || topology->type == MONGOC_TOPOLOGY_RS_WITH_PRIMARY) {
        ...
        default: BSON_UNREACHABLE("Invalid optype");
        }
    }
 ...
 *--------------------------------------------------------------------------
 */
bool mongoc_topology_description_has_data_node(const mongoc_topology_description_t *td) {
    const mongoc_set_t *servers = mc_tpld_servers_const(td);
 ...
 *-------------------------------------------------------------------------
 */
const mongoc_server_description_t *mongoc_topology_description_select(const mongoc_topology_description_t *topology,
                                                                      mongoc_ss_optype_t optype,
                                                                      const mongoc_read_prefs_t *read_pref,
                                                                      bool *must_use_primary,
                                                                      int64_t local_threshold_ms) {
 ...
 *--------------------------------------------------------------------------
 */
static void _mongoc_topology_description_remove_server(mongoc_topology_description_t *description,
                                                       const mongoc_server_description_t *server) {
    BSON_ASSERT(description);
    BSON_ASSERT(server);
...
static bool _mongoc_topology_description_has_server_cb(const void *item, void *ctx /* IN - OUT */) {
    const mongoc_server_description_t *server = item;
 ...
 *--------------------------------------------------------------------------
 */
static bool _mongoc_topology_description_has_set_version(mongoc_topology_description_t *td) {
 ...
 *--------------------------------------------------------------------------
 */
static bool _mongoc_topology_description_has_server(mongoc_topology_description_t *description,
                                                    const char *address,
                                                    uint32_t *id /* OUT */) {
...
static bool _mongoc_label_unknown_member_cb(void *item, void *ctx) {
 ...
 *--------------------------------------------------------------------------
 */
static void _mongoc_topology_description_label_unknown_member(mongoc_topology_description_t *description,
                                                              const char *address,
                                                              mongoc_server_description_type_t type) {
 ...
 *--------------------------------------------------------------------------
 */
static void _mongoc_topology_description_set_state(mongoc_topology_description_t *description,
                                                   mongoc_topology_description_type_t type) {
    description->type = type;
}

static void _update_rs_type(mongoc_topology_description_t *topology) {
 ...
 *--------------------------------------------------------------------------
 */
static void _mongoc_topology_description_check_if_has_primary(mongoc_topology_description_t *topology,
                                                              const mongoc_server_description_t *server) {
    BSON_UNUSED(server);
 ...
 */
void mongoc_topology_description_invalidate_server(mongoc_topology_description_t *td,
                                                   uint32_t id,
 ...
 *--------------------------------------------------------------------------
 */
bool mongoc_topology_description_add_server(mongoc_topology_description_t *topology,
                                            const char *server,
                                            uint32_t *id /* OUT */) {
    uint32_t server_id;
...
void mongoc_topology_description_update_cluster_time(mongoc_topology_description_t *td, const bson_t *reply) {
    bson_iter_t iter;
    bson_iter_t child;
    const uint8_t *data;
    uint32_t size;
...
static void _mongoc_topology_description_add_new_servers(mongoc_topology_description_t *topology,
                                                         const mongoc_server_description_t *server) {
...
static bool _mongoc_topology_description_invalidate_primaries_cb(void *item, void *ctx) {
    mongoc_server_description_t *server = (mongoc_server_description_t *)item;
...
static void _mongoc_topology_description_remove_unreported_servers(mongoc_topology_description_t *topology,
                                                                   const mongoc_server_description_t *primary) {
 ...
 *--------------------------------------------------------------------------
 */
static bool _mongoc_topology_description_matches_me(const mongoc_server_description_t *server) {
 ...
 *--------------------------------------------------------------------------
 */
static void _mongoc_topology_description_update_rs_from_primary(mongoc_topology_description_t *topology,
                                                                const mongoc_server_description_t *server) {
 ...
 *--------------------------------------------------------------------------
 */
static void _mongoc_topology_description_update_rs_without_primary(mongoc_topology_description_t *topology,
                                                                   const mongoc_server_description_t *server) {
    BSON_ASSERT(topology);
    BSON_ASSERT(server);
 ...
 *--------------------------------------------------------------------------
 */
static void _mongoc_topology_description_update_rs_with_primary_from_member(mongoc_topology_description_t *topology,
                                                                            const mongoc_server_description_t *server) {
    BSON_ASSERT(topology);
    BSON_ASSERT(server);
 ...
 *--------------------------------------------------------------------------
 */
static void _mongoc_topology_description_set_topology_type_to_sharded(mongoc_topology_description_t *topology,
                                                                      const mongoc_server_description_t *server) {
    BSON_UNUSED(server);
 ...
 *--------------------------------------------------------------------------
 */
static void _mongoc_topology_description_remove_and_check_primary(mongoc_topology_description_t *topology,
                                                                  const mongoc_server_description_t *server) {
    _mongoc_topology_description_remove_server(topology, server);
    _update_rs_type(topology);
}
 ...
 *--------------------------------------------------------------------------
 */
static void _mongoc_topology_description_update_unknown_with_standalone(mongoc_topology_description_t *topology,
                                                                        const mongoc_server_description_t *server) {
    BSON_ASSERT(topology);
    BSON_ASSERT(server);
 ...
 *--------------------------------------------------------------------------
 */
static const char *_tpld_type_str(mongoc_topology_description_type_t type) {
    switch (type) {
    case MONGOC_TOPOLOGY_UNKNOWN: return "Unknown";
    case MONGOC_TOPOLOGY_SHARDED: return "Sharded";
    case MONGOC_TOPOLOGY_RS_NO_PRIMARY: return "RSNoPrimary";
    case MONGOC_TOPOLOGY_RS_WITH_PRIMARY: return "RSWithPrimary";
    case MONGOC_TOPOLOGY_SINGLE: return "Single";
    case MONGOC_TOPOLOGY_LOAD_BALANCED: return "LoadBalanced";
    case MONGOC_TOPOLOGY_DESCRIPTION_TYPES:
    default: MONGOC_ERROR("Invalid mongoc_topology_description_type_t type"); return "Invalid";
    }
}
...
static void _mongoc_topology_description_update_session_timeout(mongoc_topology_description_t *td) {
    mongoc_set_t *set;
...
static void _mongoc_topology_description_check_compatible(mongoc_topology_description_t *td) {
    const mongoc_set_t *const servers = mc_tpld_servers_const(td);

    memset(&td->compatibility_error, 0, sizeof(bson_error_t));

    for (size_t i = 0; i < servers->items_len; i++) {
        const mongoc_server_description_t *const sd = mongoc_set_get_item_const(servers, i);
        if (sd->type == MONGOC_SERVER_UNKNOWN || sd->type == MONGOC_SERVER_POSSIBLE_PRIMARY) {
            continue;
        }
...
void mongoc_topology_description_handle_hello(mongoc_topology_description_t *topology,
                                              uint32_t server_id,
 ...
 *--------------------------------------------------------------------------
 */
bool mongoc_topology_description_has_readable_server(const mongoc_topology_description_t *td,
 ...
 *--------------------------------------------------------------------------
 */
bool mongoc_topology_description_has_writable_server(const mongoc_topology_description_t *td) {
    bson_error_t error;
...
const char *mongoc_topology_description_type(const mongoc_topology_description_t *td) {
    ...
    case MONGOC_TOPOLOGY_UNKNOWN: return "Unknown";
    case MONGOC_TOPOLOGY_SHARDED: return "Sharded";
    case MONGOC_TOPOLOGY_RS_NO_PRIMARY: return "ReplicaSetNoPrimary";
    case MONGOC_TOPOLOGY_RS_WITH_PRIMARY: return "ReplicaSetWithPrimary";
    case MONGOC_TOPOLOGY_SINGLE: return "Single";
    case MONGOC_TOPOLOGY_LOAD_BALANCED: return "LoadBalanced";
    case MONGOC_TOPOLOGY_DESCRIPTION_TYPES:
    default: fprintf(stderr, "ERROR: Unknown topology type %d\n", td->type); BSON_ASSERT(0);
    }
...
static bool _count_num_hosts_to_remove(void *sd_void, void *ctx_void) {
    mongoc_server_description_t *sd;
...
static bool _remove_if_not_in_host_list_cb(void *sd_void, void *ctx_void) {
...
void mongoc_topology_description_reconcile(mongoc_topology_description_t *td, mongoc_host_list_t *host_list) {
...
void _mongoc_topology_description_clear_connection_pool(mongoc_topology_description_t *td,
                                                        uint32_t server_id,
