...
static int cache_cmp(cache_entry_list_t *out, OCSP_CERTID *id) {
    ENTRY;
    if (!out || !out->id || !id) {
        RETURN(1);
    }
...
static cache_entry_list_t *get_cache_entry(OCSP_CERTID *id) {
...
static void update_entry(cache_entry_list_t *entry,
                         int cert_status,
                         int reason,
...
static int _cmp_time(ASN1_TIME *a, ASN1_TIME *b) {
...
static void cache_entry_destroy(cache_entry_list_t *entry) {
