...
const mongoc_ssl_opt_t *mongoc_ssl_opt_get_default(void) {
...
void _mongoc_ssl_opts_from_uri(mongoc_ssl_opt_t *ssl_opt, _mongoc_internal_tls_opts_t *internal, mongoc_uri_t *uri) {
...
void _mongoc_ssl_opts_copy_to(const mongoc_ssl_opt_t *src, mongoc_ssl_opt_t *dst, bool copy_internal) {
    BSON_ASSERT(src);
    BSON_ASSERT(dst);
...
void _mongoc_ssl_opts_cleanup(mongoc_ssl_opt_t *opt, bool free_internal) {
...
bool _mongoc_ssl_opts_disable_certificate_revocation_check(const mongoc_ssl_opt_t *ssl_opt) {
    if (!ssl_opt->internal) {
        return false;
    }
...
bool _mongoc_ssl_opts_disable_ocsp_endpoint_check(const mongoc_ssl_opt_t *ssl_opt) {
    if (!ssl_opt->internal) {
        return false;
    }
...
bool _mongoc_ssl_opts_from_bson(mongoc_ssl_opt_t *ssl_opt, const bson_t *bson, bson_string_t *errmsg) {
    bson_iter_t iter;
    ...
    while (bson_iter_next(&iter)) {
        ...
        bson_string_append_printf(errmsg,
