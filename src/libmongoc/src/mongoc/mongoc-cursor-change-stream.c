...
static void _update_post_batch_resume_token(mongoc_cursor_t *cursor) {
    ...
    if (bson_iter_init(&iter, &data->response.reply)
        && bson_iter_find_descendant(&iter, "cursor.postBatchResumeToken", &child)
...
static mongoc_cursor_state_t _prime(mongoc_cursor_t *cursor) {
    BSON_UNUSED(cursor);
...
static mongoc_cursor_state_t _pop_from_batch(mongoc_cursor_t *cursor) {
    _data_change_stream_t *data = (_data_change_stream_t *)cursor->impl.data;
...
mongoc_cursor_state_t _get_next_batch(mongoc_cursor_t *cursor) {
    _data_change_stream_t *data = (_data_change_stream_t *)cursor->impl.data;
    bson_t getmore_cmd;
...
static void _destroy(mongoc_cursor_impl_t *impl) {
...
static void _clone(mongoc_cursor_impl_t *dst, const mongoc_cursor_impl_t *src) {
    BSON_UNUSED(dst);
    BSON_UNUSED(src);
...
mongoc_cursor_t *_mongoc_cursor_change_stream_new(mongoc_client_t *client, bson_t *reply, const bson_t *getmore_opts) {
...
static bool _bson_iter_has_next(bson_iter_t *iter) {
