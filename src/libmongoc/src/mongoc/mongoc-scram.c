...
static int _scram_hash_size(mongoc_scram_t *scram) {
...
static void _mongoc_scram_cache_apply_secrets(mongoc_scram_cache_t *cache, mongoc_scram_t *scram) {
    BSON_ASSERT(cache);
    BSON_ASSERT(scram);
...
static mongoc_scram_cache_t *_mongoc_scram_cache_copy(const mongoc_scram_cache_t *cache) {
...
void _mongoc_scram_cache_destroy(mongoc_scram_cache_t *cache) {
    BSON_ASSERT(cache);
...
static bool _mongoc_scram_cache_has_presecrets(mongoc_scram_cache_t *cache, mongoc_scram_t *scram) {
    BSON_ASSERT(cache);
    BSON_ASSERT(scram);

    return cache->hashed_password && scram->hashed_password && !strcmp(cache->hashed_password, scram->hashed_password)
        && cache->iterations == scram->iterations
        && !memcmp(cache->decoded_salt, scram->decoded_salt, sizeof(cache->decoded_salt));
}

mongoc_scram_cache_t *_mongoc_scram_get_cache(mongoc_scram_t *scram) {
    BSON_ASSERT(scram);

    return _mongoc_scram_cache_copy(scram->cache);
}

void _mongoc_scram_set_cache(mongoc_scram_t *scram, mongoc_scram_cache_t *cache) {
    BSON_ASSERT(scram);
...
void _mongoc_scram_set_pass(mongoc_scram_t *scram, const char *pass) {
    BSON_ASSERT(scram);
...
void _mongoc_scram_set_user(mongoc_scram_t *scram, const char *user) {
    BSON_ASSERT(scram);
...
void _mongoc_scram_init(mongoc_scram_t *scram, mongoc_crypto_hash_algorithm_t algo) {
    BSON_ASSERT(scram);
...
void _mongoc_scram_destroy(mongoc_scram_t *scram) {
    BSON_ASSERT(scram);
...
static void _mongoc_scram_update_cache(mongoc_scram_t *scram) {
 ...
 */
static bool _mongoc_scram_start(mongoc_scram_t *scram,
                                uint8_t *outbuf,
                                uint32_t outbufmax,
                                uint32_t *outbuflen,
                                bson_error_t *error) {
...
static void _mongoc_scram_salt_password(mongoc_scram_t *scram,
                                        const char *password,
 ...
 */
static bool _mongoc_scram_step2(mongoc_scram_t *scram,
                                const uint8_t *inbuf,
                                uint32_t inbuflen,
                                uint8_t *outbuf,
                                uint32_t outbufmax,
...
static bool _mongoc_scram_verify_server_signature(mongoc_scram_t *scram, uint8_t *verification, uint32_t len) {
    ...
    encoded_server_signature_len = mcommon_b64_ntop(server_signature,
                                                    _scram_hash_size(scram),
...
static bool _mongoc_scram_step3(mongoc_scram_t *scram,
                                const uint8_t *inbuf,
                                uint32_t inbuflen,
                                uint8_t *outbuf,
                                uint32_t outbufmax,
...
bool _mongoc_scram_step(mongoc_scram_t *scram,
                        const uint8_t *inbuf,
                        uint32_t inbuflen,
                        uint8_t *outbuf,
                        uint32_t outbufmax,
    ...
    switch (scram->step) {
    case 1: return _mongoc_scram_start(scram, outbuf, outbufmax, outbuflen, error);
    case 2: return _mongoc_scram_step2(scram, inbuf, inbuflen, outbuf, outbufmax, outbuflen, error);
    case 3: return _mongoc_scram_step3(scram, inbuf, inbuflen, outbuf, outbufmax, outbuflen, error);
    default:
...
bool _mongoc_sasl_prep_required(const char *str) {
    unsigned char c;
    while (*str) {
        c = (unsigned char)*str;
...
#ifdef MONGOC_ENABLE_ICU
char *_mongoc_sasl_prep_impl(const char *name, const char *in_utf8, int in_utf8_len, bson_error_t *err) {
...
#define SASL_PREP_ERR_RETURN(msg)                                                                                      \
    ...
    (void)usprep_prepare(prep, in_utf16, in_utf16_len, out_utf16, out_utf16_len + 1, USPREP_DEFAULT, NULL, &error_code);
    if (error_code) {
        bson_free(in_utf16);
        bson_free(out_utf16);
        usprep_close(prep);
...
char *_mongoc_sasl_prep(const char *in_utf8, int in_utf8_len, bson_error_t *err) {
