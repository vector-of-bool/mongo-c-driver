...
static void _set_bit(uint8_t *bf, uint32_t byte_count, uint32_t bit) {
    uint32_t byte = bit / 8;
...
char *_mongoc_handshake_get_config_hex_string(void) {
...
static char *_get_os_architecture(void) {
    const char *ret = NULL;
    ...
    switch (arch) {
    case PROCESSOR_ARCHITECTURE_AMD64: ret = "x86_64"; break;
    case PROCESSOR_ARCHITECTURE_ARM: ret = "ARM"; break;
    case PROCESSOR_ARCHITECTURE_IA64: ret = "IA64"; break;
    case PROCESSOR_ARCHITECTURE_INTEL: ret = "x86"; break;
    case PROCESSOR_ARCHITECTURE_UNKNOWN: ret = "Unknown"; break;
...
static char *_get_os_version(void) {
...
static void _get_system_info(mongoc_handshake_t *handshake) {
...
static void _free_system_info(mongoc_handshake_t *handshake) {
...
static void _get_driver_info(mongoc_handshake_t *handshake) {
...
static void _free_driver_info(mongoc_handshake_t *handshake) {
    bson_free(handshake->driver_name);
    bson_free(handshake->driver_version);
}

static void _set_platform_string(mongoc_handshake_t *handshake) {
    bson_string_t *str;
...
static void _set_compiler_info(mongoc_handshake_t *handshake) {
    bson_string_t *str;
    char *config_str;
...
static void _set_flags(mongoc_handshake_t *handshake) {
    bson_string_t *str;
...
static void _free_platform_string(mongoc_handshake_t *handshake) {
    bson_free(handshake->platform);
    bson_free(handshake->compiler_info);
    bson_free(handshake->flags);
}

void _mongoc_handshake_init(void) {
...
void _mongoc_handshake_cleanup(void) {
...
static void _append_platform_field(bson_t *doc, const char *platform) {
    ...
    const int max_platform_str_size = HANDSHAKE_MAX_SIZE
                                    - ((int)doc->len +
                                       /* 1 byte for utf8 tag */
                                       1 +
     ...
     * compiler info */
    if (bson_cmp_greater_su(max_platform_str_size, combined_platform->len + strlen(compiler_info) + 1u)) {
        bson_string_append(combined_platform, compiler_info);
    }
    if (bson_cmp_greater_su(max_platform_str_size, combined_platform->len + strlen(flags) + 1u)) {
 ...
 */
bool _mongoc_handshake_build_doc_with_application(bson_t *doc, const char *appname) {
...
void _mongoc_handshake_freeze(void) {
 ...
 */
static void _append_and_truncate(char **s, const char *suffix, size_t max_len) {
    char *old_str = *s;
 ...
 */
bool mongoc_handshake_data_append(const char *driver_name, const char *driver_version, const char *platform) {
    int platform_space;
...
mongoc_handshake_t *_mongoc_handshake_get(void) {
    return &gMongocHandshake;
}

bool _mongoc_handshake_appname_is_valid(const char *appname) {
    return strlen(appname) <= MONGOC_HANDSHAKE_APPNAME_MAX;
}

void _mongoc_handshake_append_sasl_supported_mechs(const mongoc_uri_t *uri, bson_t *cmd) {
    const char *username;
    char *db_user;
...
void _mongoc_handshake_parse_sasl_supported_mechs(const bson_t *hello,
