    ...
    do {                                                                                                               \
        ...
        for (_i = 0; _i < rpc->n_##_name; _i++) {                                                                      \
            ...
            case 1:                                                                                                    \
                ...
                break;                                                                                                 \
            default: MONGOC_ERROR("Unknown Payload Type: %d", rpc->_name[_i].payload_type); BSON_ASSERT(0);            \
            }                                                                                                          \
            header->msg_len += (int32_t)iov.iov_len;                                                                   \
            _mongoc_array_append_val(array, iov);                                                                      \
        }                                                                                                              \
...
#define CHECKSUM_FIELD(_name) printf("  " #_name " : %" PRIu32 "\n", rpc->_name);
    ...
    do {                                                                                                               \
        ...
        for (ssize_t _i = 0; _i < rpc->n_##_name; _i++) {                                                              \
            if (rpc->_name[_i].payload_type == 0) {                                                                    \
                ...
                bson_reader_t *__r;                                                                                    \
                BSON_ASSERT(bson_in_range_signed(size_t, rpc->_name[_i].payload.sequence.size));                       \
                const size_t max_size = (size_t)rpc->_name[_i].payload.sequence.size                                   \
                                      - strlen(rpc->_name[_i].payload.sequence.identifier) - 1u - sizeof(int32_t);     \
                bool __eof;                                                                                            \
                const bson_t *__b;                                                                                     \
                printf("  Identifier: %s\n", rpc->_name[_i].payload.sequence.identifier);                              \
                printf("  Size: %zu\n", max_size);                                                                     \
                __r = bson_reader_new_from_data(rpc->_name[_i].payload.sequence.bson_documents, max_size);             \
                while ((__b = bson_reader_read(__r, &__eof))) {                                                        \
                    char *s = bson_as_relaxed_extended_json(__b, NULL);                                                \
                    bson_free(s);                                                                                      \
                }                                                                                                      \
...
void _mongoc_rpc_gather(mongoc_rpc_t *rpc, mongoc_array_t *array) {
    mongoc_counter_op_egress_total_inc();
    switch ((mongoc_opcode_t)rpc->header.opcode) {
    case MONGOC_OPCODE_REPLY: _mongoc_rpc_gather_reply(&rpc->reply, &rpc->header, array); return;
...
void _mongoc_rpc_swab_to_le(mongoc_rpc_t *rpc) {
...
#if BSON_BYTE_ORDER != BSON_LITTLE_ENDIAN
    {
        ...
        switch (opcode) {
        case MONGOC_OPCODE_REPLY: _mongoc_rpc_swab_to_le_reply(&rpc->reply); break;
        case MONGOC_OPCODE_MSG: _mongoc_rpc_swab_to_le_msg(&rpc->msg); break;
        case MONGOC_OPCODE_UPDATE: _mongoc_rpc_swab_to_le_update(&rpc->update); break;
        case MONGOC_OPCODE_INSERT: _mongoc_rpc_swab_to_le_insert(&rpc->insert); break;
        case MONGOC_OPCODE_QUERY: _mongoc_rpc_swab_to_le_query(&rpc->query); break;
        case MONGOC_OPCODE_GET_MORE: _mongoc_rpc_swab_to_le_get_more(&rpc->get_more); break;
        case MONGOC_OPCODE_DELETE: _mongoc_rpc_swab_to_le_delete(&rpc->delete_); break;
        case MONGOC_OPCODE_KILL_CURSORS: _mongoc_rpc_swab_to_le_kill_cursors(&rpc->kill_cursors); break;
        case MONGOC_OPCODE_COMPRESSED: _mongoc_rpc_swab_to_le_compressed(&rpc->compressed); break;
        default: MONGOC_WARNING("Unknown rpc type: 0x%08x", opcode); break;
        }
    }
#endif
#if 0
...
void _mongoc_rpc_swab_from_le(mongoc_rpc_t *rpc) {
...
#if BSON_BYTE_ORDER != BSON_LITTLE_ENDIAN
    {
        ...
        switch (opcode) {
        case MONGOC_OPCODE_REPLY: _mongoc_rpc_swab_from_le_reply(&rpc->reply); break;
        case MONGOC_OPCODE_MSG: _mongoc_rpc_swab_from_le_msg(&rpc->msg); break;
        case MONGOC_OPCODE_UPDATE: _mongoc_rpc_swab_from_le_update(&rpc->update); break;
        case MONGOC_OPCODE_INSERT: _mongoc_rpc_swab_from_le_insert(&rpc->insert); break;
        case MONGOC_OPCODE_QUERY: _mongoc_rpc_swab_from_le_query(&rpc->query); break;
        case MONGOC_OPCODE_GET_MORE: _mongoc_rpc_swab_from_le_get_more(&rpc->get_more); break;
        case MONGOC_OPCODE_DELETE: _mongoc_rpc_swab_from_le_delete(&rpc->delete_); break;
        case MONGOC_OPCODE_KILL_CURSORS: _mongoc_rpc_swab_from_le_kill_cursors(&rpc->kill_cursors); break;
        case MONGOC_OPCODE_COMPRESSED: _mongoc_rpc_swab_from_le_compressed(&rpc->compressed); break;
        default: MONGOC_WARNING("Unknown rpc type: 0x%08x", rpc->header.opcode); break;
        }
    }
#endif
#if 0
...
void _mongoc_rpc_printf(mongoc_rpc_t *rpc) {
    switch ((mongoc_opcode_t)rpc->header.opcode) {
    case MONGOC_OPCODE_REPLY: _mongoc_rpc_printf_reply(&rpc->reply); break;
    case MONGOC_OPCODE_MSG: _mongoc_rpc_printf_msg(&rpc->msg); break;
    case MONGOC_OPCODE_UPDATE: _mongoc_rpc_printf_update(&rpc->update); break;
    case MONGOC_OPCODE_INSERT: _mongoc_rpc_printf_insert(&rpc->insert); break;
    case MONGOC_OPCODE_QUERY: _mongoc_rpc_printf_query(&rpc->query); break;
    case MONGOC_OPCODE_GET_MORE: _mongoc_rpc_printf_get_more(&rpc->get_more); break;
    case MONGOC_OPCODE_DELETE: _mongoc_rpc_printf_delete(&rpc->delete_); break;
    case MONGOC_OPCODE_KILL_CURSORS: _mongoc_rpc_printf_kill_cursors(&rpc->kill_cursors); break;
    case MONGOC_OPCODE_COMPRESSED: _mongoc_rpc_printf_compressed(&rpc->compressed); break;
    default: MONGOC_WARNING("Unknown rpc type: 0x%08x", rpc->header.opcode); break;
    }
    printf("\n");
}
...
bool _mongoc_rpc_decompress(mongoc_rpc_t *rpc_le, uint8_t *buf, size_t buflen) {
...
char *_mongoc_rpc_compress(struct _mongoc_cluster_t *cluster,
    ...
    const size_t size = _mongoc_cluster_buffer_iovec(cluster->iov.data, cluster->iov.len, 16, data);
    size_t output_length = mongoc_compressor_max_compressed_length(compressor_id, size);
...
bool _mongoc_rpc_scatter(mongoc_rpc_t *rpc, const uint8_t *buf, size_t buflen) {
    mongoc_opcode_t opcode;
    ...
    case MONGOC_OPCODE_MSG: mongoc_counter_op_ingress_msg_inc(); return _mongoc_rpc_scatter_msg(&rpc->msg, buf, buflen);

    /* useless, we are never *getting* these opcodes */
    case MONGOC_OPCODE_UPDATE: return _mongoc_rpc_scatter_update(&rpc->update, buf, buflen);

    case MONGOC_OPCODE_INSERT: return _mongoc_rpc_scatter_insert(&rpc->insert, buf, buflen);

    case MONGOC_OPCODE_QUERY: return _mongoc_rpc_scatter_query(&rpc->query, buf, buflen);

    case MONGOC_OPCODE_GET_MORE: return _mongoc_rpc_scatter_get_more(&rpc->get_more, buf, buflen);

    case MONGOC_OPCODE_DELETE: return _mongoc_rpc_scatter_delete(&rpc->delete_, buf, buflen);

    case MONGOC_OPCODE_KILL_CURSORS: return _mongoc_rpc_scatter_kill_cursors(&rpc->kill_cursors, buf, buflen);

    default: MONGOC_WARNING("Unknown rpc type: 0x%08x", opcode); return false;
    }
}

bool _mongoc_rpc_scatter_reply_header_only(mongoc_rpc_t *rpc, const uint8_t *buf, size_t buflen) {
...
bool _mongoc_rpc_get_first_document(mongoc_rpc_t *rpc, bson_t *reply) {
...
bool _mongoc_rpc_reply_get_first_msg(mongoc_rpc_msg_t *reply_msg, bson_t *bson_reply) {
    ...
    return bson_init_static(bson_reply, reply_msg->sections[0].payload.bson_document, document_len);
}

bool _mongoc_rpc_reply_get_first(mongoc_rpc_reply_t *reply, bson_t *bson) {
    int32_t len;
...
void _mongoc_rpc_prep_command(mongoc_rpc_t *rpc, const char *cmd_ns, mongoc_cmd_t *cmd)
...
static bool _parse_error_reply(const bson_t *doc, bool check_wce, uint32_t *code, const char **msg) {
    bson_iter_t iter;
 ...
 *--------------------------------------------------------------------------
 */
bool _mongoc_cmd_check_ok(const bson_t *doc, int32_t error_api_version, bson_error_t *error) {
 ...
 *--------------------------------------------------------------------------
 */
bool _mongoc_cmd_check_ok_no_wce(const bson_t *doc, int32_t error_api_version, bson_error_t *error) {
    mongoc_error_domain_t domain =
        error_api_version >= MONGOC_ERROR_API_VERSION_2 ? MONGOC_ERROR_SERVER : MONGOC_ERROR_QUERY;
    uint32_t code;
    const char *msg = "Unknown command error";
...
static void _mongoc_populate_query_error(const bson_t *doc, int32_t error_api_version, bson_error_t *error) {
    mongoc_error_domain_t domain =
        error_api_version >= MONGOC_ERROR_API_VERSION_2 ? MONGOC_ERROR_SERVER : MONGOC_ERROR_QUERY;
...
bool _mongoc_rpc_check_ok(mongoc_rpc_t *rpc,
 ...
 */
bool _mongoc_rpc_decompress_if_necessary(mongoc_rpc_t *rpc,
