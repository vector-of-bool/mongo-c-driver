...
mongoc_stream_t *_mongoc_interrupt_get_stream(mongoc_interrupt_t *interrupt) {
    return interrupt->stream;
}

static void _log_errno(char *prefix, int _errno) {
    char buf[128] = {0};
...
mongoc_interrupt_t *_mongoc_interrupt_new(uint32_t timeout_ms) {
...
void _mongoc_interrupt_destroy(mongoc_interrupt_t *interrupt) {
    if (!interrupt) {
        return;
    }
...
bool _mongoc_interrupt_flush(mongoc_interrupt_t *interrupt) {
    uint8_t buf[1];
    while (true) {
...
bool _mongoc_interrupt_interrupt(mongoc_interrupt_t *interrupt) {
...
static bool _set_pipe_flags(int pipe_fd) {
    int flags;
