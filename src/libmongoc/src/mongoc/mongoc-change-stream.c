...
static bool _is_resumable_error(mongoc_change_stream_t *stream, const bson_t *reply) {
    ...
    case MONGOC_SERVER_ERR_FAILEDTOSATISFYREADPREFERENCE: return true;
    default: return false;
    }
}

static void _set_resume_token(mongoc_change_stream_t *stream, const bson_t *resume_token) {
    BSON_ASSERT(stream);
 ...
 *     cursor: { batchSize: x } }
 */
static void _make_command(mongoc_change_stream_t *stream, bson_t *command) {
    bson_iter_t iter;
 ...
 *--------------------------------------------------------------------------
 */
static bool _make_cursor(mongoc_change_stream_t *stream) {
    ...
    server_stream =
        mongoc_cluster_stream_for_reads(&stream->client->cluster, stream->read_prefs, cs, &reply, &stream->err);
    if (!server_stream) {
 ...
 *--------------------------------------------------------------------------
 */
void _change_stream_init(mongoc_change_stream_t *stream, const bson_t *pipeline, const bson_t *opts) {
...
bool mongoc_change_stream_error_document(const mongoc_change_stream_t *stream, bson_error_t *err, const bson_t **bson) {
    BSON_ASSERT(stream);
