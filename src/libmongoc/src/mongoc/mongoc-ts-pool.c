 ...
 */
static size_t _pool_node_data_offset(const mongoc_ts_pool *pool) {
 ...
 */
static pool_node *_pool_node_new(const mongoc_ts_pool *pool) {
    BSON_ASSERT_PARAM(pool);
 ...
 */
static const void *_pool_node_get_data_const(const pool_node *node) {
 ...
 * @brief Return a pointer to the item owned by the given node.
 */
static void *_pool_node_get_data(pool_node *node) {
    BSON_ASSERT_PARAM(node);
    return node->data + _pool_node_data_offset(node->owner_pool);
}
 ...
 */
static pool_node *_pool_node_from_item(void *item, const mongoc_ts_pool *pool) {
 ...
 */
static bool _should_prune(const pool_node *node) {
 ...
 */
static pool_node *_new_item(mongoc_ts_pool *pool, bson_error_t *error) {
 ...
 */
static void _delete_item(pool_node *node) {
    mongoc_ts_pool *pool = node->owner_pool;
 ...
 */
static pool_node *_try_get(mongoc_ts_pool *pool) {
    pool_node *node;
...
mongoc_ts_pool *mongoc_ts_pool_new(mongoc_ts_pool_params params) {
...
void mongoc_ts_pool_free(mongoc_ts_pool *pool) {
    if (AUDIT_POOL_ENABLED) {
...
void mongoc_ts_pool_clear(mongoc_ts_pool *pool) {
    pool_node *node;
    {
        bson_mutex_lock(&pool->mtx);
        node = pool->head;
...
void *mongoc_ts_pool_get_existing(mongoc_ts_pool *pool) {
    pool_node *node;
retry:
    node = _try_get(pool);
...
void *mongoc_ts_pool_get(mongoc_ts_pool *pool, bson_error_t *error) {
    pool_node *node;
retry:
    node = _try_get(pool);
    if (node && _should_prune(node)) {
...
void mongoc_ts_pool_return(mongoc_ts_pool *pool, void *item) {
...
void mongoc_ts_pool_drop(mongoc_ts_pool *pool, void *item) {
    pool_node *node = _pool_node_from_item(item, pool);
...
int mongoc_ts_pool_is_empty(const mongoc_ts_pool *pool) {
    return mongoc_ts_pool_size(pool) == 0;
}

size_t mongoc_ts_pool_size(const mongoc_ts_pool *pool) {
    return bson_atomic_int32_fetch(&pool->size, bson_memory_order_relaxed);
}

void mongoc_ts_pool_visit_each(mongoc_ts_pool *pool,
                               void *visit_userdata,
