...
static void _async_connected(mongoc_async_cmd_t *acmd);

static void _async_success(mongoc_async_cmd_t *acmd, const bson_t *hello_response, int64_t duration_usec);

static void _async_error_or_timeout(mongoc_async_cmd_t *acmd, int64_t duration_usec, const char *default_err_msg);

static void _async_handler(mongoc_async_cmd_t *acmd,
                           mongoc_async_cmd_result_t async_status,
                           const bson_t *hello_response,
                           int64_t duration_usec);

static void _mongoc_topology_scanner_monitor_heartbeat_started(const mongoc_topology_scanner_t *ts,
                                                               const mongoc_host_list_t *host);

static void _mongoc_topology_scanner_monitor_heartbeat_succeeded(const mongoc_topology_scanner_t *ts,
                                                                 const mongoc_host_list_t *host,
                                                                 const bson_t *reply,
                                                                 int64_t duration_usec);

static void _mongoc_topology_scanner_monitor_heartbeat_failed(const mongoc_topology_scanner_t *ts,
                                                              const mongoc_host_list_t *host,
                                                              const bson_error_t *error,
                                                              int64_t duration_usec);

/* reset "retired" nodes that failed or were removed in the previous scan */
static void _delete_retired_nodes(mongoc_topology_scanner_t *ts);
...
static void _cancel_commands_excluding(mongoc_topology_scanner_node_t *node, mongoc_async_cmd_t *acmd);

/* return the number of pending async commands for a node. */
static int _count_acmds(mongoc_topology_scanner_node_t *node);

/* if acmd fails, schedule the sibling commands sooner. */
static void _jumpstart_other_acmds(mongoc_topology_scanner_node_t *node, mongoc_async_cmd_t *acmd);

static void _add_hello(mongoc_topology_scanner_t *ts) {
...
static void _init_hello(mongoc_topology_scanner_t *ts) {
...
static void _reset_hello(mongoc_topology_scanner_t *ts) {
    bson_t *prev_cmd;
...
const char *_mongoc_topology_scanner_get_speculative_auth_mechanism(const mongoc_uri_t *uri) {
...
void _mongoc_topology_scanner_add_speculative_authentication(bson_t *cmd,
...
void _mongoc_topology_scanner_parse_speculative_authentication(const bson_t *hello, bson_t *speculative_authenticate) {
    bson_iter_t iter;
    uint32_t data_len;
    const uint8_t *data;
    ...
    if (uri) {
        ...
        if (bson_iter_init(&iter, compressors)) {
            while (bson_iter_next(&iter)) {
                const size_t keylen = bson_uint32_to_string(count++, &key, buf, sizeof(buf));
...
const bson_t *_mongoc_topology_scanner_get_monitoring_cmd(mongoc_topology_scanner_t *ts, bool hello_ok) {
    return hello_ok || mongoc_topology_scanner_uses_server_api(ts) ? &ts->hello_cmd : &ts->legacy_hello_cmd;
}

void _mongoc_topology_scanner_dup_handshake_cmd(mongoc_topology_scanner_t *ts, bson_t *copy_into) {
    bson_t *new_cmd;
    const char *appname;
    ...
    bson_mutex_unlock(&ts->handshake_cmd_mtx);
    new_cmd = _build_handshake_cmd(mongoc_topology_scanner_uses_server_api(ts) ? &ts->hello_cmd : &ts->legacy_hello_cmd,
                                   appname,
                                   ts->uri,
                                   ts->loadbalanced);
    bson_mutex_lock(&ts->handshake_cmd_mtx);
...
static void _begin_hello_cmd(mongoc_topology_scanner_node_t *node,
...
mongoc_topology_scanner_t *mongoc_topology_scanner_new(const mongoc_uri_t *uri,
...
#ifdef MONGOC_ENABLE_SSL
void mongoc_topology_scanner_set_ssl_opts(mongoc_topology_scanner_t *ts, mongoc_ssl_opt_t *opts) {
    ts->ssl_opts = opts;
    ts->setup = mongoc_async_cmd_tls_setup;
}
#endif

void mongoc_topology_scanner_set_stream_initiator(mongoc_topology_scanner_t *ts,
...
void mongoc_topology_scanner_destroy(mongoc_topology_scanner_t *ts) {
...
bool mongoc_topology_scanner_valid(mongoc_topology_scanner_t *ts) {
    return ts->nodes != NULL;
}

void mongoc_topology_scanner_add(mongoc_topology_scanner_t *ts,
                                 const mongoc_host_list_t *host,
                                 uint32_t id,
                                 bool hello_ok) {
...
void mongoc_topology_scanner_scan(mongoc_topology_scanner_t *ts, uint32_t id) {
    mongoc_topology_scanner_node_t *node;
...
void mongoc_topology_scanner_disconnect(mongoc_topology_scanner_t *scanner) {
    mongoc_topology_scanner_node_t *node;
...
void mongoc_topology_scanner_node_retire(mongoc_topology_scanner_node_t *node) {
...
void mongoc_topology_scanner_node_disconnect(mongoc_topology_scanner_node_t *node, bool failed) {
...
void mongoc_topology_scanner_node_destroy(mongoc_topology_scanner_node_t *node, bool failed) {
 ...
 *--------------------------------------------------------------------------
 */
mongoc_topology_scanner_node_t *mongoc_topology_scanner_get_node(mongoc_topology_scanner_t *ts, uint32_t id) {
    mongoc_topology_scanner_node_t *ele, *tmp;
 ...
 *--------------------------------------------------------------------------
 */
bool mongoc_topology_scanner_has_node_for_host(mongoc_topology_scanner_t *ts, mongoc_host_list_t *host) {
    mongoc_topology_scanner_node_t *ele, *tmp;
...
static void _async_handler(mongoc_async_cmd_t *acmd,
    ...
    switch (async_status) {
    case MONGOC_ASYNC_CMD_CONNECTED: _async_connected(acmd); return;
    case MONGOC_ASYNC_CMD_SUCCESS: _async_success(acmd, hello_response, duration_usec); return;
    case MONGOC_ASYNC_CMD_TIMEOUT: _async_error_or_timeout(acmd, duration_usec, "connection timeout"); return;
    case MONGOC_ASYNC_CMD_ERROR: _async_error_or_timeout(acmd, duration_usec, "connection error"); return;
...
mongoc_stream_t *_mongoc_topology_scanner_node_setup_stream_for_tls(mongoc_topology_scanner_node_t *node,
                                                                    mongoc_stream_t *stream) {
#ifdef MONGOC_ENABLE_SSL
...
mongoc_stream_t *_mongoc_topology_scanner_tcp_initiate(mongoc_async_cmd_t *acmd) {
    mongoc_topology_scanner_node_t *node = (mongoc_topology_scanner_node_t *)acmd->data;
...
bool mongoc_topology_scanner_node_setup_tcp(mongoc_topology_scanner_node_t *node, bson_error_t *error) {
...
bool mongoc_topology_scanner_node_connect_unix(mongoc_topology_scanner_node_t *node, bson_error_t *error) {
#ifdef _WIN32
    ENTRY;
    bson_set_error(error,
                   MONGOC_ERROR_STREAM,
...
void mongoc_topology_scanner_node_setup(mongoc_topology_scanner_node_t *node, bson_error_t *error) {
    bool success = false;
    mongoc_stream_t *stream;
    int64_t start;
    ...
    if (!success) {
        _mongoc_topology_scanner_monitor_heartbeat_failed(node->ts,
                                                          &node->host,
                                                          error,
 ...
 *--------------------------------------------------------------------------
 */
bool mongoc_topology_scanner_node_in_cooldown(mongoc_topology_scanner_node_t *node, int64_t when) {
...
bool mongoc_topology_scanner_in_cooldown(mongoc_topology_scanner_t *ts, int64_t when) {
    mongoc_topology_scanner_node_t *node;
...
void mongoc_topology_scanner_start(mongoc_topology_scanner_t *ts, bool obey_cooldown) {
...
void _mongoc_topology_scanner_finish(mongoc_topology_scanner_t *ts) {
    mongoc_topology_scanner_node_t *node, *tmp;
...
void mongoc_topology_scanner_work(mongoc_topology_scanner_t *ts) {
...
void mongoc_topology_scanner_get_error(mongoc_topology_scanner_t *ts, bson_error_t *error) {
    BSON_ASSERT(ts);
    BSON_ASSERT(error);
 ...
 */
bool _mongoc_topology_scanner_set_appname(mongoc_topology_scanner_t *ts, const char *appname) {
    char *s;
    const char *prev;
 ...
 */
void _mongoc_topology_scanner_set_cluster_time(mongoc_topology_scanner_t *ts, const bson_t *cluster_time) {
    bson_destroy(&ts->cluster_time);
...
void _mongoc_topology_scanner_set_dns_cache_timeout(mongoc_topology_scanner_t *ts, int64_t timeout_ms) {
...
void _mongoc_topology_scanner_set_server_api(mongoc_topology_scanner_t *ts, const mongoc_server_api_t *api) {
    BSON_ASSERT(ts);
    BSON_ASSERT(api);
    mongoc_server_api_destroy(ts->api);
...
void _mongoc_topology_scanner_set_loadbalanced(mongoc_topology_scanner_t *ts, bool val) {
    BSON_UNUSED(val);
...
bool mongoc_topology_scanner_uses_server_api(const mongoc_topology_scanner_t *topology_scanner) {
