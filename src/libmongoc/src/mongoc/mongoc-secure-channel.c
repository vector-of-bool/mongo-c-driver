...
void _bson_append_szoid(bson_string_t *retval, PCCERT_CONTEXT cert, const char *label, void *oid) {
...
char *_mongoc_secure_channel_extract_subject(const char *filename, const char *passphrase) {
...
bool mongoc_secure_channel_setup_ca(mongoc_stream_tls_secure_channel_t *secure_channel, mongoc_ssl_opt_t *opt) {
    FILE *file;
    long length;
    const char *pem_key;
...
bool mongoc_secure_channel_setup_crl(mongoc_stream_tls_secure_channel_t *secure_channel, mongoc_ssl_opt_t *opt) {
    HCERTSTORE cert_store = NULL;
    PCCERT_CONTEXT cert = NULL;
    LPWSTR str;
    int chars;
...
ssize_t mongoc_secure_channel_read(mongoc_stream_tls_t *tls, void *data, size_t data_length) {
    BSON_ASSERT_PARAM(tls);

    if (BSON_UNLIKELY(!bson_in_range_signed(int32_t, tls->timeout_msec))) {
        // CDRIVER-4589
        MONGOC_ERROR("timeout_msec value %" PRIu64 " exceeds supported 32-bit range", tls->timeout_msec);
        return -1;
    }
...
ssize_t mongoc_secure_channel_write(mongoc_stream_tls_t *tls, const void *data, size_t data_length) {
    BSON_ASSERT_PARAM(tls);
...
void mongoc_secure_channel_realloc_buf(size_t *size, uint8_t **buf, size_t new_size) {
...
void _mongoc_secure_channel_init_sec_buffer(SecBuffer *buffer,
...
void _mongoc_secure_channel_init_sec_buffer_desc(SecBufferDesc *desc,
...
bool mongoc_secure_channel_handshake_step_1(mongoc_stream_tls_t *tls, char *hostname, bson_error_t *error) {
    SecBuffer outbuf;
    ssize_t written = -1;
...
bool mongoc_secure_channel_handshake_step_2(mongoc_stream_tls_t *tls, char *hostname, bson_error_t *error) {
    mongoc_stream_tls_secure_channel_t *secure_channel = (mongoc_stream_tls_secure_channel_t *)tls->ctx;
    SECURITY_STATUS sspi_status = SEC_E_OK;
...
bool mongoc_secure_channel_handshake_step_3(mongoc_stream_tls_t *tls, char *hostname, bson_error_t *error) {
    mongoc_stream_tls_secure_channel_t *secure_channel = (mongoc_stream_tls_secure_channel_t *)tls->ctx;
