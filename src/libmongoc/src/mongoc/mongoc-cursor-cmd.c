...
static getmore_type_t _getmore_type(mongoc_cursor_t *cursor) {
...
static mongoc_cursor_state_t _pop_from_batch(mongoc_cursor_t *cursor) {
    ...
    case CMD_RESPONSE: _mongoc_cursor_response_read(cursor, &data->response, &cursor->current); break;
    case OP_GETMORE_RESPONSE: cursor->current = bson_reader_read(data->response_legacy.reader, NULL); break;
    case NONE:
    default: fprintf(stderr, "trying to pop from an uninitialized cursor reader.\n"); BSON_ASSERT(false);
    }
    if (cursor->current) {
        return IN_BATCH;
    } else {
...
static mongoc_cursor_state_t _get_next_batch(mongoc_cursor_t *cursor) {
    data_cmd_t *data = (data_cmd_t *)cursor->impl.data;
    bson_t getmore_cmd;
