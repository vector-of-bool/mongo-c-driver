...
static void _server_monitor_heartbeat_started(mongoc_server_monitor_t *server_monitor, bool awaited) {
...
static void _server_monitor_heartbeat_succeeded(mongoc_server_monitor_t *server_monitor,
                                                const bson_t *reply,
...
static void _server_monitor_heartbeat_failed(mongoc_server_monitor_t *server_monitor,
...
static void _server_monitor_append_cluster_time(mongoc_server_monitor_t *server_monitor, bson_t *cmd) {
...
static bool _server_monitor_send_and_recv_hello_opmsg(mongoc_server_monitor_t *server_monitor,
                                                      bson_t *cmd,
                                                      bson_t *reply,
                                                      bson_error_t *error) {
...
static bool _server_monitor_send_and_recv_opquery(mongoc_server_monitor_t *server_monitor,
                                                  const bson_t *cmd,
                                                  bson_t *reply,
                                                  bson_error_t *error) {
    mongoc_rpc_t rpc = {._init = 0};
...
static bool _server_monitor_polling_hello(mongoc_server_monitor_t *server_monitor,
                                          bool hello_ok,
 ...
 */
static bool _server_monitor_poll_with_interrupt(mongoc_server_monitor_t *server_monitor,
 ...
 */
int64_t _get_timeout_ms(int64_t expire_at_ms, bson_error_t *error) {
    int64_t timeout_ms;
 ...
 * On cancellation, no error is set, but cancelled is set to true.
 */
static bool _server_monitor_awaitable_hello_recv(mongoc_server_monitor_t *server_monitor,
                                                 bson_t *hello_response,
                                                 bool *cancelled,
                                                 bson_error_t *error) {
    bool ret = false;
 ...
 */
static bool _server_monitor_awaitable_hello(mongoc_server_monitor_t *server_monitor,
 ...
 * Locks server monitor mutex.
 */
static void _update_topology_description(mongoc_server_monitor_t *server_monitor,
                                         mongoc_server_description_t *description) {
    mongoc_topology_t *topology;
 ...
 */
static bool _server_monitor_setup_connection(mongoc_server_monitor_t *server_monitor,
                                             bson_t *hello_response,
                                             int64_t *start_us,
                                             bson_error_t *error) {
    ...
    if (server_monitor->initiator) {
        server_monitor->stream = server_monitor->initiator(server_monitor->uri,
...
_server_monitor_check_server(mongoc_server_monitor_t *server_monitor,
    ...
    *cancelled = false;
    ...
    mongoc_server_description_init(description,
...
exit:
    ...
    if (command_or_network_error) {
         ...
         * removed. */
        _mongoc_topology_description_clear_connection_pool(tdmod.new_td,
                                                           server_monitor->description->id,
 ...
 */
void mongoc_server_monitor_request_scan(mongoc_server_monitor_t *server_monitor) {
 ...
 * Locks server monitor mutex.
 */
void mongoc_server_monitor_request_cancel(mongoc_server_monitor_t *server_monitor) {
 ...
 * Locks server monitor mutex.
 */
void mongoc_server_monitor_wait(mongoc_server_monitor_t *server_monitor) {
    int64_t start_ms;
    int64_t scan_due_ms;
    ...
    while (true) {
        ...
        cond_ret =
            mongoc_cond_timedwait(&server_monitor->shared.cond, &server_monitor->shared.mutex, sleep_duration_ms);
...
static bool _server_monitor_ping_server(mongoc_server_monitor_t *server_monitor, bool hello_ok, int64_t *rtt_ms) {
    bool ret = false;
...
void mongoc_server_monitor_run(mongoc_server_monitor_t *server_monitor) {
    bson_mutex_lock(&server_monitor->shared.mutex);
...
void mongoc_server_monitor_run_as_rtt(mongoc_server_monitor_t *server_monitor) {
    bson_mutex_lock(&server_monitor->shared.mutex);
    if (server_monitor->shared.state == MONGOC_THREAD_OFF) {
 ...
 * Locks server monitor mutex.
 */
bool mongoc_server_monitor_request_shutdown(mongoc_server_monitor_t *server_monitor) {
    bool off = false;
 ...
 */
void mongoc_server_monitor_wait_for_shutdown(mongoc_server_monitor_t *server_monitor) {
 ...
 */
void mongoc_server_monitor_destroy(mongoc_server_monitor_t *server_monitor) {
    if (!server_monitor) {
        return;
    }
