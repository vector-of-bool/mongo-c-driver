...
const char *mongoc_server_api_version_to_string(mongoc_server_api_version_t version) {
    switch (version) {
    case MONGOC_SERVER_API_V1: return "1";
    default: return NULL;
    }
}

bool mongoc_server_api_version_from_string(const char *version, mongoc_server_api_version_t *out) {
...
mongoc_server_api_t *mongoc_server_api_new(mongoc_server_api_version_t version) {
...
mongoc_server_api_t *mongoc_server_api_copy(const mongoc_server_api_t *api) {
...
void mongoc_server_api_destroy(mongoc_server_api_t *api) {
    if (!api) {
        return;
    }
...
void mongoc_server_api_strict(mongoc_server_api_t *api, bool strict) {
    BSON_ASSERT(api);
    mongoc_optional_set_value(&api->strict, strict);
}

void mongoc_server_api_deprecation_errors(mongoc_server_api_t *api, bool deprecation_errors) {
    BSON_ASSERT(api);
    mongoc_optional_set_value(&api->deprecation_errors, deprecation_errors);
}

const mongoc_optional_t *mongoc_server_api_get_deprecation_errors(const mongoc_server_api_t *api) {
    BSON_ASSERT(api);
    return &api->deprecation_errors;
}

const mongoc_optional_t *mongoc_server_api_get_strict(const mongoc_server_api_t *api) {
    BSON_ASSERT(api);
    return &api->strict;
}
