...
static BSON_INLINE int64_t get_expiration(int32_t timeout_msec) {
    if (timeout_msec < 0) {
        return -1;
...
static int _mongoc_stream_socket_close(mongoc_stream_t *stream) {
...
static void _mongoc_stream_socket_destroy(mongoc_stream_t *stream) {
    mongoc_stream_socket_t *ss = (mongoc_stream_socket_t *)stream;
...
static void _mongoc_stream_socket_failed(mongoc_stream_t *stream) {
    ENTRY;
...
static int _mongoc_stream_socket_setsockopt(mongoc_stream_t *stream,
                                            int level,
                                            int optname,
                                            void *optval,
...
static int _mongoc_stream_socket_flush(mongoc_stream_t *stream) {
    ENTRY;
    BSON_UNUSED(stream);
    RETURN(0);
}

static ssize_t _mongoc_stream_socket_readv(mongoc_stream_t *stream,
                                           mongoc_iovec_t *iov,
                                           size_t iovcnt,
                                           size_t min_bytes,
...
static ssize_t _mongoc_stream_socket_poll(mongoc_stream_poll_t *streams, size_t nstreams, int32_t timeout_msec)
...
mongoc_socket_t *mongoc_stream_socket_get_socket(mongoc_stream_socket_t *stream) /* IN */
{
    BSON_ASSERT(stream);
...
static bool _mongoc_stream_socket_check_closed(mongoc_stream_t *stream) /* IN */
{
    mongoc_stream_socket_t *ss = (mongoc_stream_socket_t *)stream;
...
static bool _mongoc_stream_socket_timed_out(mongoc_stream_t *stream) /* IN */
{
    mongoc_stream_socket_t *ss = (mongoc_stream_socket_t *)stream;
...
static bool _mongoc_stream_socket_should_retry(mongoc_stream_t *stream) /* IN */
{
    mongoc_stream_socket_t *ss = (mongoc_stream_socket_t *)stream;
...
mongoc_stream_t *mongoc_stream_socket_new(mongoc_socket_t *sock) /* IN */
{
