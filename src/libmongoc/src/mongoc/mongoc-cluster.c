...
static mongoc_server_stream_t *_cluster_fetch_stream_single(mongoc_cluster_t *cluster,
                                                            const mongoc_topology_description_t *td,
                                                            uint32_t server_id,
                                                            bool reconnect_ok,
                                                            bson_error_t *error);

static mongoc_server_stream_t *_cluster_fetch_stream_pooled(mongoc_cluster_t *cluster,
                                                            const mongoc_topology_description_t *td,
                                                            uint32_t server_id,
                                                            bool reconnect_ok,
                                                            bson_error_t *error);

static bool mongoc_cluster_run_opmsg(mongoc_cluster_t *cluster, mongoc_cmd_t *cmd, bson_t *reply, bson_error_t *error);

static void _bson_error_message_printf(bson_error_t *error, const char *format, ...) BSON_GNUC_PRINTF(2, 3);
...
size_t _mongoc_cluster_buffer_iovec(mongoc_iovec_t *iov, size_t iovcnt, int skip, char *buffer) {
...
static void _bson_error_message_printf(bson_error_t *error, const char *format, ...) {
    va_list args;
...
static bool mongoc_cluster_run_command_opquery(mongoc_cluster_t *cluster,
                                               mongoc_cmd_t *cmd,
...
bool _in_sharded_txn(const mongoc_client_session_t *session) {
...
bool mongoc_cluster_run_command_monitored(mongoc_cluster_t *cluster,
                                          mongoc_cmd_t *cmd,
                                          bson_t *reply,
                                          bson_error_t *error) {
    bool retval;
...
bool mongoc_cluster_run_command_private(mongoc_cluster_t *cluster,
                                        mongoc_cmd_t *cmd,
                                        bson_t *reply,
                                        bson_error_t *error) {
    bool retval;
...
bool mongoc_cluster_run_command_parts(mongoc_cluster_t *cluster,
 ...
 *--------------------------------------------------------------------------
 */
static mongoc_server_description_t *_stream_run_hello(mongoc_cluster_t *cluster,
                                                      mongoc_stream_t *stream,
                                                      const char *address,
                                                      uint32_t server_id,
 ...
 *--------------------------------------------------------------------------
 */
static mongoc_server_description_t *_cluster_run_hello(mongoc_cluster_t *cluster,
...
static char *_mongoc_cluster_build_basic_auth_digest(mongoc_cluster_t *cluster, const char *nonce) {
    const char *username;
    const char *password;
...
static bool _mongoc_cluster_auth_node_cr(mongoc_cluster_t *cluster,
                                         mongoc_stream_t *stream,
                                         mongoc_server_description_t *sd,
                                         bson_error_t *error) {
    mongoc_cmd_parts_t parts;
...
static bool _mongoc_cluster_auth_node_plain(mongoc_cluster_t *cluster,
                                            mongoc_stream_t *stream,
                                            mongoc_server_description_t *sd,
                                            bson_error_t *error) {
    mongoc_cmd_parts_t parts;
...
bool _mongoc_cluster_get_auth_cmd_x509(const mongoc_uri_t *uri,
...
static bool _mongoc_cluster_auth_node_x509(mongoc_cluster_t *cluster,
                                           mongoc_stream_t *stream,
                                           mongoc_server_description_t *sd,
                                           bson_error_t *error) {
#ifndef MONGOC_ENABLE_SSL
...
bool mongoc_cluster_uses_server_api(const mongoc_cluster_t *cluster) {
    return mongoc_client_uses_server_api(cluster->client);
}

#ifdef MONGOC_ENABLE_CRYPTO
void _mongoc_cluster_init_scram(const mongoc_cluster_t *cluster,
                                mongoc_scram_t *scram,
...
bool _mongoc_cluster_get_auth_cmd_scram(mongoc_crypto_hash_algorithm_t algo,
                                        mongoc_scram_t *scram,
                                        bson_t *cmd /* OUT */,
                                        bson_error_t *error /* OUT */) {
    uint8_t buf[4096] = {0};
...
static bool _mongoc_cluster_run_scram_command(mongoc_cluster_t *cluster,
                                              mongoc_stream_t *stream,
...
static bool _mongoc_cluster_auth_scram_start(mongoc_cluster_t *cluster,
                                             mongoc_stream_t *stream,
                                             const mongoc_server_description_t *handshake_sd,
                                             mongoc_crypto_hash_algorithm_t algo,
                                             mongoc_scram_t *scram,
...
static bool _mongoc_cluster_scram_handle_reply(mongoc_scram_t *scram,
                                               const bson_t *reply,
                                               bool *done /* OUT */,
                                               int *conv_id /* OUT */,
                                               uint8_t *buf /* OUT */,
...
static bool _mongoc_cluster_auth_scram_continue(mongoc_cluster_t *cluster,
                                                mongoc_stream_t *stream,
                                                const mongoc_server_description_t *handshake_sd,
                                                mongoc_scram_t *scram,
...
static bool _mongoc_cluster_auth_node_scram(mongoc_cluster_t *cluster,
                                            mongoc_stream_t *stream,
...
static bool _mongoc_cluster_auth_node_scram_sha_1(mongoc_cluster_t *cluster,
                                                  mongoc_stream_t *stream,
                                                  mongoc_server_description_t *sd,
                                                  bson_error_t *error) {
...
static bool _mongoc_cluster_auth_node_scram_sha_256(mongoc_cluster_t *cluster,
                                                    mongoc_stream_t *stream,
                                                    mongoc_server_description_t *sd,
                                                    bson_error_t *error) {
#ifndef MONGOC_ENABLE_CRYPTO
...
static bool _mongoc_cluster_auth_node(mongoc_cluster_t *cluster,
                                      mongoc_stream_t *stream,
                                      mongoc_server_description_t *sd,
...
void mongoc_cluster_disconnect_node(mongoc_cluster_t *cluster, uint32_t server_id) {
...
static void _mongoc_cluster_node_destroy(mongoc_cluster_node_t *node) {
...
static void _mongoc_cluster_node_dtor(void *data_, void *ctx_) {
...
static mongoc_cluster_node_t *_mongoc_cluster_node_new(mongoc_stream_t *stream, const char *connection_address) {
    mongoc_cluster_node_t *node;
...
static bool _mongoc_cluster_finish_speculative_auth(mongoc_cluster_t *cluster,
                                                    mongoc_stream_t *stream,
                                                    mongoc_server_description_t *handshake_sd,
 ...
 *--------------------------------------------------------------------------
 */
static mongoc_cluster_node_t *_cluster_add_node(mongoc_cluster_t *cluster,
                                                const mongoc_topology_description_t *td,
                                                uint32_t server_id,
                                                bson_error_t *error /* OUT */) {
    ...
    if (cluster->requires_auth) {
        ...
        bool is_auth = _mongoc_cluster_finish_speculative_auth(cluster,
                                                               stream,
                                                               handshake_sd,
                                                               &speculative_auth_response,
                                                               &scram,
...
static void node_not_found(const mongoc_topology_description_t *td, uint32_t server_id, bson_error_t *error /* OUT */) {
...
static void stream_not_found(const mongoc_topology_description_t *td,
                             uint32_t server_id,
...
static mongoc_server_stream_t *_try_get_server_stream(mongoc_cluster_t *cluster,
                                                      const mongoc_topology_description_t *td,
                                                      uint32_t server_id,
                                                      bool reconnect_ok,
                                                      bson_error_t *error) {
...
static mongoc_server_stream_t *_mongoc_cluster_stream_for_server(mongoc_cluster_t *cluster,
                                                                 uint32_t server_id,
                                                                 bool reconnect_ok,
...
mongoc_server_stream_t *mongoc_cluster_stream_for_server(mongoc_cluster_t *cluster,
                                                         uint32_t server_id,
                                                         bool reconnect_ok,
...
bool mongoc_cluster_stream_valid(mongoc_cluster_t *cluster, mongoc_server_stream_t *server_stream) {
    ...
    if (!sd
        || server_stream->sd->generation
               < _mongoc_topology_get_connection_pool_generation(td.ptr,
                                                                 server_stream->sd->id,
                                                                 &server_stream->sd->service_id)) {
...
mongoc_server_stream_t *_mongoc_cluster_create_server_stream(const mongoc_topology_description_t *td,
                                                             const mongoc_server_description_t *handshake_sd,
                                                             mongoc_stream_t *stream) {
...
void mongoc_cluster_init(mongoc_cluster_t *cluster, const mongoc_uri_t *uri, void *client) {
    ENTRY;
...
void mongoc_cluster_destroy(mongoc_cluster_t *cluster) /* INOUT */
{
    ENTRY;
...
static uint32_t _mongoc_cluster_select_server_id(mongoc_client_session_t *cs,
                                                 mongoc_topology_t *topology,
...
static mongoc_server_stream_t *_mongoc_cluster_stream_for_optype(mongoc_cluster_t *cluster,
                                                                 mongoc_ss_optype_t optype,
                                                                 const mongoc_read_prefs_t *read_prefs,
                                                                 mongoc_client_session_t *cs,
                                                                 bool is_retryable,
...
mongoc_server_stream_t *mongoc_cluster_stream_for_reads(mongoc_cluster_t *cluster,
                                                        const mongoc_read_prefs_t *read_prefs,
                                                        mongoc_client_session_t *cs,
                                                        bson_t *reply,
                                                        bson_error_t *error) {
    ...
    return _mongoc_cluster_stream_for_optype(cluster, MONGOC_SS_READ, prefs_override, cs, is_retryable, reply, error);
}

mongoc_server_stream_t *mongoc_cluster_stream_for_writes(mongoc_cluster_t *cluster,
                                                         mongoc_client_session_t *cs,
                                                         bson_t *reply,
                                                         bson_error_t *error) {
    const bool is_retryable =
    ...
    return _mongoc_cluster_stream_for_optype(cluster, MONGOC_SS_WRITE, NULL, cs, is_retryable, reply, error);
}

mongoc_server_stream_t *mongoc_cluster_stream_for_aggr_with_write(mongoc_cluster_t *cluster,
                                                                  const mongoc_read_prefs_t *read_prefs,
                                                                  mongoc_client_session_t *cs,
                                                                  bson_t *reply,
                                                                  bson_error_t *error) {
...
static bool _mongoc_cluster_min_of_max_obj_size_sds(const void *item, void *ctx) {
...
static bool _mongoc_cluster_min_of_max_obj_size_nodes(void *item, void *ctx) {
...
static bool _mongoc_cluster_min_of_max_msg_size_sds(const void *item, void *ctx) {
    const mongoc_server_description_t *sd = item;
    int32_t *current_min = (int32_t *)ctx;
...
static bool _mongoc_cluster_min_of_max_msg_size_nodes(void *item, void *ctx) {
    mongoc_cluster_node_t *node = (mongoc_cluster_node_t *)item;
    int32_t *current_min = (int32_t *)ctx;
 ...
 *--------------------------------------------------------------------------
 */
int32_t mongoc_cluster_get_max_bson_obj_size(mongoc_cluster_t *cluster) {
 ...
 *--------------------------------------------------------------------------
 */
int32_t mongoc_cluster_get_max_msg_size(mongoc_cluster_t *cluster) {
...
bool mongoc_cluster_check_interval(mongoc_cluster_t *cluster, uint32_t server_id) {
    mongoc_cmd_parts_t parts;
    mongoc_topology_t *topology;
    mongoc_topology_scanner_node_t *scanner_node;
    mongoc_stream_t *stream;
...
bool mongoc_cluster_legacy_rpc_sendv_to_server(mongoc_cluster_t *cluster,
                                               mongoc_rpc_t *rpc,
                                               mongoc_server_stream_t *server_stream,
                                               bson_error_t *error) {
    uint32_t server_id;
    ...
    if (bson_cmp_greater_us(BSON_UINT32_FROM_LE(rpc->header.msg_len), max_msg_size)) {
        bson_set_error(error,
                       MONGOC_ERROR_CLIENT,
                       MONGOC_ERROR_CLIENT_TOO_BIG,
                       "Attempted to send an RPC larger than the "
                       "max allowed message size. Was %" PRIu32 ", allowed %" PRId32 ".",
...
bool mongoc_cluster_try_recv(mongoc_cluster_t *cluster,
                             mongoc_rpc_t *rpc,
...
static void network_error_reply(bson_t *reply, mongoc_cmd_t *cmd) {
    bson_t labels;
...
static bool mongoc_cluster_run_opmsg(mongoc_cluster_t *cluster, mongoc_cmd_t *cmd, bson_t *reply, bson_error_t *error) {
    ...
    if (cmd->payload) {
        ...
        section[1].payload.sequence.size = cmd->payload_size + (int32_t)identifier_len + 1 + (int32_t)sizeof(int32_t);
