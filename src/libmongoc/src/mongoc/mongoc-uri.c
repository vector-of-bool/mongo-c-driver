...
static bool _mongoc_uri_set_option_as_int32(mongoc_uri_t *uri, const char *option, int32_t value);
...
static void mongoc_uri_do_unescape(char **str) {
    char *tmp;
...
static char *lowercase_str_new(const char *key) {
...
static bool _upsert_into_host_list(mongoc_uri_t *uri, mongoc_host_list_t *host, bson_error_t *error) {
...
bool mongoc_uri_upsert_host_and_port(mongoc_uri_t *uri, const char *host_and_port, bson_error_t *error) {
...
bool mongoc_uri_upsert_host(mongoc_uri_t *uri, const char *host, uint16_t port, bson_error_t *error) {
    mongoc_host_list_t temp;
...
void mongoc_uri_remove_host(mongoc_uri_t *uri, const char *host, uint16_t port) {
...
static char *scan_to_unichar(const char *str, bson_unichar_t match, const char *terminators, const char **end) {
    bson_unichar_t c;
    const char *iter;
...
static bool mongoc_uri_parse_scheme(mongoc_uri_t *uri, const char *str, const char **end) {
...
static bool mongoc_uri_has_unescaped_chars(const char *str, const char *chars) {
    const char *c;
    const char *tmp;
    char *s;
...
static bool mongoc_uri_parse_userpass(mongoc_uri_t *uri, const char *str, bson_error_t *error) {
...
bool mongoc_uri_parse_host(mongoc_uri_t *uri, const char *host_and_port_in) {
...
static bool mongoc_uri_parse_srv(mongoc_uri_t *uri, const char *str, bson_error_t *error) {
    if (*str == '\0') {
 ...
 * -----------------------------------------------------------------------------
 */
static bool mongoc_uri_parse_database(mongoc_uri_t *uri, const char *str, const char **end) {
...
static bool mongoc_uri_parse_auth_mechanism_properties(mongoc_uri_t *uri, const char *str) {
    char *field;
    char *value;
    const char *end_scan;
    bson_t properties;
...
static bool mongoc_uri_check_srv_service_name(mongoc_uri_t *uri, const char *str) {
...
static bool mongoc_uri_parse_tags(mongoc_uri_t *uri, /* IN */
                                  const char *str)   /* IN */
{
...
static void mongoc_uri_bson_append_or_replace_key(bson_t *options, const char *option, const char *value) {
    bson_iter_t iter;
    bool found = false;
...
bool mongoc_uri_has_option(const mongoc_uri_t *uri, const char *key) {
    bson_iter_t iter;
...
bool mongoc_uri_option_is_int64(const char *key) {
...
const char *mongoc_uri_canonicalize_option(const char *key) {
...
static bool _mongoc_uri_parse_int64(const char *key, const char *value, int64_t *result) {
    char *endptr;
    int64_t i;
...
static bool mongoc_uri_parse_int32(const char *key, const char *value, int32_t *result) {
    int64_t i;
...
static bool dns_option_allowed(const char *lkey) {
 ...
 */
static bool mongoc_uri_options_validate_names(const bson_t *a, const bson_t *b, bson_error_t *error) {
...
static bool mongoc_uri_apply_options(mongoc_uri_t *uri, const bson_t *options, bool from_dns, bson_error_t *error) {
    bson_iter_t iter;
    int32_t v_int;
    int64_t v_int64;
    const char *key = NULL;
 ...
 */
bool mongoc_uri_parse_options(mongoc_uri_t *uri, const char *str, bool from_dns, bson_error_t *error) {
    bson_t options;
...
static bool mongoc_uri_finalize_tls(mongoc_uri_t *uri, bson_error_t *error) {
...
static bool mongoc_uri_finalize_auth(mongoc_uri_t *uri, bson_error_t *error) {
    bson_iter_t iter;
...
static bool mongoc_uri_finalize_directconnection(mongoc_uri_t *uri, bson_error_t *error) {
...
static bool mongoc_uri_parse_before_slash(mongoc_uri_t *uri, const char *before_slash, bson_error_t *error) {
    char *userpass;
    const char *hosts;
...
static bool mongoc_uri_parse(mongoc_uri_t *uri, const char *str, bson_error_t *error) {
    BSON_ASSERT_PARAM(str);
...
bool mongoc_uri_get_mechanism_properties(const mongoc_uri_t *uri, bson_t *properties /* OUT */) {
    bson_iter_t iter;
...
bool mongoc_uri_set_mechanism_properties(mongoc_uri_t *uri, const bson_t *properties) {
    BSON_ASSERT(uri);
    BSON_ASSERT(properties);
...
static bool _mongoc_uri_assign_read_prefs_mode(mongoc_uri_t *uri, bson_error_t *error) {
    BSON_ASSERT(uri);
...
static bool _mongoc_uri_build_write_concern(mongoc_uri_t *uri, bson_error_t *error) {
    mongoc_write_concern_t *write_concern;
    int64_t wtimeoutms;
    ...
    bsonParse(
        ...
        find(iKeyWithType(MONGOC_URI_SAFE, bool),
             do(mongoc_write_concern_set_w(write_concern, bsonAs(bool) ? 1 : MONGOC_WRITE_CONCERN_W_UNACKNOWLEDGED))));
...
static int32_t _mongoc_uri_get_max_staleness_option(const mongoc_uri_t *uri) {
...
/* can't use mongoc_uri_get_option_as_int32, it treats 0 specially */
int32_t mongoc_uri_get_local_threshold_option(const mongoc_uri_t *uri) {
    const bson_t *options;
    bson_iter_t iter;
...
const char *mongoc_uri_get_srv_hostname(const mongoc_uri_t *uri) {
    if (uri->is_srv) {
        return uri->srv;
    }
...
const char *mongoc_uri_get_service(const mongoc_uri_t *uri) {
...
const char *mongoc_uri_get_srv_service_name(const mongoc_uri_t *uri) {
    bson_iter_t iter;
...
char *mongoc_uri_unescape(const char *escaped_string) {
    ...
    for (; *ptr; ptr = bson_utf8_next_char(ptr)) {
        ...
        case '%':
            ...
            break;
        default: bson_string_append_unichar(str, c); break;
        }
    }
...
const mongoc_read_prefs_t *mongoc_uri_get_read_prefs_t(const mongoc_uri_t *uri) /* IN */
{
    BSON_ASSERT(uri);
...
void mongoc_uri_set_read_prefs_t(mongoc_uri_t *uri, const mongoc_read_prefs_t *prefs) {
    BSON_ASSERT(uri);
    BSON_ASSERT(prefs);
...
const mongoc_read_concern_t *mongoc_uri_get_read_concern(const mongoc_uri_t *uri) /* IN */
{
    BSON_ASSERT(uri);
...
void mongoc_uri_set_read_concern(mongoc_uri_t *uri, const mongoc_read_concern_t *rc) {
    BSON_ASSERT(uri);
    BSON_ASSERT(rc);
...
const mongoc_write_concern_t *mongoc_uri_get_write_concern(const mongoc_uri_t *uri) /* IN */
{
    BSON_ASSERT(uri);
...
void mongoc_uri_set_write_concern(mongoc_uri_t *uri, const mongoc_write_concern_t *wc) {
    BSON_ASSERT(uri);
    BSON_ASSERT(wc);
...
bool mongoc_uri_get_tls(const mongoc_uri_t *uri) /* IN */
{
    bson_iter_t iter;
...
bool mongoc_uri_get_ssl(const mongoc_uri_t *uri) /* IN */
{
...
int32_t mongoc_uri_get_option_as_int32(const mongoc_uri_t *uri, const char *option_orig, int32_t fallback) {
    const char *option;
    const bson_t *options;
    bson_iter_t iter;
    int64_t retval = 0;
...
bool mongoc_uri_set_option_as_int32(mongoc_uri_t *uri, const char *option_orig, int32_t value) {
    const char *option;
    bson_error_t error;
    bool r;
...
static bool _mongoc_uri_set_option_as_int32_with_error(mongoc_uri_t *uri,
...
static bool _mongoc_uri_set_option_as_int32(mongoc_uri_t *uri, const char *option_orig, int32_t value) {
    const char *option;
    const bson_t *options;
    bson_iter_t iter;
    char *option_lowercase = NULL;
...
int64_t mongoc_uri_get_option_as_int64(const mongoc_uri_t *uri, const char *option_orig, int64_t fallback) {
    const char *option;
    const bson_t *options;
    bson_iter_t iter;
...
bool mongoc_uri_set_option_as_int64(mongoc_uri_t *uri, const char *option_orig, int64_t value) {
    const char *option;
    bson_error_t error;
    bool r;
    ...
    if (!mongoc_uri_option_is_int64(option)) {
        if (mongoc_uri_option_is_int32(option_orig)) {
            ...
            MONGOC_WARNING("Unsupported value for \"%s\": %" PRId64 ", \"%s\" is not an int64 option",
                           option_orig,
                           value,
                           option);
            return false;
...
static bool _mongoc_uri_set_option_as_int64_with_error(mongoc_uri_t *uri,
                                                       const char *option_orig,
                                                       int64_t value,
                                                       bson_error_t *error) {
    const char *option;
...
bool mongoc_uri_get_option_as_bool(const mongoc_uri_t *uri, const char *option_orig, bool fallback) {
    const char *option;
    const bson_t *options;
    bson_iter_t iter;
...
bool mongoc_uri_set_option_as_bool(mongoc_uri_t *uri, const char *option_orig, bool value) {
    const char *option;
...
const char *mongoc_uri_get_option_as_utf8(const mongoc_uri_t *uri, const char *option_orig, const char *fallback) {
    const char *option;
    const bson_t *options;
    bson_iter_t iter;
...
bool mongoc_uri_set_option_as_utf8(mongoc_uri_t *uri, const char *option_orig, const char *value) {
    const char *option;
    size_t len;
    char *option_lowercase = NULL;
 ...
 *--------------------------------------------------------------------------
 */
bool _mongoc_uri_requires_auth_negotiation(const mongoc_uri_t *uri) {
...
mongoc_uri_t *_mongoc_uri_copy_and_replace_host_list(const mongoc_uri_t *original, const char *host) {
...
bool mongoc_uri_init_with_srv_host_list(mongoc_uri_t *uri, mongoc_host_list_t *host_list, bson_error_t *error) {
...
void _mongoc_uri_init_scram(const mongoc_uri_t *uri, mongoc_scram_t *scram, mongoc_crypto_hash_algorithm_t algo) {
    BSON_ASSERT(uri);
    BSON_ASSERT(scram);
...
static bool mongoc_uri_finalize_loadbalanced(const mongoc_uri_t *uri, bson_error_t *error) {
...
static bool mongoc_uri_finalize_srv(const mongoc_uri_t *uri, bson_error_t *error) {
