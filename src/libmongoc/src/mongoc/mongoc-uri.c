                                     ...
                                     const bson_t *properties)
{
   ...
   uri->credentials = new_credentials;
   ...
   bsonParse (uri->options,
              find ( //
                 ...
                 cond ( //
                    ...
                    (true, do({
                        pref = bson_iter_utf8 (&bsonVisitIter, NULL);
                        bsonParseError = "Unsupported read preference";
                     })))));
         ...
         cond (
            ...
            (anyOf (eq (int32, MONGOC_WRITE_CONCERN_W_ERRORS_IGNORED),
             ...
             if (eval (mongoc_write_concern_get_journal (write_concern)),
                 then (error ("Journal option conflicts with special 'w' "
                              "integer value"))),
            ...
            // Other string:
            (type (utf8),
             do(mongoc_write_concern_set_wtag (write_concern, bsonAs (str)))),
            // Invalid value:
            (true, error ("Unsupported 'w' value (Must be UTF-8 or Int32)")))));

   if (bsonParseError) {
      MONGOC_URI_ERROR (
         error, "Error while parsing the 'w' URI option: %s", bsonParseError);
      return false;
   }
   return true;
