...
size_t mongoc_compressor_max_compressed_length(int32_t compressor_id, size_t len) {
    TRACE("Getting compression length for '%s' (%d)", mongoc_compressor_id_to_name(compressor_id), compressor_id);
    switch (compressor_id) {
#ifdef MONGOC_ENABLE_COMPRESSION_SNAPPY
    case MONGOC_COMPRESSOR_SNAPPY_ID: return snappy_max_compressed_length(len); break;
#endif
...
#ifdef MONGOC_ENABLE_COMPRESSION_ZSTD
    case MONGOC_COMPRESSOR_ZSTD_ID: return ZSTD_compressBound(len); break;
#endif
    case MONGOC_COMPRESSOR_NOOP_ID: return len; break;
    default: return 0;
    }
}
...
const char *mongoc_compressor_id_to_name(int32_t compressor_id) {
    switch (compressor_id) {
    case MONGOC_COMPRESSOR_SNAPPY_ID: return MONGOC_COMPRESSOR_SNAPPY_STR;

    case MONGOC_COMPRESSOR_ZLIB_ID: return MONGOC_COMPRESSOR_ZLIB_STR;

    case MONGOC_COMPRESSOR_ZSTD_ID: return MONGOC_COMPRESSOR_ZSTD_STR;

    case MONGOC_COMPRESSOR_NOOP_ID: return MONGOC_COMPRESSOR_NOOP_STR;
...
bool mongoc_uncompress(int32_t compressor_id,
    ...
    default: MONGOC_WARNING("Unknown compressor ID %d", compressor_id);
    }
