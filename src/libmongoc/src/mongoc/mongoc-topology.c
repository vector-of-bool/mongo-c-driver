...
static void _topology_collect_errors(const mongoc_topology_description_t *topology, bson_error_t *error_out);

static bool _mongoc_topology_reconcile_add_nodes(mongoc_server_description_t *sd, mongoc_topology_scanner_t *scanner) {
 ...
 */
void mongoc_topology_reconcile(const mongoc_topology_t *topology, mongoc_topology_description_t *td) {
...
static bool _mongoc_topology_update_no_lock(uint32_t id,
...
void _mongoc_topology_scanner_setup_err_cb(uint32_t id, void *data, const bson_error_t *error /* IN */) {
...
void _mongoc_topology_scanner_cb(uint32_t id,
                                 const bson_t *hello_response,
                                 int64_t rtt_msec,
                                 void *data,
                                 const bson_error_t *error /* IN */) {
...
static void _server_session_init(mongoc_server_session_t *session, mongoc_topology_t *unused, bson_error_t *error) {
    BSON_UNUSED(unused);

    _mongoc_server_session_init(session, error);
}

static void _server_session_destroy(mongoc_server_session_t *session, mongoc_topology_t *unused) {
    BSON_UNUSED(unused);

    _mongoc_server_session_destroy(session);
}

static int _server_session_should_prune(mongoc_server_session_t *session, mongoc_topology_t *topo) {
...
static void _tpld_destroy_and_free(void *tpl_descr) {
 ...
 *-------------------------------------------------------------------------
 */
mongoc_topology_t *mongoc_topology_new(const mongoc_uri_t *uri, bool single_threaded) {
    ...
    topology->session_pool = mongoc_server_session_pool_new_with_params(_server_session_init,
    ...
    topology->server_selection_timeout_msec =
 ...
 *-------------------------------------------------------------------------
 */
void mongoc_topology_destroy(mongoc_topology_t *topology) {
    if (!topology) {
        return;
    }
...
bool mongoc_topology_apply_scanned_srv_hosts(mongoc_uri_t *uri,
                                             mongoc_topology_description_t *td,
 ...
 * --------------------------------------------------------------------------
 */
void mongoc_topology_rescan_srv(mongoc_topology_t *topology) {
 ...
 *--------------------------------------------------------------------------
 */
void _mongoc_topology_do_blocking_scan(mongoc_topology_t *topology, bson_error_t *error) {
...
bool mongoc_topology_compatible(const mongoc_topology_description_t *td,
...
static void _mongoc_server_selection_error(const char *msg, const bson_error_t *scanner_error, bson_error_t *error) {
...
mongoc_server_description_t *mongoc_topology_select(mongoc_topology_t *topology,
    ...
    if (server_id) {
        ...
        const mongoc_server_description_t *sd =
            mongoc_topology_description_server_by_id_const(td.ptr, server_id, error);
...
static uint32_t _mongoc_topology_select_server_id_loadbalanced(mongoc_topology_t *topology, bson_error_t *error) {
    const mongoc_server_description_t *selected_server;
...
uint32_t mongoc_topology_select_server_id(mongoc_topology_t *topology,
    ...
    if (topology->single_threaded) {
        ...
        for (;;) {
            ...
            if (try_once) {
                if (tried_once) {
                    _mongoc_server_selection_error("No suitable servers found (`serverSelectionTryOnce` set)",
                                                   &scanner_error,
                                                   error);
...
void _mongoc_topology_request_scan(mongoc_topology_t *topology) {
    _mongoc_topology_background_monitoring_request_scan(topology);
}

bool _mongoc_topology_update_from_handshake(mongoc_topology_t *topology, const mongoc_server_description_t *sd) {
    bool has_server;
    mc_tpld_modification tdmod;
...
void _mongoc_topology_update_last_used(mongoc_topology_t *topology, uint32_t server_id) {
    mongoc_topology_scanner_node_t *node;
...
mongoc_topology_description_type_t _mongoc_topology_get_type(const mongoc_topology_t *topology) {
    mc_shared_tpld td = mc_tpld_take_ref(topology);
...
bool _mongoc_topology_set_appname(mongoc_topology_t *topology, const char *appname) {
    bool ret = false;
...
void _mongoc_topology_update_cluster_time(mongoc_topology_t *topology, const bson_t *reply) {
    bson_iter_t iter;
    bson_iter_t child;
    const uint8_t *data;
    uint32_t size;
...
mongoc_server_session_t *_mongoc_topology_pop_server_session(mongoc_topology_t *topology, bson_error_t *error) {
    int64_t timeout;
...
void _mongoc_topology_push_server_session(mongoc_topology_t *topology, mongoc_server_session_t *server_session) {
    ENTRY;
...
bool _mongoc_topology_end_sessions_cmd(mongoc_topology_t *topology, bson_t *cmd) {
    bson_t ar;
...
void _mongoc_topology_dup_handshake_cmd(const mongoc_topology_t *topology, bson_t *copy_into) {
...
static void _find_topology_version(const bson_t *reply, bson_t *topology_version) {
    bson_iter_t iter;
    const uint8_t *bytes;
    uint32_t len;
...
static bool _handle_sdam_app_error_command(mongoc_topology_t *topology,
...
bool _mongoc_topology_handle_app_error(mongoc_topology_t *topology,
                                       uint32_t server_id,
 ...
 */
static void _topology_collect_errors(const mongoc_topology_description_t *td, bson_error_t *error_out) {
...
void _mongoc_topology_set_rr_resolver(mongoc_topology_t *topology, _mongoc_rr_resolver_fn rr_resolver) {
    topology->rr_resolver = rr_resolver;
}

void _mongoc_topology_set_srv_polling_rescan_interval_ms(mongoc_topology_t *topology, int64_t val) {
    topology->srv_polling_rescan_interval_ms = val;
}

uint32_t _mongoc_topology_get_connection_pool_generation(const mongoc_topology_description_t *td,
                                                         uint32_t server_id,
                                                         const bson_oid_t *service_id) {
    const mongoc_server_description_t *sd;
    bson_error_t error;
...
mc_tpld_modification mc_tpld_modify_begin(mongoc_topology_t *tpl) {
...
void mc_tpld_modify_commit(mc_tpld_modification mod) {
...
void mc_tpld_modify_drop(mc_tpld_modification mod) {
    bson_mutex_unlock(&mod.topology->tpld_modification_mtx);
    mongoc_topology_description_destroy(mod.new_td);
}

bool mongoc_topology_uses_server_api(const mongoc_topology_t *topology) {
