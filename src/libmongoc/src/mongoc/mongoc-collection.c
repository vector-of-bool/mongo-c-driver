...
static void _mongoc_collection_write_command_execute(mongoc_write_command_t *command,
...
static void _mongoc_collection_write_command_execute_idl(mongoc_write_command_t *command,
                                                         const mongoc_collection_t *collection,
...
void mongoc_collection_destroy(mongoc_collection_t *collection) /* IN */
{
    ENTRY;
...
mongoc_collection_t *mongoc_collection_copy(mongoc_collection_t *collection) /* IN */
{
    ENTRY;
...
mongoc_cursor_t *mongoc_collection_aggregate(mongoc_collection_t *collection,       /* IN */
...
mongoc_cursor_t *mongoc_collection_find(mongoc_collection_t *collection,       /* IN */
                                        mongoc_query_flags_t flags,            /* IN */
                                        uint32_t skip,                         /* IN */
                                        uint32_t limit,                        /* IN */
...
bool mongoc_collection_read_command_with_opts(mongoc_collection_t *collection,
...
bool mongoc_collection_write_command_with_opts(mongoc_collection_t *collection,
                                               const bson_t *command,
                                               const bson_t *opts,
                                               bson_t *reply,
                                               bson_error_t *error) {
...
bool mongoc_collection_read_write_command_with_opts(mongoc_collection_t *collection,
                                                    const bson_t *command,
...
bool mongoc_collection_command_with_opts(mongoc_collection_t *collection,
                                         const bson_t *command,
                                         const mongoc_read_prefs_t *read_prefs,
                                         const bson_t *opts,
                                         bson_t *reply,
...
int64_t mongoc_collection_count(mongoc_collection_t *collection,       /* IN */
                                mongoc_query_flags_t flags,            /* IN */
                                const bson_t *query,                   /* IN */
                                int64_t skip,                          /* IN */
                                int64_t limit,                         /* IN */
...
int64_t mongoc_collection_count_with_opts(mongoc_collection_t *collection,       /* IN */
                                          mongoc_query_flags_t flags,            /* IN */
                                          const bson_t *query,                   /* IN */
                                          int64_t skip,                          /* IN */
                                          int64_t limit,                         /* IN */
...
int64_t mongoc_collection_estimated_document_count(mongoc_collection_t *coll,
                                                   const bson_t *opts,
                                                   const mongoc_read_prefs_t *read_prefs,
                                                   bson_t *reply,
                                                   bson_error_t *error) {
    ...
    server_stream = mongoc_cluster_stream_for_reads(&coll->client->cluster, read_prefs, NULL, reply, error);
...
int64_t mongoc_collection_count_documents(mongoc_collection_t *coll,
                                          const bson_t *filter,
                                          const bson_t *opts,
                                          const mongoc_read_prefs_t *read_prefs,
                                          bson_t *reply,
...
bool mongoc_collection_drop(mongoc_collection_t *collection, /* IN */
                            bson_error_t *error)             /* OUT */
{
    return mongoc_collection_drop_with_opts(collection, NULL, error);
}

static bool drop_with_opts(mongoc_collection_t *collection, const bson_t *opts, bson_error_t *error) {
    bool ret;
    bson_t cmd;
...
static bool drop_with_opts_with_encryptedFields(mongoc_collection_t *collection,
                                                const bson_t *opts,
...
bool mongoc_collection_drop_with_opts(mongoc_collection_t *collection, const bson_t *opts, bson_error_t *error) {
    ...
    if (bson_empty(&encryptedFields)) {
        ...
        if (collection->client->topology->encrypted_fields_map != NULL) {
            ...
            if (!_mongoc_get_encryptedFields_from_server(collection->client,
                                                         collection->db,
                                                         mongoc_collection_get_name(collection),
                                                         &encryptedFields,
                                                         error)) {
...
bool mongoc_collection_drop_index(mongoc_collection_t *collection, /* IN */
...
static bool _mongoc_collection_index_keys_equal(const bson_t *expected, const bson_t *actual) {
...
bool _mongoc_collection_create_index_if_not_exists(mongoc_collection_t *collection,
                                                   const bson_t *keys,
                                                   const bson_t *opts,
                                                   bson_error_t *error) {
    mongoc_cursor_t *cursor;
...
bool mongoc_collection_create_index_with_opts(mongoc_collection_t *collection,
                                              const bson_t *keys,
                                              const mongoc_index_opt_t *opt,
                                              const bson_t *opts,
                                              bson_t *reply,
...
bool mongoc_collection_insert_one(mongoc_collection_t *collection,
                                  const bson_t *document,
                                  const bson_t *opts,
                                  bson_t *reply,
                                  bson_error_t *error) {
...
bool mongoc_collection_insert_many(mongoc_collection_t *collection,
                                   const bson_t **documents,
                                   size_t n_documents,
                                   const bson_t *opts,
                                   bson_t *reply,
...
static bool _mongoc_collection_update_or_replace(mongoc_collection_t *collection,
                                                 const bson_t *selector,
                                                 const bson_t *update,
    ...
    _mongoc_write_result_init(&result);
    _mongoc_write_command_init_update_idl(&command,
                                          selector,
                                          update,
                                          &cmd_opts,
                                          extra,
...
bool mongoc_collection_replace_one(mongoc_collection_t *collection,
                                   const bson_t *selector,
...
static bool _mongoc_delete_one_or_many(mongoc_collection_t *collection,
                                       bool multi,
                                       const bson_t *selector,
    ...
    _mongoc_write_command_init_delete_idl(&command,
                                          selector,
                                          &cmd_opts,
                                          &opts,
                                          ++collection->client->cluster.operation_id);
...
bool mongoc_collection_delete_one(mongoc_collection_t *collection,
                                  const bson_t *selector,
                                  const bson_t *opts,
                                  bson_t *reply,
                                  bson_error_t *error) {
...
bool mongoc_collection_delete_many(mongoc_collection_t *collection,
                                   const bson_t *selector,
                                   const bson_t *opts,
                                   bson_t *reply,
                                   bson_error_t *error) {
...
const bson_t *mongoc_collection_get_last_error(const mongoc_collection_t *collection) /* IN */
{
    BSON_ASSERT_PARAM(collection);
...
bool mongoc_collection_validate(mongoc_collection_t *collection, /* IN */
...
mongoc_bulk_operation_t *mongoc_collection_create_bulk_operation_with_opts(mongoc_collection_t *collection,
                                                                           const bson_t *opts) {
