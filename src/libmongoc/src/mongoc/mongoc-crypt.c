...
_log_callback(mongocrypt_log_level_t mongocrypt_log_level, const char *message, uint32_t message_len, void *ctx) {
    ...
    case MONGOCRYPT_LOG_LEVEL_FATAL: log_level = MONGOC_LOG_LEVEL_CRITICAL; break;
    case MONGOCRYPT_LOG_LEVEL_ERROR: log_level = MONGOC_LOG_LEVEL_ERROR; break;
    case MONGOCRYPT_LOG_LEVEL_WARNING: log_level = MONGOC_LOG_LEVEL_WARNING; break;
    case MONGOCRYPT_LOG_LEVEL_INFO: log_level = MONGOC_LOG_LEVEL_INFO; break;
    case MONGOCRYPT_LOG_LEVEL_TRACE: log_level = MONGOC_LOG_LEVEL_TRACE; break;
    default: log_level = MONGOC_LOG_LEVEL_CRITICAL; break;
    }
 ...
 */
bool _ctx_check_error(mongocrypt_ctx_t *ctx, bson_error_t *error, bool error_expected) {
...
bool _kms_ctx_check_error(mongocrypt_kms_ctx_t *kms_ctx, bson_error_t *error, bool error_expected) {
    mongocrypt_status_t *status;
...
bool _crypt_check_error(mongocrypt_t *crypt, bson_error_t *error, bool error_expected) {
    mongocrypt_status_t *status;
...
_state_machine_t *_state_machine_new(_mongoc_crypt_t *crypt) {
 ...
 */
static bool _needs_on_demand_aws_kms(const bson_t *kms_providers, bson_error_t *error) {
    bson_iter_t iter;
 ...
 */
static bool _check_azure_kms_auto(const bson_t *kmsprov, bson_error_t *error) {
    if (error) {
 ...
 * credentials.
 */
static bool _try_add_aws_from_env(bson_t *out, bson_error_t *error) {
 ...
 */
static bool _request_new_azure_token(mcd_azure_access_token *out, bson_error_t *error) {
 ...
 */
static bool _try_add_azure_from_env(_mongoc_crypt_t *crypt, bson_t *out, bson_error_t *error) {
 ...
 * @retval false Otherwise. Check error->code for failure.
 */
static bool _check_gcp_kms_auto(const bson_t *kmsprov, bson_error_t *error) {
    if (error) {
        *error = (bson_error_t){0};
    }
 ...
 * @retval false If any error occurs.
 */
static bool _request_new_gcp_token(gcp_service_account_token *out, bson_error_t *error) {
 ...
 * @retval false If there was an error obtaining or appending credentials
 */
static bool _try_add_gcp_from_env(bson_t *out, bson_error_t *error) {
...
static bool _state_need_kms_credentials(_state_machine_t *sm, bson_error_t *error) {
 ...
 * --------------------------------------------------------------------------
 */
bool _state_machine_run(_state_machine_t *state_machine, bson_t *result, bson_error_t *error) {
    bool ret = false;
    ...
    while (true) {
        ...
        default:
        case MONGOCRYPT_CTX_ERROR: _ctx_check_error(state_machine->ctx, error, true); goto fail;
...
static bool _parse_one_tls_opts(bson_iter_t *iter, mongoc_ssl_opt_t *out_opt, bson_error_t *error) {
    bool ok = false;
...
static bool _parse_all_tls_opts(_mongoc_crypt_t *crypt, const bson_t *tls_opts, bson_error_t *error) {
    bson_iter_t iter;
    bool ok = false;
    bool has_aws = false;
...
/* Note, _mongoc_crypt_t only has one member, to the top-level handle of
   ...
   file.
*/
_mongoc_crypt_t *_mongoc_crypt_new(const bson_t *kms_providers,
...
void _mongoc_crypt_destroy(_mongoc_crypt_t *crypt) {
    if (!crypt) {
        return;
    }
...
bool _mongoc_crypt_auto_encrypt(_mongoc_crypt_t *crypt,
                                mongoc_collection_t *keyvault_coll,
                                mongoc_client_t *mongocryptd_client,
                                mongoc_client_t *collinfo_client,
                                const char *db_name,
...
bool _mongoc_crypt_auto_decrypt(_mongoc_crypt_t *crypt,
                                mongoc_collection_t *keyvault_coll,
                                const bson_t *doc_in,
                                bson_t *doc_out,
                                bson_error_t *error) {
...
static _state_machine_t *_create_explicit_state_machine(_mongoc_crypt_t *crypt,
                                                        mongoc_collection_t *keyvault_coll,
...
bool _mongoc_crypt_explicit_encrypt(_mongoc_crypt_t *crypt,
                                    mongoc_collection_t *keyvault_coll,
                                    const char *algorithm,
                                    const bson_value_t *keyid,
                                    const char *keyaltname,
...
bool _mongoc_crypt_explicit_encrypt_expression(_mongoc_crypt_t *crypt,
                                               mongoc_collection_t *keyvault_coll,
                                               const char *algorithm,
                                               const bson_value_t *keyid,
                                               const char *keyaltname,
...
bool _mongoc_crypt_explicit_decrypt(_mongoc_crypt_t *crypt,
                                    mongoc_collection_t *keyvault_coll,
                                    const bson_value_t *value_in,
                                    bson_value_t *value_out,
                                    bson_error_t *error) {
...
bool _mongoc_crypt_create_datakey(_mongoc_crypt_t *crypt,
                                  const char *kms_provider,
...
bool _mongoc_crypt_rewrap_many_datakey(_mongoc_crypt_t *crypt,
                                       mongoc_collection_t *keyvault_coll,
                                       const bson_t *filter,
                                       const char *provider,
...
const char *_mongoc_crypt_get_crypt_shared_version(const _mongoc_crypt_t *crypt) {
