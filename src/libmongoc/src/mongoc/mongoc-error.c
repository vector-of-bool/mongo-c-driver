...
bool _mongoc_error_is_server(const bson_error_t *error) {
    if (!error) {
        return false;
    }
...
static bool _mongoc_write_error_is_retryable(bson_error_t *error) {
    ...
    case MONGOC_SERVER_ERR_NOTPRIMARYORSECONDARY: return true;
    default: return false;
    }
}

void _mongoc_write_error_append_retryable_label(bson_t *reply) {
...
void _mongoc_write_error_handle_labels(bool cmd_ret,
 ...
 *--------------------------------------------------------------------------
 */
mongoc_read_err_type_t _mongoc_read_error_get_type(bool cmd_ret, const bson_error_t *cmd_err, const bson_t *reply) {
    bson_error_t error;
    ...
    case MONGOC_SERVER_ERR_HOSTUNREACHABLE:
    case MONGOC_SERVER_ERR_NETWORKTIMEOUT:
    case MONGOC_SERVER_ERR_SOCKETEXCEPTION: return MONGOC_READ_ERR_RETRY;
    default:
...
void _mongoc_error_copy_labels_and_upsert(const bson_t *src, bson_t *dst, char *label) {
    bson_iter_t iter;
 ...
 */
bool _mongoc_error_is_shutdown(bson_error_t *error) {
    if (!_mongoc_error_is_server(error)) {
        return false;
    }
    switch (error->code) {
...
bool _mongoc_error_is_not_primary(bson_error_t *error) {
    if (!_mongoc_error_is_server(error)) {
        return false;
    }
    ...
    case MONGOC_ERROR_QUERY_FAILURE: return NULL != strstr(error->message, "not master");
    default: return false;
    }
}

bool _mongoc_error_is_recovering(bson_error_t *error) {
    if (!_mongoc_error_is_server(error)) {
        return false;
    }
    switch (error->code) {
    ...
    case MONGOC_SERVER_ERR_NOTPRIMARYORSECONDARY:
    case MONGOC_SERVER_ERR_PRIMARYSTEPPEDDOWN:
    case MONGOC_SERVER_ERR_SHUTDOWNINPROGRESS: return true;
    /* All errors where no code was found are marked as
     * MONGOC_ERROR_QUERY_FAILURE */
...
bool _mongoc_error_is_state_change(bson_error_t *error) {
    return _mongoc_error_is_recovering(error) || _mongoc_error_is_not_primary(error);
}

bool _mongoc_error_is_network(const bson_error_t *error) {
    if (!error) {
        return false;
    }
...
bool _mongoc_error_is_auth(const bson_error_t *error) {
    if (!error) {
        return false;
    }
