...
static void txn_opts_set(mongoc_transaction_opt_t *opts,
...
static void txn_opts_cleanup(mongoc_transaction_opt_t *opts) {
    /* null inputs are ok */
...
static void txn_opts_copy(const mongoc_transaction_opt_t *src, mongoc_transaction_opt_t *dst) {
...
static bool txn_abort(mongoc_client_session_t *session, bson_t *reply, bson_error_t *error) {
...
static mongoc_write_concern_t *create_commit_retry_wc(const mongoc_write_concern_t *existing_wc) {
...
static bool txn_commit(mongoc_client_session_t *session, bool explicitly_retrying, bson_t *reply, bson_error_t *error) {
    bson_t cmd = BSON_INITIALIZER;
    bson_t opts = BSON_INITIALIZER;
...
retry:
    ...
    if (!r
        && (err_ptr->domain == MONGOC_ERROR_SERVER_SELECTION || error_type == MONGOC_WRITE_ERR_RETRY
...
int64_t mongoc_transaction_opts_get_max_commit_time_ms(mongoc_transaction_opt_t *opts) {
    BSON_ASSERT(opts);
    return opts->max_commit_time_ms;
}

void mongoc_transaction_opts_set_read_concern(mongoc_transaction_opt_t *opts,
                                              const mongoc_read_concern_t *read_concern) {
    BSON_ASSERT(opts);
    mongoc_read_concern_destroy(opts->read_concern);
...
void mongoc_transaction_opts_set_write_concern(mongoc_transaction_opt_t *opts,
                                               const mongoc_write_concern_t *write_concern) {
    BSON_ASSERT(opts);
    mongoc_write_concern_destroy(opts->write_concern);
...
bool mongoc_session_opts_get_snapshot(const mongoc_session_opt_t *opts) {
    ENTRY;
...
void mongoc_session_opts_set_snapshot(mongoc_session_opt_t *opts, bool snapshot) {
    ENTRY;
...
void mongoc_session_opts_set_default_transaction_opts(mongoc_session_opt_t *opts,
...
static void _mongoc_session_opts_copy(const mongoc_session_opt_t *src, mongoc_session_opt_t *dst) {
...
static bool _mongoc_server_session_uuid(uint8_t *data /* OUT */, bson_error_t *error) {
...
bool _mongoc_parse_cluster_time(const bson_t *cluster_time, uint32_t *timestamp, uint32_t *increment) {
    bson_iter_t iter;
    char *s;
...
bool _mongoc_cluster_time_greater(const bson_t *new, const bson_t *old) {
...
void _mongoc_client_session_handle_reply(mongoc_client_session_t *session,
...
bool _mongoc_server_session_init(mongoc_server_session_t *self, bson_error_t *error) {
...
bool _mongoc_server_session_timed_out(const mongoc_server_session_t *server_session, int64_t session_timeout_minutes) {
...
void _mongoc_server_session_destroy(mongoc_server_session_t *self) {
    bson_destroy(&self->lsid);
}

mongoc_client_session_t *_mongoc_client_session_new(mongoc_client_t *client,
...
const bson_t *mongoc_client_session_get_lsid(const mongoc_client_session_t *session) {
    BSON_ASSERT(session);
...
void mongoc_client_session_get_operation_time(const mongoc_client_session_t *session,
                                              uint32_t *timestamp,
                                              uint32_t *increment) {
    BSON_ASSERT(session);
...
void mongoc_client_session_advance_operation_time(mongoc_client_session_t *session,
                                                  uint32_t timestamp,
                                                  uint32_t increment) {
    ENTRY;
...
static bool timeout_exceeded(int64_t expire_at) {
    int64_t current_time = bson_get_monotonic_time();
    return current_time >= expire_at;
}

static bool _max_time_ms_failure(bson_t *reply) {
    bson_iter_t iter;
    ...
    if (bson_iter_find_descendant(&iter, "writeConcernError.codeName", &descendant) && BSON_ITER_HOLDS_UTF8(&descendant)
...
bool mongoc_client_session_with_transaction(mongoc_client_session_t *session,
...
bool mongoc_client_session_start_transaction(mongoc_client_session_t *session,
                                             const mongoc_transaction_opt_t *opts,
                                             bson_error_t *error) {
...
mongoc_transaction_state_t mongoc_client_session_get_transaction_state(const mongoc_client_session_t *session) {
    ...
    switch (session->txn.state) {
    case MONGOC_INTERNAL_TRANSACTION_NONE: RETURN(MONGOC_TRANSACTION_NONE);
    case MONGOC_INTERNAL_TRANSACTION_STARTING: RETURN(MONGOC_TRANSACTION_STARTING);
    case MONGOC_INTERNAL_TRANSACTION_IN_PROGRESS: RETURN(MONGOC_TRANSACTION_IN_PROGRESS);
    case MONGOC_INTERNAL_TRANSACTION_COMMITTED_EMPTY:
    case MONGOC_INTERNAL_TRANSACTION_COMMITTED: RETURN(MONGOC_TRANSACTION_COMMITTED);
    case MONGOC_INTERNAL_TRANSACTION_ABORTED: RETURN(MONGOC_TRANSACTION_ABORTED);
    case MONGOC_INTERNAL_TRANSACTION_ENDING:
...
bool _mongoc_client_session_from_iter(mongoc_client_t *client,
...
bool _mongoc_client_session_in_txn(const mongoc_client_session_t *session) {
    if (!session) {
        return false;
    }
    ...
    switch (session->txn.state) {
    case MONGOC_INTERNAL_TRANSACTION_STARTING:
    case MONGOC_INTERNAL_TRANSACTION_IN_PROGRESS: return true;
    case MONGOC_INTERNAL_TRANSACTION_NONE:
    case MONGOC_INTERNAL_TRANSACTION_ENDING:
    case MONGOC_INTERNAL_TRANSACTION_COMMITTED:
    case MONGOC_INTERNAL_TRANSACTION_COMMITTED_EMPTY:
...
bool _mongoc_client_session_in_txn_or_ending(const mongoc_client_session_t *session) {
    if (!session) {
        return false;
    }
    ...
    case MONGOC_INTERNAL_TRANSACTION_STARTING:
    case MONGOC_INTERNAL_TRANSACTION_IN_PROGRESS:
    case MONGOC_INTERNAL_TRANSACTION_ENDING: return true;
    case MONGOC_INTERNAL_TRANSACTION_NONE:
    case MONGOC_INTERNAL_TRANSACTION_COMMITTED:
    case MONGOC_INTERNAL_TRANSACTION_COMMITTED_EMPTY:
    case MONGOC_INTERNAL_TRANSACTION_ABORTED:
...
bool _mongoc_client_session_txn_in_progress(const mongoc_client_session_t *session) {
    if (!session) {
        return false;
    }
...
bool _mongoc_client_session_append_txn(mongoc_client_session_t *session, bson_t *cmd, bson_error_t *error) {
...
void _mongoc_client_session_append_read_concern(const mongoc_client_session_t *cs,
                                                const bson_t *rc,
...
void _mongoc_client_session_unpin(mongoc_client_session_t *session) {
    BSON_ASSERT(session);

    session->server_id = 0;
}

void _mongoc_client_session_pin(mongoc_client_session_t *session, uint32_t server_id) {
    BSON_ASSERT(session);

    session->server_id = server_id;
}

void _mongoc_client_session_set_snapshot_time(mongoc_client_session_t *session, uint32_t t, uint32_t i) {
    BSON_ASSERT(session);
...
void _mongoc_client_session_clear_snapshot_time(mongoc_client_session_t *session) {
    BSON_ASSERT(session);

    session->snapshot_time_set = false;
}

bool mongoc_client_session_get_dirty(mongoc_client_session_t *session) {
