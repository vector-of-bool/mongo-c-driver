...
static bool _translate_query_opt(const char *query_field, const char **cmd_field, int *len);
...
static int64_t _mongoc_cursor_get_opt_int64(const mongoc_cursor_t *cursor, const char *option, int64_t default_value) {
    bson_iter_t iter;
...
static bool _mongoc_cursor_set_opt_bool(mongoc_cursor_t *cursor, const char *option, bool value) {
    bson_iter_t iter;
...
static const char *_first_dollar_field(const bson_t *bson) {
    bson_iter_t iter;
    const char *key;
 ...
 * error may be NULL.
 */
void _mongoc_cursor_flags_to_opts(mongoc_query_flags_t qflags,
                                  bson_t *opts, /* IN/OUT */
...
bool mongoc_cursor_error_document(mongoc_cursor_t *cursor, bson_error_t *error, const bson_t **doc) {
    ENTRY;
...
static mongoc_cursor_state_t _call_transition(mongoc_cursor_t *cursor) {
    mongoc_cursor_state_t state = cursor->state;
    _mongoc_cursor_impl_transition_t fn = NULL;
    switch (state) {
    case UNPRIMED: fn = cursor->impl.prime; break;
    case IN_BATCH: fn = cursor->impl.pop_from_batch; break;
    case END_OF_BATCH: fn = cursor->impl.get_next_batch; break;
    case DONE:
...
bool mongoc_cursor_is_alive(const mongoc_cursor_t *cursor) /* IN */
{
    return mongoc_cursor_more((mongoc_cursor_t *)cursor);
}

const bson_t *mongoc_cursor_current(const mongoc_cursor_t *cursor) /* IN */
{
    BSON_ASSERT(cursor);
