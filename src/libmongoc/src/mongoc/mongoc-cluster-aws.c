 ...
 */
static bool _run_command(mongoc_cluster_t *cluster,
 ...
 */
static bool _sasl_reply_parse_payload_as_bson(const bson_t *reply, bson_t *payload, bson_error_t *error) {
    bson_iter_t iter;
 ...
 * credentials.
 */
static bool _send_http_request(bool use_tls,
                               const char *ip,
                               int port,
                               const char *method,
                               const char *path,
...
static bool _creds_empty(_mongoc_aws_credentials_t *creds) {
 ...
 * set.
 */
static bool _validate_and_set_creds(const char *access_key_id,
 ...
 * set.
 */
static bool _obtain_creds_from_uri(_mongoc_aws_credentials_t *creds, mongoc_uri_t *uri, bson_error_t *error) {
    bool ret = false;
...
static bool _obtain_creds_from_env(_mongoc_aws_credentials_t *creds, bson_error_t *error) {
    bool ret = false;
...
static bool expiration_ms_to_timer(int64_t expiration_ms, mcd_timer *expiration_timer, bson_error_t *error) {
    bool ret = false;
...
static bool expiration_iso8601_to_timer(const char *expiration_str, mcd_timer *expiration_timer, bson_error_t *error) {
    bool ret = false;
...
static char *generate_AWS_ROLE_SESSION_NAME(bson_error_t *error) {
#define NUM_BYTES 16
    bool ok = false;
...
static bool _obtain_creds_from_ecs(_mongoc_aws_credentials_t *creds, bson_error_t *error) {
    bool ret = false;
...
static bool _obtain_creds_from_assumerolewithwebidentity(_mongoc_aws_credentials_t *creds, bson_error_t *error) {
    bool ret = false;
    char *http_response_headers = NULL;
    char *http_response_body = NULL;
...
static bool _obtain_creds_from_ec2(_mongoc_aws_credentials_t *creds, bson_error_t *error) {
    bool ret = false;
    char *http_response_headers = NULL;
    char *http_response_body = NULL;
 ...
 * On failure, returns false and sets error.
 */
bool _mongoc_aws_credentials_obtain(mongoc_uri_t *uri, _mongoc_aws_credentials_t *creds, bson_error_t *error) {
    bool ret = false;
 ...
 */
bool _mongoc_validate_and_derive_region(char *sts_fqdn, size_t sts_fqdn_len, char **region, bson_error_t *error) {
    bool ret = false;
    char *ptr;
    char *ptr_prev;
 ...
 * --------------------------------------------------------------------------
 */
static bool _client_first(mongoc_cluster_t *cluster,
                          mongoc_stream_t *stream,
                          mongoc_server_description_t *sd,
 ...
 * --------------------------------------------------------------------------
 */
static bool _client_second(mongoc_cluster_t *cluster,
                           mongoc_stream_t *stream,
                           mongoc_server_description_t *sd,
                           _mongoc_aws_credentials_t *creds,
...
bool _mongoc_cluster_auth_node_aws(mongoc_cluster_t *cluster,
                                   mongoc_stream_t *stream,
                                   mongoc_server_description_t *sd,
                                   bson_error_t *error) {
    bool ret = false;
...
void _mongoc_aws_credentials_cleanup(_mongoc_aws_credentials_t *creds) {
    bson_free(creds->access_key_id);
    bson_free(creds->secret_access_key);
    bson_free(creds->session_token);
}

void _mongoc_aws_credentials_copy_to(const _mongoc_aws_credentials_t *src, _mongoc_aws_credentials_t *dst) {
...
void _mongoc_aws_credentials_cache_init(void) {
    _mongoc_aws_credentials_cache_t *cache = &mongoc_aws_credentials_cache;
    bson_mutex_init(&cache->mutex);
}

static bool check_expired(const _mongoc_aws_credentials_t *creds) {
    if (!creds->expiration.set) {
        return true;
    }
...
void _mongoc_aws_credentials_cache_lock(void) {
    _mongoc_aws_credentials_cache_t *cache = &mongoc_aws_credentials_cache;
    bson_mutex_lock(&cache->mutex);
}

void _mongoc_aws_credentials_cache_unlock(void) {
    _mongoc_aws_credentials_cache_t *cache = &mongoc_aws_credentials_cache;
    bson_mutex_unlock(&cache->mutex);
}

void _mongoc_aws_credentials_cache_put_nolock(const _mongoc_aws_credentials_t *creds) {
    _mongoc_aws_credentials_cache_t *cache = &mongoc_aws_credentials_cache;
    BSON_ASSERT_PARAM(creds);
...
void _mongoc_aws_credentials_cache_put(const _mongoc_aws_credentials_t *creds) {
    _mongoc_aws_credentials_cache_lock();
    _mongoc_aws_credentials_cache_put_nolock(creds);
    _mongoc_aws_credentials_cache_unlock();
}

bool _mongoc_aws_credentials_cache_get_nolock(_mongoc_aws_credentials_t *creds) {
    _mongoc_aws_credentials_cache_t *cache = &mongoc_aws_credentials_cache;
    BSON_ASSERT_PARAM(creds);
...
bool _mongoc_aws_credentials_cache_get(_mongoc_aws_credentials_t *creds) {
    _mongoc_aws_credentials_cache_lock();
...
void _mongoc_aws_credentials_cache_clear_nolock(void) {
    _mongoc_aws_credentials_cache_t *cache = &mongoc_aws_credentials_cache;
...
void _mongoc_aws_credentials_cache_clear(void) {
    _mongoc_aws_credentials_cache_lock();
    _mongoc_aws_credentials_cache_clear_nolock();
    _mongoc_aws_credentials_cache_unlock();
}

void _mongoc_aws_credentials_cache_cleanup(void) {
    _mongoc_aws_credentials_cache_t *cache = &mongoc_aws_credentials_cache;
    if (cache->cached.set) {
        _mongoc_aws_credentials_cleanup(&cache->cached.value);
    }
