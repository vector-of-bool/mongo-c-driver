...
static void _mongoc_client_op_killcursors(mongoc_cluster_t *cluster,
...
static void _mongoc_client_killcursors_command(mongoc_cluster_t *cluster,
                                               mongoc_server_stream_t *server_stream,
                                               int64_t cursor_id,
                                               const char *db,
                                               const char *collection,
...
static bool srv_callback(const char *hostname, PDNS_RECORD pdns, mongoc_rr_data_t *rr_data, bson_error_t *error) {
...
static bool txt_callback(const char *hostname, PDNS_RECORD pdns, mongoc_rr_data_t *rr_data, bson_error_t *error) {
    DWORD i;
    bson_string_t *txt;
...
static const char *_mongoc_hstrerror(int code) {
    switch (code) {
    case HOST_NOT_FOUND: return "The specified host is unknown.";
    case NO_ADDRESS: return "The requested name is valid but does not have an IP address.";
...
static bool _mongoc_get_rr_search(const char *hostname,
...
bool _mongoc_client_get_rr(const char *hostname,
                           mongoc_rr_type_t rr_type,
                           mongoc_rr_data_t *rr_data,
                           size_t initial_buffer_size,
                           bson_error_t *error) {
...
static mongoc_stream_t *mongoc_client_connect_unix(const mongoc_host_list_t *host, bson_error_t *error) {
#ifdef _WIN32
    ENTRY;
    bson_set_error(error,
                   MONGOC_ERROR_STREAM,
...
mongoc_stream_t *mongoc_client_connect(bool buffered,
                                       bool use_ssl,
                                       void *ssl_opts_void,
    ...
    case AF_INET6:
#endif
    case AF_INET: base_stream = mongoc_client_connect_tcp(connecttimeoutms, host, error); break;
    case AF_UNIX: base_stream = mongoc_client_connect_unix(host, error); break;
    default:
        bson_set_error(error,
                       MONGOC_ERROR_STREAM,
...
mongoc_stream_t *mongoc_client_default_stream_initiator(const mongoc_uri_t *uri,
                                                        const mongoc_host_list_t *host,
                                                        void *user_data,
                                                        bson_error_t *error) {
...
void _mongoc_client_set_internal_tls_opts(mongoc_client_t *client, _mongoc_internal_tls_opts_t *internal) {
    if (!client->use_ssl) {
        return;
    }
...
mongoc_client_t *mongoc_client_new_from_uri_with_error(const mongoc_uri_t *uri, bson_error_t *error) {
    mongoc_client_t *client;
...
mongoc_client_t *_mongoc_client_new_from_topology(mongoc_topology_t *topology) {
    mongoc_client_t *client;
...
static bool _mongoc_client_retryable_write_command_with_stream(mongoc_client_t *client,
...
static bool _mongoc_client_retryable_read_command_with_stream(mongoc_client_t *client,
                                                              mongoc_cmd_parts_t *parts,
                                                              mongoc_server_stream_t *server_stream,
                                                              bson_t *reply,
                                                              bson_error_t *error) {
...
static bool _mongoc_client_command_with_stream(mongoc_client_t *client,
                                               mongoc_cmd_parts_t *parts,
                                               const mongoc_read_prefs_t *read_prefs,
                                               mongoc_server_stream_t *server_stream,
                                               bson_t *reply,
...
bool mongoc_client_command_simple(mongoc_client_t *client,
     ...
     * preference argument."
     */
    server_stream = mongoc_cluster_stream_for_reads(cluster, read_prefs, NULL, reply, error);
 ...
 *--------------------------------------------------------------------------
 */
bool _mongoc_client_command_with_opts(mongoc_client_t *client,
                                      const char *db_name,
                                      const bson_t *command,
    ...
    if (read_write_opts.serverId) {
        ...
        server_stream = mongoc_cluster_stream_for_reads(cluster, prefs, cs, reply_ptr, error);
    }
    ...
    if (mode & MONGOC_CMD_WRITE) {
        wc_wire_version = !strcasecmp(command_name, "findandmodify") ? WIRE_VERSION_FAM_WRITE_CONCERN
...
bool mongoc_client_command_with_opts(mongoc_client_t *client,
                                     const char *db_name,
                                     const bson_t *command,
                                     const mongoc_read_prefs_t *read_prefs,
                                     const bson_t *opts,
...
static void _mongoc_client_prepare_killcursors_command(int64_t cursor_id, const char *collection, bson_t *command) {
    bson_t child;
...
void _mongoc_client_kill_cursor(mongoc_client_t *client,
                                uint32_t server_id,
                                int64_t cursor_id,
                                int64_t operation_id,
                                const char *db,
...
static void _mongoc_client_monitor_op_killcursors(mongoc_cluster_t *cluster,
                                                  mongoc_server_stream_t *server_stream,
                                                  int64_t cursor_id,
                                                  int64_t operation_id,
                                                  const char *db,
...
static void _mongoc_client_monitor_op_killcursors_succeeded(mongoc_cluster_t *cluster,
                                                            int64_t duration,
                                                            mongoc_server_stream_t *server_stream,
                                                            int64_t cursor_id,
                                                            int64_t operation_id) {
...
static void _mongoc_client_monitor_op_killcursors_failed(mongoc_cluster_t *cluster,
                                                         int64_t duration,
                                                         mongoc_server_stream_t *server_stream,
...
static void _mongoc_client_op_killcursors(mongoc_cluster_t *cluster,
    ...
    if (has_ns) {
        if (r) {
            _mongoc_client_monitor_op_killcursors_succeeded(cluster,
                                                            bson_get_monotonic_time() - started,
                                                            server_stream,
                                                            cursor_id,
                                                            operation_id);
        } else {
            _mongoc_client_monitor_op_killcursors_failed(cluster,
                                                         bson_get_monotonic_time() - started,
                                                         server_stream,
                                                         &error,
                                                         operation_id);
...
int32_t mongoc_client_get_max_message_size(mongoc_client_t *client) /* IN */
{
    BSON_ASSERT(client);
...
int32_t mongoc_client_get_max_bson_size(mongoc_client_t *client) /* IN */
{
    BSON_ASSERT(client);
...
bool mongoc_client_get_server_status(mongoc_client_t *client,         /* IN */
...
bool _mongoc_client_set_apm_callbacks_private(mongoc_client_t *client,
...
mongoc_server_session_t *_mongoc_client_pop_server_session(mongoc_client_t *client, bson_error_t *error) {
 ...
 *--------------------------------------------------------------------------
 */
bool _mongoc_client_lookup_session(const mongoc_client_t *client,
...
void _mongoc_client_unregister_session(mongoc_client_t *client, mongoc_client_session_t *session) {
    mongoc_set_rm(client->client_sessions, session->client_session_id);
}

void _mongoc_client_push_server_session(mongoc_client_t *client, mongoc_server_session_t *server_session) {
...
void _mongoc_client_end_sessions(mongoc_client_t *client) {
...
bool mongoc_client_enable_auto_encryption(mongoc_client_t *client,
...
mongoc_server_description_t *mongoc_client_get_handshake_description(mongoc_client_t *client,
                                                                     uint32_t server_id,
                                                                     bson_t *opts,
                                                                     bson_error_t *error) {
    mongoc_server_stream_t *server_stream;
