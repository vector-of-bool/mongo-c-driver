...
static int _mongoc_openssl_password_cb(char *buf, int num, int rwflag, void *user_data) {
...
#ifdef _WIN32
bool _mongoc_openssl_import_cert_store(LPWSTR store_name, DWORD dwFlags, X509_STORE *openssl_store) {
...
bool _mongoc_openssl_import_cert_stores(SSL_CTX *context) {
    bool retval;
...
bool _mongoc_openssl_check_peer_hostname(SSL *ssl, const char *host, bool allow_invalid_hostname) {
    X509 *peer = NULL;
 ...
 *
 */
static bool _mongoc_openssl_hostcheck(const char *pattern, const char *hostname) {
...
static bool _mongoc_openssl_setup_ca(SSL_CTX *ctx, const char *cert, const char *cert_dir) {
    BSON_ASSERT(ctx);
...
static bool _mongoc_openssl_setup_crl(SSL_CTX *ctx, const char *crlfile) {
    X509_STORE *store;
    X509_LOOKUP *lookup;
    int status;
...
static bool _mongoc_openssl_setup_pem_file(SSL_CTX *ctx, const char *pem_file, const char *password) {
...
static X509 *_get_issuer(X509 *cert, STACK_OF(X509) * chain) {
 ...
 * byte. */
bool _mongoc_tlsfeature_has_status_request(const uint8_t *data, int length) {
    int i;
...
bool _get_must_staple(X509 *cert) {
...
int _mongoc_ocsp_tlsext_status(SSL *ssl, mongoc_openssl_ocsp_opt_t *opts) {
...
validate:
    ...
    default: MONGOC_DEBUG("OCSP Certificate Status: Unknown"); break;
    }
...
char *_mongoc_openssl_extract_subject(const char *filename, const char *passphrase) {
...
static unsigned long _mongoc_openssl_thread_id_callback(void) {
    unsigned long ret;
...
static void _mongoc_openssl_thread_locking_callback(int mode, int type, const char *file, int line) {
    if (mode & CRYPTO_LOCK) {
