...
char *_mongoc_cfstringref_to_cstring(CFStringRef str) {
    CFIndex length;
...
static void _bson_append_cftyperef(bson_string_t *retval, const char *label, CFTypeRef str) {
    char *cs;
...
CFTypeRef _mongoc_secure_transport_dict_get(CFArrayRef values, CFStringRef label) {
...
char *_mongoc_secure_transport_RFC2253_from_cert(SecCertificateRef cert) {
    CFTypeRef value;
...
bool _mongoc_secure_transport_import_pem(const char *filename,
    ...
    if (error) {
        ...
        MONGOC_ERROR("Failed importing PEM '%s': %s",
                     filename,
    ...
    if (type == kSecItemTypeAggregate) {
        for (CFIndex i = 0; i < CFArrayGetCount(items); ++i) {
            ...
            if (item_id == SecCertificateGetTypeID()) {
                retval =
                    _mongoc_secure_transport_RFC2253_from_cert((SecCertificateRef)CFArrayGetValueAtIndex(items, i));
                break;
            }
        }
...
OSStatus mongoc_secure_transport_read(SSLConnectionRef connection, void *data, size_t *data_length) {
    ...
    switch (errno) {
    case ENOENT: RETURN(errSSLClosedGraceful); break;
    case ECONNRESET: RETURN(errSSLClosedAbort); break;
    case EAGAIN: RETURN(errSSLWouldBlock); break;
    default:
        RETURN(-36); /* ioErr */
        break;
    }
