...
void mongoc_cmd_parts_init(mongoc_cmd_parts_t *parts,
...
static void _mongoc_cmd_parts_ensure_copied(mongoc_cmd_parts_t *parts) {
...
static void _mongoc_cmd_parts_add_write_concern(mongoc_cmd_parts_t *parts) {
...
static void _mongoc_cmd_parts_add_read_prefs(bson_t *query, const mongoc_read_prefs_t *prefs) {
    bson_t child;
    const char *mode_str;
    const bson_t *tags;
    int64_t stale;
...
static void _iter_concat(bson_t *dst, bson_iter_t *iter) {
    uint32_t len;
    const uint8_t *data;
    bson_t src;
 ...
 */
static void _mongoc_cmd_parts_assemble_mongos(mongoc_cmd_parts_t *parts, const mongoc_server_stream_t *server_stream) {
     ...
     */
    switch (mode) {
    case MONGOC_READ_PRIMARY: break;
    ...
    default: parts->assembled.query_flags |= MONGOC_QUERY_SECONDARY_OK; add_read_prefs = true;
    }
...
static void _mongoc_cmd_parts_assemble_mongod(mongoc_cmd_parts_t *parts, const mongoc_server_stream_t *server_stream) {
    ENTRY;
...
static const bson_t *_largest_cluster_time(const bson_t *a, const bson_t *b) {
    if (!a) {
        return b;
    }
 ...
 */
static bool _allow_txn_number(const mongoc_cmd_parts_t *parts, const mongoc_server_stream_t *server_stream) {
...
static bool _is_retryable_write(const mongoc_cmd_parts_t *parts, const mongoc_server_stream_t *server_stream) {
...
void mongoc_cmd_parts_cleanup(mongoc_cmd_parts_t *parts) {
    bson_destroy(&parts->read_concern_document);
    bson_destroy(&parts->write_concern_document);
