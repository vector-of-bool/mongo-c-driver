...
static void _mongoc_monitor_legacy_write(mongoc_client_t *client,
 ...
 */
static void _mongoc_monitor_legacy_write_succeeded(mongoc_client_t *client,
                                                   int64_t duration,
                                                   mongoc_write_command_t *command,
                                                   mongoc_server_stream_t *stream,
                                                   int64_t request_id) {
...
void _mongoc_write_command_delete_legacy(mongoc_write_command_t *command,
    ...
    const int32_t max_bson_obj_size = mongoc_server_stream_max_bson_obj_size(server_stream);
    ...
    while ((bson = bson_reader_read(reader, &eof))) {
        ...
        if (bson_cmp_greater_us(len, max_bson_obj_size)) {
            ...
            _mongoc_write_command_too_large_error(error, 0, (int32_t)len, max_bson_obj_size);
            result->failed = true;
...
again:
    ...
    while ((bson = bson_reader_read(reader, &eof))) {
        ...
        if (bson_cmp_greater_us(bson->len, max_bson_obj_size)) {
            ...
            _mongoc_write_command_too_large_error(error, idx, (int32_t)bson->len, max_bson_obj_size);
...
void _mongoc_write_command_update_legacy(mongoc_write_command_t *command,
    ...
    bson_reader_t *const reader = bson_reader_new_from_data(command->payload.data, command->payload.len);
