...
const char *first_key(const bson_t *bson) {
    bson_iter_t iter;
...
void check_operation_ids(const bson_t *events) {
    bson_iter_t iter;
    ...
    while (bson_iter_next(&iter)) {
        ...
        if (!strcmp(first_key(&event), "command_started_event")) {
            ...
            if (first_operation_id == -1) {
                ...
                test_error("%s sent wrong operation_id",
...
static bool command_monitoring_test_run_operation(json_test_ctx_t *ctx, const bson_t *test, const bson_t *operation) {
    bson_t reply;
    bool res;
...
static void test_command_monitoring_cb(bson_t *scenario) {
...
static void test_get_error_failed_cb(const mongoc_apm_command_failed_t *event) {
    bson_error_t *error;
...
static void insert_200_docs(mongoc_collection_t *collection) {
    int i;
    bson_t *doc;
    bool r;
    bson_error_t error;
...
static void increment(const mongoc_apm_command_started_t *event) {
    int *i = (int *)mongoc_apm_command_started_get_context(event);

    ++(*i);
}

static mongoc_apm_callbacks_t *increment_callbacks(void) {
    mongoc_apm_callbacks_t *callbacks;
...
static void decrement(const mongoc_apm_command_started_t *event) {
    int *i = (int *)mongoc_apm_command_started_get_context(event);

    --(*i);
}

static mongoc_apm_callbacks_t *decrement_callbacks(void) {
    mongoc_apm_callbacks_t *callbacks;
...
static void test_change_callbacks(void) {
...
static void test_reset_callbacks(void) {
    mongoc_apm_callbacks_t *inc_callbacks;
    mongoc_apm_callbacks_t *dec_callbacks;
    int incremented = 0;
    int decremented = 0;
...
static void test_set_callbacks_cb(const mongoc_apm_command_started_t *event) {
    int *n_calls = (int *)mongoc_apm_command_started_get_context(event);

    (*n_calls)++;
}

static void _test_set_callbacks(bool pooled, bool try_pop) {
    mongoc_client_t *client;
...
static void test_set_callbacks_single(void) {
    _test_set_callbacks(false, false);
}

static void test_set_callbacks_pooled(void) {
    _test_set_callbacks(true, false);
}

static void test_set_callbacks_pooled_try_pop(void) {
...
static void op_id_test_init(op_id_test_t *test) {
...
static void op_id_test_cleanup(op_id_test_t *test) {
    _mongoc_array_destroy(&test->started_ids);
    _mongoc_array_destroy(&test->succeeded_ids);
    _mongoc_array_destroy(&test->failed_ids);
}

static void test_op_id_started_cb(const mongoc_apm_command_started_t *event) {
    op_id_test_t *test;
    ids_t ids;
...
static void test_op_id_succeeded_cb(const mongoc_apm_command_succeeded_t *event) {
    op_id_test_t *test;
    ids_t ids;
...
static void test_op_id_failed_cb(const mongoc_apm_command_failed_t *event) {
    op_id_test_t *test;
    ids_t ids;
...
static void _test_bulk_operation_id(bool pooled, bool use_bulk_operation_new) {
    mongoc_client_t *client;
    mongoc_client_pool_t *pool = NULL;
    mongoc_apm_callbacks_t *callbacks;
    mongoc_collection_t *collection;
...
static void test_collection_bulk_op_single(void) {
    _test_bulk_operation_id(false, false);
}

static void test_collection_bulk_op_pooled(void) {
    _test_bulk_operation_id(true, false);
}

static void test_bulk_op_single(void) {
    _test_bulk_operation_id(false, true);
}

static void test_bulk_op_pooled(void) {
    _test_bulk_operation_id(true, true);
}

static void _test_query_operation_id(bool pooled) {
    mock_server_t *server;
    mongoc_client_t *client;
    mongoc_client_pool_t *pool = NULL;
    mongoc_apm_callbacks_t *callbacks;
...
static void test_query_operation_id_single_cmd(void) {
    _test_query_operation_id(false);
}

static void test_query_operation_id_pooled_cmd(void) {
...
static void cmd_test_init(cmd_test_t *test) {
    memset(test, 0, sizeof *test);
    bson_init(&test->cmd);
}

static void cmd_test_cleanup(cmd_test_t *test) {
    bson_destroy(&test->cmd);
}

static void cmd_started_cb(const mongoc_apm_command_started_t *event) {
    cmd_test_t *test;
...
static void cmd_succeeded_cb(const mongoc_apm_command_succeeded_t *event) {
    cmd_test_t *test;
    int64_t duration;
...
static void cmd_failed_cb(const mongoc_apm_command_failed_t *event) {
    cmd_test_t *test;
    int64_t duration;
...
static void set_cmd_test_callbacks(mongoc_client_t *client, void *context) {
    mongoc_apm_callbacks_t *callbacks;
...
static void test_client_cmd(void) {
    cmd_test_t test;
    mongoc_client_t *client;
    mongoc_cursor_t *cursor;
    const bson_t *reply;
...
static void test_client_cmd_simple(void) {
    cmd_test_t test;
    mongoc_client_t *client;
    bool r;
    bson_error_t error;
...
static void test_client_cmd_op_ids(void) {
    op_id_test_t test;
    mongoc_client_t *client;
    mongoc_apm_callbacks_t *callbacks;
    bool r;
...
static void test_killcursors_deprecated(void *unused) {
    cmd_test_t test;
    mongoc_client_t *client;
    bool r;
    bson_error_t error;
...
static void cmd_failed_reply_test_init(cmd_failed_reply_test_t *test) {
    memset(test, 0, sizeof *test);
    bson_init(&test->reply);
}

static void cmd_failed_reply_test_cleanup(cmd_failed_reply_test_t *test) {
    bson_destroy(&test->reply);
}

static void command_failed_reply_command_failed_cb(const mongoc_apm_command_failed_t *event) {
...
static void test_command_failed_reply_mock(void) {
    mock_server_t *server;
    mongoc_client_t *client;
    mongoc_apm_callbacks_t *callbacks;
    mongoc_collection_t *collection;
...
static void test_command_failed_reply_hangup(void) {
    mock_server_t *server;
    mongoc_client_t *client;
    mongoc_apm_callbacks_t *callbacks;
    mongoc_collection_t *collection;
...
static void assert_service_id(service_id_test_t *test, const bson_oid_t *actual_service_id) {
...
static void service_id_cmd_started_cb(const mongoc_apm_command_started_t *event) {
...
static void service_id_cmd_succeeded_cb(const mongoc_apm_command_succeeded_t *event) {
...
static void service_id_cmd_failed_cb(const mongoc_apm_command_failed_t *event) {
...
static void _test_service_id(bool is_loadbalanced) {
    mock_server_t *server;
    mongoc_client_t *client;
    mongoc_uri_t *uri;
    request_t *request;
...
void test_service_id_loadbalanced(void) {
    _test_service_id(true);
}

void test_service_id_not_loadbalanced(void) {
    _test_service_id(false);
}

void test_command_monitoring_install(TestSuite *suite) {
