...
static void _test_gcp_parse(void) {
    ...
    ASSERT(gcp_access_token_try_parse_from_json(
        &token,
        BSON_STR({"access_token" : "helloworld", "token_type" : "bearer", "expires_in" : "3788"}),
        -1,
        &error));
...
static void _test_gcp_http_request(void) {
...
static const char *_get_test_host(void) {
    return getenv("TEST_KMS_PROVIDER_HOST");
}

static void _run_http_test_case(const char *case_,
...
static void _test_with_mock_server(void *ctx) {
    BSON_UNUSED(ctx);
...
static int have_mock_server_env(TestSuite *ctx) {
    return _get_test_host() != NULL;
}

void test_service_gcp_install(TestSuite *suite) {
