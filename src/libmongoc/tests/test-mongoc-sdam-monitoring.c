...
static void check_json_sdam_events(const bson_t *events, const bson_t *expectations) {
...
static void context_init(context_t *context) {
...
static void context_append(context_t *context, bson_t *event) {
    char str[16];
    const char *key;
...
static void append_array(bson_t *bson, const char *key, const bson_t *array) {
    if (array->len) {
...
static void sd_to_bson(const mongoc_server_description_t *sd, bson_t *bson) {
...
static void td_to_bson(const mongoc_topology_description_t *td, bson_t *bson) {
    bson_t servers = BSON_INITIALIZER;
    bson_t server;
    char str[16];
    const char *key;
    const mongoc_set_t *servers_set = mc_tpld_servers_const(td);
...
static void server_heartbeat_succeeded(const mongoc_apm_server_heartbeat_succeeded_t *event) {
    context_t *ctx;
    const mongoc_host_list_t *host;
    int64_t duration;
...
static void server_heartbeat_failed(const mongoc_apm_server_heartbeat_failed_t *event) {
    context_t *ctx;
    const mongoc_host_list_t *host;
    int64_t duration;
...
static mongoc_apm_callbacks_t *topology_event_callbacks(void) {
...
static void client_set_topology_event_callbacks(mongoc_client_t *client, context_t *context) {
    mongoc_apm_callbacks_t *callbacks;
...
static void pool_set_topology_event_callbacks(mongoc_client_pool_t *pool, context_t *context) {
    mongoc_apm_callbacks_t *callbacks;
...
static mongoc_apm_callbacks_t *heartbeat_event_callbacks(void) {
    mongoc_apm_callbacks_t *callbacks;
...
static void client_set_heartbeat_event_callbacks(mongoc_client_t *client, context_t *context) {
    mongoc_apm_callbacks_t *callbacks;
...
static void pool_set_heartbeat_event_callbacks(mongoc_client_pool_t *pool, context_t *context) {
    mongoc_apm_callbacks_t *callbacks;
 ...
 *-----------------------------------------------------------------------
 */
static void test_sdam_monitoring_cb(bson_t *test) {
...
static void test_topology_events_single(void) {
    _test_topology_events(false);
}

static void test_topology_events_pooled(void) {
    _test_topology_events(true);
}

static void test_topology_events_disabled(void) {
    mongoc_client_t *client;
    context_t context;
    bool r;
    bson_error_t error;
...
static void _test_heartbeat_events(bool pooled, bool succeeded) {
    context_t context;
...
static void test_heartbeat_events_single_succeeded(void) {
    _test_heartbeat_events(false, true);
}

static void test_heartbeat_events_pooled_succeeded(void) {
    _test_heartbeat_events(true, true);
}

static void test_heartbeat_events_single_failed(void) {
    _test_heartbeat_events(false, false);
}

static void test_heartbeat_events_pooled_failed(void) {
    _test_heartbeat_events(true, false);
}

static void _test_heartbeat_fails_dns(bool pooled) {
    context_t context;
    mongoc_uri_t *uri;
    mongoc_client_t *client;
    mongoc_client_pool_t *pool = NULL;
...
static void test_heartbeat_fails_dns_single(void *ctx) {
    BSON_UNUSED(ctx);

    _test_heartbeat_fails_dns(false);
}

static void test_heartbeat_fails_dns_pooled(void *ctx) {
    BSON_UNUSED(ctx);
...
void duplicates_server_changed(const mongoc_apm_server_changed_t *event) {
...
void duplicates_topology_changed(const mongoc_apm_topology_changed_t *event) {
    duplicates_counter_t *counters;
...
void test_sdam_monitoring_install(TestSuite *suite) {
