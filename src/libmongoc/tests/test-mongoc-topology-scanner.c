...
static void test_topology_scanner_helper(uint32_t id,
                                         const bson_t *bson,
                                         int64_t rtt_msec,
                                         void *data,
...
static void _test_topology_scanner(bool with_ssl) {
    ...
    for (i = 0; i < NSERVERS; i++) {
        ...
        mongoc_topology_scanner_add(topology_scanner,
...
void test_topology_scanner(void) {
    _test_topology_scanner(false);
}

#ifdef MONGOC_ENABLE_SSL_OPENSSL
void test_topology_scanner_ssl(void) {
 ...
 */
void test_topology_scanner_discovery(void) {
...
void test_topology_scanner_oscillate(void) {
...
void test_topology_scanner_connection_error(void) {
    mongoc_client_t *client;
    bson_error_t error;
...
void test_topology_scanner_socket_timeout(void) {
...
static void test_topology_scanner_blocking_initiator(void) {
    mock_rs_t *rs;
    mongoc_uri_t *uri;
    mongoc_client_t *client;
    initiator_data_t data;
...
static mock_server_t *_mock_server_listening_on(char *server_bind_to) {
...
static void _test_topology_scanner_dns_helper(uint32_t id,
                                              const bson_t *bson,
                                              int64_t rtt_msec,
                                              void *data,
                                              const bson_error_t *error /* IN */) {
...
static void test_topology_scanner_dns_testcase(dns_testcase_t *testcase) {
...
static void test_topology_scanner_dns(void) {
...
static void _retired_fails_to_initiate_cb(uint32_t id,
                                          const bson_t *bson,
                                          int64_t rtt_msec,
                                          void *data,
                                          const bson_error_t *error /* IN */) {
...
static mongoc_stream_t *null_initiator(mongoc_async_cmd_t *acmd) {
    BSON_UNUSED(acmd);
 ...
 */
static void test_topology_retired_fails_to_initiate(void) {
    mock_server_t *server;
    mongoc_topology_scanner_t *scanner;
...
static void _test_topology_scanner_does_not_renegotiate(bool pooled) {
    mongoc_uri_t *uri;
...
static void test_topology_scanner_does_not_renegotiate_single(void *ctx) {
    BSON_UNUSED(ctx);

    _test_topology_scanner_does_not_renegotiate(false);
}

static void test_topology_scanner_does_not_renegotiate_pooled(void *ctx) {
    BSON_UNUSED(ctx);

    _test_topology_scanner_does_not_renegotiate(true);
}

void test_topology_scanner_install(TestSuite *suite) {
