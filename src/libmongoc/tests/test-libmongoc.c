...
static log_entry_t *log_entry_create(mongoc_log_level_t level, const char *msg) {
...
static void log_entry_destroy(log_entry_t *log_entry) {
...
bool has_captured_log(mongoc_log_level_t level, const char *msg) {
    size_t i;
    log_entry_t *log_entry;
...
void assert_all_captured_logs_have_prefix(const char *prefix) {
    size_t i;
    log_entry_t *log_entry;
...
char *gen_collection_name(const char *str) {
    return bson_strdup_printf("%s_%u_%u", str, (uint32_t)bson_get_monotonic_time(), (uint32_t)gettestpid());
}

mongoc_collection_t *get_test_collection(mongoc_client_t *client, const char *prefix) {
...
char *test_framework_getenv_required(const char *name) {
...
bool test_framework_setenv(const char *name, const char *value) {
#ifdef _WIN32
    char *envstring;
 ...
 *--------------------------------------------------------------------------
 */
int64_t test_framework_getenv_int64(const char *name, int64_t default_value) {
    char *value = test_framework_getenv(name);
    char *endptr;
    int64_t ret;
...
static char *test_framework_get_unix_domain_socket_path(void) {
...
static char *_uri_str_from_env(void) {
...
static mongoc_uri_t *_uri_from_env(void) {
    char *env_uri_str;
    mongoc_uri_t *uri;
...
char *test_framework_get_host_and_port(void) {
 ...
 *--------------------------------------------------------------------------
 */
static bool test_framework_get_user_password(char **user, char **password) {
 ...
 *--------------------------------------------------------------------------
 */
char *test_framework_add_user_password(const char *uri_str, const char *user, const char *password) {
 ...
 *--------------------------------------------------------------------------
 */
char *test_framework_get_compressors(void) {
 ...
 *--------------------------------------------------------------------------
 */
bool test_framework_has_compressors(void) {
    bool retval;
 ...
 *--------------------------------------------------------------------------
 */
static void call_hello_with_host_and_port(const char *host_and_port, bson_t *reply) {
    char *user;
    char *password;
    char *uri_str;
    mongoc_uri_t *uri;
 ...
 *--------------------------------------------------------------------------
 */
static void call_hello(bson_t *reply) {
...
static char *set_name(bson_t *hello_response) {
    bson_iter_t iter;
...
static bool uri_str_has_db(bson_string_t *uri_string) {
...
static void add_option_to_uri_str(bson_string_t *uri_string, const char *option, const char *value) {
...
mongoc_uri_t *test_framework_get_uri_multi_mongos_loadbalanced(void) {
    char *uri_str_no_auth;
    char *uri_str;
    mongoc_uri_t *uri;
    bson_error_t error;
...
bool test_framework_uri_apply_multi_mongos(mongoc_uri_t *uri, bool use_multi, bson_error_t *error) {
    bool ret = false;
...
char *test_framework_replset_name(void) {
    bson_t reply;
    bson_iter_t iter;
    char *replset_name;
...
size_t test_framework_data_nodes_count(void) {
    bson_t reply;
    bson_iter_t iter, array;
    size_t count = 0;
 ...
 *--------------------------------------------------------------------------
 */
mongoc_client_t *test_framework_new_default_client(void) {
    char *test_uri_str = test_framework_get_uri_str();
 ...
 *--------------------------------------------------------------------------
 */
mongoc_client_t *test_framework_client_new_no_server_api(void) {
    mongoc_uri_t *uri = test_framework_get_uri();
...
mongoc_server_api_t *test_framework_get_default_server_api(void) {
 ...
 *--------------------------------------------------------------------------
 */
mongoc_client_t *test_framework_client_new(const char *uri_str, const mongoc_server_api_t *api) {
 ...
 *--------------------------------------------------------------------------
 */
mongoc_client_t *test_framework_client_new_from_uri(const mongoc_uri_t *uri, const mongoc_server_api_t *api) {
    mongoc_client_t *client = mongoc_client_new_from_uri(uri);
    bson_error_t error;
    mongoc_server_api_t *default_api = NULL;
 ...
 *--------------------------------------------------------------------------
 */
mongoc_client_pool_t *test_framework_new_default_client_pool(void) {
 ...
 *--------------------------------------------------------------------------
 */
mongoc_client_pool_t *test_framework_client_pool_new_from_uri(const mongoc_uri_t *uri, const mongoc_server_api_t *api) {
...
#ifdef MONGOC_ENABLE_SSL
static void test_framework_global_ssl_opts_init(void) {
...
static void test_framework_global_ssl_opts_cleanup(void) {
...
bool test_framework_server_is_secondary(mongoc_client_t *client, uint32_t server_id) {
    bson_t reply;
    bson_iter_t iter;
...
bool test_framework_clustertime_supported(void) {
    bson_t reply;
...
int64_t test_framework_session_timeout_minutes(void) {
    bson_t reply;
    bson_iter_t iter;
    int64_t timeout = -1;
...
void test_framework_get_max_wire_version(int64_t *max_version) {
    bson_t reply;
    bson_iter_t iter;
...
bool test_framework_has_auth(void) {
    char *user;
...
static bool _test_framework_has_crypto(void) {
...
int test_framework_skip_if_no_sessions(void) {
    if (!TestSuite_CheckLive()) {
        return 0;
    }
...
int test_framework_skip_if_no_cluster_time(void) {
    if (!TestSuite_CheckLive()) {
        return 0;
    }
...
int test_framework_skip_if_crypto(void) {
    return _test_framework_has_crypto() ? 0 : 1;
}

int test_framework_skip_if_no_crypto(void) {
...
int test_framework_skip_if_rhel8_zseries(void) {
...
int test_framework_skip_if_no_uds(void) {
#ifdef _WIN32
    return 0;
#else
    char *path;
...
int test_framework_skip_if_no_txns(void) {
...
int64_t test_framework_max_write_batch_size(void) {
    bson_t reply;
    bson_iter_t iter;
    int64_t size;
...
static server_version_t _parse_server_version(const bson_t *buildinfo) {
    bson_iter_t iter;
...
server_version_t test_framework_get_server_version_with_client(mongoc_client_t *client) {
    bson_t reply;
    bson_error_t error;
    server_version_t ret = 0;
...
int test_framework_skip_if_no_mongohouse(void) {
...
int test_framework_skip_if_no_dual_ip_hostname(void) {
...
int test_framework_skip_if_no_compressors(void) {
    char *compressors = test_framework_get_compressors();
...
int test_framework_skip_if_compressors(void) {
    return !test_framework_skip_if_no_compressors();
}

int test_framework_skip_if_no_failpoint(void) {
    mongoc_client_t *client;
    bool ret;
    bson_error_t error;
...
int test_framework_skip_if_no_client_side_encryption(void) {
...
int test_framework_skip_if_no_aws(void) {
...
int test_framework_skip_if_no_setenv(void) {
    char *value;
...
bool test_framework_is_serverless(void) {
    return test_framework_getenv_bool("MONGOC_TEST_IS_SERVERLESS");
}

int test_framework_skip_if_serverless(void) {
...
int test_framework_skip_if_time_sensitive(void) {
...
int test_framework_skip_due_to_cdriver3708(void) {
...
void test_libmongoc_init(TestSuite *suite, BSON_MAYBE_UNUSED const char *name, int argc, char **argv) {
#if defined(_MSC_VER) && defined(_WIN64)
...
void test_libmongoc_destroy(TestSuite *suite) {
 ...
 */
bool test_framework_supports_legacy_opcodes(void) {
    /* Wire v14+ removed legacy opcodes */
    return test_framework_skip_if_max_wire_version_less_than_14() == 0;
}

int test_framework_skip_if_no_legacy_opcodes(void) {
    if (!TestSuite_CheckLive()) {
        return 0;
    }
...
int test_framework_skip_if_no_getlasterror(void) {
    if (!TestSuite_CheckLive()) {
        return 0;
    }
...
bool test_framework_is_loadbalanced(void) {
