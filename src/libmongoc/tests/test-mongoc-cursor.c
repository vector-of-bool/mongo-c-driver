...
static mongoc_cursor_t *_make_cmd_deprecated_cursor(mongoc_collection_t *coll);

static mongoc_cursor_t *_make_array_cursor(mongoc_collection_t *coll);

/* test that the host a cursor returns belongs to a server it connected to. */
static void _test_common_get_host(void *ctx) {
...
static void _test_common_clone(void *ctx) {
    mongoc_client_t *client;
    mongoc_collection_t *coll;
    make_cursor_fn ctor;
    mongoc_cursor_t *cursor;
...
static void _test_common_clone_w_concerns(void *ctx) {
    mongoc_client_t *client;
    mongoc_collection_t *coll;
    make_cursor_fn ctor;
    mongoc_cursor_t *cursor;
...
static void _test_common_advancing_past_end(void *ctx) {
    mongoc_client_t *client;
    mongoc_collection_t *coll;
    make_cursor_fn ctor;
    mongoc_cursor_t *cursor;
...
static void _test_common_server_hint_command_started(const mongoc_apm_command_started_t *event) {
...
static void _test_common_server_hint(void *ctx) {
    mongoc_client_t *client;
    mongoc_collection_t *coll;
    make_cursor_fn ctor;
    mongoc_cursor_t *cursor;
...
static void _test_common_opts(void *ctx) {
    mongoc_client_t *client;
    mongoc_collection_t *coll;
    make_cursor_fn ctor;
    mongoc_cursor_t *cursor;
...
static void _test_common_opts_after_prime(void *ctx) {
    mongoc_client_t *client;
    mongoc_collection_t *coll;
    make_cursor_fn ctor;
    mongoc_cursor_t *cursor;
...
static void _test_common_opts_agg(void *ctx) {
    mongoc_client_t *client;
    mongoc_collection_t *coll;
    make_cursor_fn ctor;
    mongoc_cursor_t *cursor;
...
static mongoc_cursor_t *_make_find_cursor(mongoc_collection_t *coll) {
    return mongoc_collection_find_with_opts(coll, tmp_bson("{}"), NULL, NULL);
}

static mongoc_cursor_t *_make_cmd_cursor(mongoc_collection_t *coll) {
    return mongoc_collection_find_indexes_with_opts(coll, NULL);
}

static mongoc_cursor_t *_make_cmd_cursor_from_agg(mongoc_collection_t *coll) {
...
static void test_common_cursor_functions_install(TestSuite *suite) {
...
static void killcursors_succeeded(const mongoc_apm_command_succeeded_t *event) {
...
extern void _mongoc_cursor_impl_find_opquery_init(mongoc_cursor_t *cursor, bson_t *filter);
 ...
 * cursor ID.
 */
static void test_kill_cursor_live(void) {
    mongoc_apm_callbacks_t *callbacks;
    mongoc_client_t *client;
    mongoc_collection_t *collection;
    bson_t *b;
...
static void _test_kill_cursors(bool pooled) {
    mock_rs_t *rs;
...
static void test_kill_cursors_single(void) {
    _test_kill_cursors(false);
}

static void test_kill_cursors_pooled(void) {
    _test_kill_cursors(true);
}

/* Test explicit mongoc_client_kill_cursor. */
static void _test_client_kill_cursor(bool has_primary) {
    mock_rs_t *rs;
    mongoc_client_t *client;
    mongoc_read_prefs_t *read_prefs;
    bson_error_t error;
...
static void test_client_kill_cursor_with_primary(void) {
    _test_client_kill_cursor(true);
}

static void test_client_kill_cursor_without_primary(void) {
    _test_client_kill_cursor(false);
}

static int count_docs(mongoc_cursor_t *cursor) {
    int n = 0;
    const bson_t *doc;
    bson_error_t error;
...
static void _test_cursor_new_from_command(const char *cmd_json, const char *collection_name) {
    mongoc_client_t *client;
    mongoc_collection_t *collection;
    mongoc_bulk_operation_t *bulk;
    bool r;
...
static void test_cursor_empty_collection(void) {
    mongoc_client_t *client;
    mongoc_collection_t *collection;
    bson_error_t error;
    const bson_t *doc;
...
static void test_cursor_new_from_aggregate(void) {
    _test_cursor_new_from_command("{'aggregate': 'test_cursor_new_from_aggregate',"
                                  " 'pipeline': [], 'cursor': {}}",
                                  "test_cursor_new_from_aggregate");
}

static void test_cursor_new_from_aggregate_no_initial(void) {
...
static void test_cursor_new_from_find(void *ctx) {
    BSON_UNUSED(ctx);

    _test_cursor_new_from_command("{'find': 'test_cursor_new_from_find'}", "test_cursor_new_from_find");
}

static void test_cursor_new_from_find_batches(void *ctx) {
    BSON_UNUSED(ctx);
...
static void test_cursor_new_invalid(void) {
    mongoc_client_t *client;
    bson_error_t error;
    mongoc_cursor_t *cursor;
...
static void test_cursor_new_tailable_await(void) {
...
static void test_cursor_int64_t_maxtimems(void) {
    mock_server_t *server;
    mongoc_client_t *client;
    mongoc_cursor_t *cursor;
    bson_error_t error;
...
static void test_cursor_new_ignores_fields(void) {
    mock_server_t *server;
    mongoc_client_t *client;
    mongoc_cursor_t *cursor;
    const bson_t *doc;
...
static void test_cursor_new_invalid_filter(void) {
    mongoc_client_t *client;
    mongoc_collection_t *collection;
    mongoc_cursor_t *cursor;
    bson_error_t error;
...
static void test_cursor_new_invalid_opts(void) {
    mongoc_client_t *client;
    mongoc_collection_t *collection;
    mongoc_cursor_t *cursor;
    bson_error_t error;
...
static void test_cursor_new_static(void) {
    mongoc_client_t *client;
    bson_error_t error;
    mongoc_cursor_t *cursor;
    bson_t *bson_alloced;
...
static void test_cursor_hint_errors(void) {
    mongoc_client_t *client;
    mongoc_collection_t *collection;
    mongoc_cursor_t *cursor;
...
static void _test_cursor_hint(bool pooled, bool use_primary) {
    mock_rs_t *rs;
    mongoc_client_pool_t *pool = NULL;
    mongoc_client_t *client;
    mongoc_collection_t *collection;
...
static void test_hint_single_secondary(void) {
    _test_cursor_hint(false, false);
}

static void test_hint_single_primary(void) {
    _test_cursor_hint(false, true);
}

static void test_hint_pooled_secondary(void) {
    _test_cursor_hint(true, false);
}

static void test_hint_pooled_primary(void) {
...
static void test_cursor_hint_mongos(void) {
    mock_server_t *server;
    mongoc_client_t *client;
    mongoc_collection_t *collection;
    size_t i;
...
static void test_cursor_hint_mongos_cmd(void) {
    mock_server_t *server;
    mongoc_client_t *client;
    mongoc_collection_t *collection;
    mongoc_cursor_t *cursor;
 ...
 */
static void _test_cursor_hint_no_warmup(bool pooled) {
    mongoc_client_pool_t *pool = NULL;
    mongoc_client_t *client;
    mongoc_collection_t *collection;
...
static void test_hint_no_warmup_single(void) {
    _test_cursor_hint_no_warmup(false);
}

static void test_hint_no_warmup_pooled(void) {
    _test_cursor_hint_no_warmup(true);
}

static void test_tailable_alive(void) {
    mongoc_client_t *client;
    ...
    collection = mongoc_database_create_collection(database,
                                                   collection_name,
...
static void _make_reply_batch(bson_string_t *reply, uint32_t n_docs, bool first_batch, bool finished) {
    uint32_t j;
...
static void _test_cursor_n_return_find_cmd(mongoc_cursor_t *cursor, mock_server_t *server, cursor_n_return_test *test) {
...
static void _test_cursor_n_return(bool find_with_opts) {
...
static void test_n_return_find_cmd(void) {
    _test_cursor_n_return(false);
}

static void test_n_return_find_cmd_with_opts(void) {
...
static void test_empty_final_batch_live(void) {
    mongoc_client_t *client;
    mongoc_collection_t *collection;
    mongoc_cursor_t *cursor;
    bson_error_t error;
...
static void test_empty_final_batch(void) {
    mock_server_t *server;
    mongoc_client_t *client;
    mongoc_collection_t *collection;
    mongoc_cursor_t *cursor;
...
static void test_error_document_query(void) {
    mongoc_client_t *client;
    mongoc_collection_t *collection;
    mongoc_cursor_t *cursor;
    bson_error_t error;
...
static void test_error_document_command(void) {
    mongoc_client_t *client;
    mongoc_cursor_t *cursor;
    bson_error_t error;
    const bson_t *doc;
...
static void test_error_document_getmore(void) {
    mongoc_client_t *client;
    mongoc_collection_t *collection;
    int i;
    bool r;
 ...
 * false */
static void test_find_error_is_alive(void) {
    mongoc_client_t *client;
    mongoc_collection_t *coll;
    mongoc_cursor_t *cursor;
    bson_error_t err;
    ...
    client = test_framework_new_default_client();
    coll = mongoc_client_get_collection(client, "test", "test");
    cursor = mongoc_collection_find(coll,
                                    MONGOC_QUERY_NONE,
                                    0,
                                    0,
                                    0,
...
void test_cursor_install(TestSuite *suite) {
