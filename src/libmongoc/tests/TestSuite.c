...
static void test_msg(const char *format, ...) {
    va_list ap;
...
void _test_error(const char *format, ...) {
    va_list ap;
...
static void TestSuite_SeedRand(TestSuite *suite, /* IN */
                               Test *test)       /* IN */
{
    BSON_UNUSED(suite);
...
void TestSuite_Init(TestSuite *suite, const char *name, int argc, char **argv) {
    const char *filename;
    int i;
...
static int TestSuite_CheckDummy(void) {
    return 1;
}

int TestSuite_CheckMockServerAllowed(void) {
...
static void TestSuite_AddHelper(void *ctx) {
    TestFunc cb = (TestFunc)((TestFnCtx *)ctx)->test_fn;

    cb();
}

void TestSuite_Add(TestSuite *suite, /* IN */
                   const char *name, /* IN */
                   TestFunc func)    /* IN */
{
...
void TestSuite_AddLive(TestSuite *suite, /* IN */
                       const char *name, /* IN */
                       TestFunc func)    /* IN */
{
...
static void _TestSuite_AddCheck(Test *test, CheckFunc check, const char *name) {
...
void _TestSuite_AddMockServerTest(TestSuite *suite, const char *name, TestFunc func, ...) {
    Test *test;
    va_list ap;
...
void TestSuite_AddWC(TestSuite *suite,  /* IN */
                     const char *name,  /* IN */
                     TestFuncWC func,   /* IN */
...
void _TestSuite_AddFull(TestSuite *suite,  /* IN */
                        const char *name,  /* IN */
                        TestFuncWC func,   /* IN */
                        TestFuncDtor dtor, /* IN */
                        void *ctx,
...
#if defined(_WIN32)
static void _print_getlasterror_win(const char *msg) {
    LPTSTR err_msg;
...
static int TestSuite_RunFuncInChild(TestSuite *suite, /* IN */
                                    Test *test)       /* IN */
{
    STARTUPINFO si;
...
static void _append_json_escaped(bson_string_t *buf, const char *s) {
...
static int TestSuite_RunTest(TestSuite *suite, /* IN */
                             Test *test,       /* IN */
                             int *count)       /* INOUT */
{
    int64_t t1, t2, t3;
...
static void TestSuite_PrintHelp(TestSuite *suite) /* IN */
{
...
static void TestSuite_PrintTests(TestSuite *suite) /* IN */
{
    Test *iter;
...
static void TestSuite_PrintJsonSystemHeader(FILE *stream) {
#ifdef _WIN32
...
static const char *egetenv(const char *name) {
    return getenv_or(name, "");
}

static void TestSuite_PrintJsonHeader(TestSuite *suite, /* IN */
                                      FILE *stream)     /* IN */
{
...
static void TestSuite_PrintJsonFooter(FILE *stream) /* IN */
{
    fprintf(stream, "  ]\n}\n");
    fflush(stream);
}

static bool TestSuite_TestMatchesName(const TestSuite *suite, const Test *test, const char *testname) {
    char name[128];
...
void _process_skip_file(const char *filename, mongoc_array_t *skips) {
...
static int TestSuite_RunAll(TestSuite *suite /* IN */) {
    Test *test;
    int count = 0;
    int status = 0;
...
int TestSuite_Run(TestSuite *suite) /* IN */
{
    int failures = 0;
    int64_t start_us;
...
bool TestSuite_NoFork(TestSuite *suite) {
    if (suite->flags & TEST_NOFORK) {
        return true;
    }
    return false;
...
char *bson_value_to_str(const bson_value_t *val) {
    bson_t *tmp = bson_new();
...
bool bson_value_eq(const bson_value_t *a, const bson_value_t *b) {
