...
mongoc_topology_description_type_t topology_type_from_test(const char *type) {
...
mongoc_server_description_type_t server_type_from_test(const char *type) {
...
static mongoc_read_mode_t read_mode_from_test(const char *mode) {
...
static mongoc_ss_optype_t optype_from_test(const char *op) {
    if (strcmp(op, "read") == 0) {
 ...
 *-----------------------------------------------------------------------
 */
const mongoc_server_description_t *server_description_by_hostname(const mongoc_topology_description_t *topology,
                                                                  const char *address) {
 ...
 *-----------------------------------------------------------------------
 */
void process_sdam_test_hello_responses(bson_t *phase, mongoc_topology_t *topology) {
 ...
 *-----------------------------------------------------------------------
 */
void test_server_selection_logic_cb(bson_t *test) {
    bool expected_error;
    bson_error_t error;
 ...
 *-----------------------------------------------------------------------
 */
void assemble_path(const char *parent_path, const char *child_name, char *dst /* OUT */) {
    char *p;
 ...
 *-----------------------------------------------------------------------
 */
int collect_tests_from_dir(char (*paths)[MAX_TEST_NAME_LENGTH] /* OUT */,
                           const char *dir_path,
                           int paths_index,
                           int max_paths) {
#ifdef _MSC_VER
 ...
 *-----------------------------------------------------------------------
 */
bson_t *get_bson_from_json_file(char *filename) {
    FILE *file;
    long length;
    bson_t *data;
    bson_error_t error;
...
static bool check_version_info(const bson_t *scenario, bool print_reason) {
    const char *s;
    char *padded;
...
bool check_scenario_version(const bson_t *scenario) {
...
static int check_test_version(const bson_t *test) {
    const char *s;
    char *padded;
    server_version_t test_version, server_version;
...
static bool check_topology_type(const bson_t *test) {
    bson_iter_t iter;
    bson_iter_t child;
    const char *s;
    bool compatible;
 ...
 */
static void _recreate(const char *db_name, const char *collection_name, const bson_t *scenario) {
...
static void _insert_data(mongoc_collection_t *collection, bson_t *documents) {
    bson_iter_t iter;
...
void insert_data(const char *db_name, const char *collection_name, const bson_t *scenario) {
    mongoc_client_t *client;
    mongoc_database_t *db;
    mongoc_collection_t *collection;
    bson_t documents;
...
void check_outcome_collection(mongoc_collection_t *collection, bson_t *test) {
    bson_t data;
    bson_iter_t iter;
...
static void execute_test(const json_test_config_t *config,
                         mongoc_client_t *client,
                         mongoc_database_t *db,
                         mongoc_collection_t *collection,
                         bson_t *test) {
    ...
    if (bson_has_field(test, "outcome.collection")) {
        ...
        if (bson_has_field(test, "outcome.collection.name")) {
            outcome_coll = mongoc_client_get_collection(outcome_client,
...
void activate_fail_point(mongoc_client_t *client, const uint32_t server_id, const bson_t *test, const char *key) {
    bson_t command;
    bson_error_t error;
    bool r;
 ...
 *-----------------------------------------------------------------------
 */
void deactivate_fail_points(mongoc_client_t *client, uint32_t server_id) {
    mongoc_server_description_t *sd;
    bson_t *command;
    bool r;
    bson_error_t error;
...
void set_uri_opts_from_bson(mongoc_uri_t *uri, const bson_t *opts) {
    bson_iter_t iter;
...
static void set_auto_encryption_opts(mongoc_client_t *client, bson_t *test) {
    bson_t opts;
    bson_iter_t iter;
...
static bool _in_deny_list(const bson_t *test, char **deny_list, uint32_t deny_list_len) {
...
static bool _should_skip_due_to_unsupported_operation(const bson_t *test) {
 ...
 *-----------------------------------------------------------------------
 */
void run_json_general_test(const json_test_config_t *config) {
...
void json_test_config_cleanup(json_test_config_t *config) {
    BSON_UNUSED(config);
    /* no-op */
}
 ...
 * indicating why. */
static bson_t *_skip_if_unsupported(const char *test_name, bson_t *original) {
    int i;
    bool skip = false;
 ...
 *-----------------------------------------------------------------------
 */
void _install_json_test_suite_with_check(TestSuite *suite,
                                         const char *base,
                                         const char *subdir,
                                         test_hook callback,
                                         ...) {
 ...
 *-----------------------------------------------------------------------
 */
void install_json_test_suite(TestSuite *suite, const char *base, const char *subdir, test_hook callback) {
