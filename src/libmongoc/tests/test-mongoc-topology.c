...
static void checks_init(checks_t *checks) {
    memset(checks, 0, sizeof(*checks));
    bson_mutex_init(&checks->mutex);
}

static void checks_cleanup(checks_t *checks) {
    bson_mutex_destroy(&checks->mutex);
}

static bool checks_cmp(checks_t *checks, const char *metric, char cmp, size_t expected) {
    size_t actual = 0;
...
static void check_started(const mongoc_apm_server_heartbeat_started_t *event) {
    checks_t *c;
...
static void check_succeeded(const mongoc_apm_server_heartbeat_succeeded_t *event) {
    checks_t *c;
...
static void check_failed(const mongoc_apm_server_heartbeat_failed_t *event) {
    checks_t *c;
...
static void server_changed_callback(const mongoc_apm_server_changed_t *event) {
    checks_t *c;
...
static mongoc_apm_callbacks_t *heartbeat_callbacks(void) {
...
static void test_topology_client_creation(void) {
    mongoc_uri_t *uri;
    ...
    server_stream = mongoc_cluster_stream_for_reads(&client_a->cluster, NULL, NULL, NULL, &error);
...
static void assert_topology_state(mongoc_topology_t *topology, mongoc_topology_scanner_state_t state) {
    ASSERT(topology);
    ASSERT(topology->scanner_state == state);
}

static void test_topology_thread_start_stop(void) {
...
static void test_topology_client_pool_creation(void) {
    mongoc_client_pool_t *pool;
    mongoc_client_t *client_a;
    mongoc_client_t *client_b;
    mongoc_topology_t *topology_a;
...
static void test_server_selection_try_once_option(void *ctx) {
...
static void _test_server_selection(bool try_once) {
...
static void test_server_selection_try_once(void *ctx) {
    BSON_UNUSED(ctx);

    _test_server_selection(true);
}

static void test_server_selection_try_once_false(void *ctx) {
    BSON_UNUSED(ctx);

    _test_server_selection(false);
}

static void host_list_init(mongoc_host_list_t *host_list, int family, const char *host, uint16_t port) {
...
static void _test_topology_invalidate_server(bool pooled) {
    ...
    /* call explicitly */
    server_stream = mongoc_cluster_stream_for_reads(&client->cluster, NULL, NULL, NULL, &error);
    ASSERT_OR_PRINT(server_stream, error);
    sd = server_stream->sd;
...
static void test_topology_invalidate_server_single(void *ctx) {
    BSON_UNUSED(ctx);

    _test_topology_invalidate_server(false);
}

static void test_topology_invalidate_server_pooled(void *ctx) {
    BSON_UNUSED(ctx);

    _test_topology_invalidate_server(true);
}

static void test_invalid_cluster_node(void *ctx) {
    mongoc_client_pool_t *pool;
    ...
    tdmod = mc_tpld_modify_begin(client->topology);
    mc_tpl_sd_increment_generation(mongoc_topology_description_server_by_id(tdmod.new_td, id, &error), &kZeroServiceId);
    mc_tpld_modify_commit(tdmod);
...
static void test_max_wire_version_race_condition(void *ctx) {
    mongoc_server_description_t *sd;
...
static void test_cooldown_standalone(void) {
    mock_server_t *server;
    mongoc_client_t *client;
    mongoc_read_prefs_t *primary_pref;
    future_t *future;
...
static void test_cooldown_rs(void) {
...
static void test_cooldown_retry(void) {
    mock_server_t *server;
    mongoc_uri_t *uri;
    mongoc_client_t *client;
    mongoc_read_prefs_t *primary_pref;
...
static void _test_select_succeed(bool try_once) {
...
static void test_select_after_timeout(void) {
    _test_select_succeed(false);
}

/* CDRIVER-1219: a secondary is unavailable, scan should try it once,
 * then we select primary */
static void test_select_after_try_once(void) {
    _test_select_succeed(true);
}

static void test_multiple_selection_errors(void *context) {
...
static bool auto_ping(request_t *request, void *data) {
    BSON_UNUSED(data);
 ...
 */
static void _test_server_removed_during_handshake(bool pooled) {
    mock_server_t *server;
    mongoc_uri_t *uri;
    mongoc_client_pool_t *pool = NULL;
    mongoc_client_t *client;
...
static void test_server_removed_during_handshake_single(void) {
    _test_server_removed_during_handshake(false);
}

static void test_server_removed_during_handshake_pooled(void) {
    _test_server_removed_during_handshake(true);
}

static void test_rtt(void *ctx) {
    mock_server_t *server;
    mongoc_client_t *client;
    future_t *future;
    request_t *request;
 ...
 * lock. */
static void test_add_and_scan_failure(void) {
    mock_server_t *server;
    mongoc_uri_t *uri;
    mongoc_client_pool_t *pool;
    mongoc_client_t *client;
...
static future_t *future_command(mongoc_client_t *client, bson_error_t *error) {
    return future_client_command_simple(client, "admin", tmp_bson("{'foo': 1}"), NULL, NULL, error);
}

static void receives_command(mock_server_t *server, future_t *future) {
    request_t *request;
    bson_error_t error;
...
static bool has_known_server(mongoc_client_t *client) {
    mongoc_server_description_t *sd;
    bool r;
...
static void _test_hello_retry_single(bool hangup, size_t n_failures) {
    checks_t checks;
    mongoc_apm_callbacks_t *callbacks;
    mock_server_t *server;
    mongoc_uri_t *uri;
...
static void _test_hello_retry_pooled(bool hangup, size_t n_failures) {
    checks_t checks;
    mongoc_apm_callbacks_t *callbacks;
    mock_server_t *server;
    mongoc_uri_t *uri;
...
static void test_hello_retry_single_hangup(void) {
    _test_hello_retry_single(true, 1u);
}

static void test_hello_retry_single_timeout(void) {
    _test_hello_retry_single(false, 1u);
}

static void test_hello_retry_single_hangup_fail(void) {
    _test_hello_retry_single(true, 2u);
}

static void test_hello_retry_single_timeout_fail(void) {
    _test_hello_retry_single(false, 2u);
}

static void test_hello_retry_pooled_hangup(void) {
    _test_hello_retry_pooled(true, 1u);
}

static void test_hello_retry_pooled_timeout(void) {
    _test_hello_retry_pooled(false, 1u);
}

static void test_hello_retry_pooled_hangup_fail(void) {
    _test_hello_retry_pooled(true, 2u);
}

static void test_hello_retry_pooled_timeout_fail(void) {
    _test_hello_retry_pooled(false, 2u);
}

static void test_incompatible_error(void) {
    mock_server_t *server;
    mongoc_uri_t *uri;
    mongoc_client_t *client;
    bson_error_t error;
    ...
    ASSERT(!mongoc_client_command_simple(client,
                                         "admin",
...
static void test_compatible_null_error_pointer(void) {
    mock_server_t *server;
    mongoc_client_t *client;
...
static char *cluster_time_fmt(int t) {
...
static void test_cluster_time_updated_during_handshake(void) {
    mock_server_t *server;
    mongoc_uri_t *uri;
    mongoc_client_pool_t *pool = NULL;
    mongoc_client_t *client;
 ...
 *   topology as stale.
 */
static void _test_request_scan_on_error(bool pooled,
...
static void test_last_server_removed_warning(void) {
    mock_server_t *server;
    mongoc_client_t *client;
    mongoc_uri_t *uri;
...
static void test_request_scan_on_error(void) {
 ...
 */
static void test_slow_server_pooled(void) {
    mock_server_t *primary;
    mock_server_t *secondary;
    char *hello_common;
    char *hello_primary;
...
static void _test_hello_versioned_api(bool pooled) {
    mock_server_t *server;
    mongoc_uri_t *uri;
    mongoc_client_pool_t *pool;
    mongoc_client_t *client;
...
static void test_hello_versioned_api_single(void) {
    _test_hello_versioned_api(false);
}

static void test_hello_versioned_api_pooled(void) {
    _test_hello_versioned_api(true);
}

static void _test_hello_ok(bool pooled) {
...
static void test_hello_ok_single(void) {
    _test_hello_ok(false);
}

static void test_hello_ok_pooled(void) {
    _test_hello_ok(true);
}

void test_topology_install(TestSuite *suite) {
