...
static bool _can_be_command(const char *query) {
    return (!bson_empty(tmp_bson(query)));
}

static void _test_op_msg(const mongoc_uri_t *uri,
...
static void _test_command(const mongoc_uri_t *uri,
                          mock_server_t *server,
                          const char *command,
                          mongoc_read_prefs_t *read_prefs,
...
static void _test_command_simple(const mongoc_uri_t *uri,
                                 mock_server_t *server,
                                 const char *command,
                                 mongoc_read_prefs_t *read_prefs,
                                 const char *expected_cmd) {
...
static mock_server_t *_run_server(read_pref_test_type_t test_type) {
    mock_server_t *server;
    ...
    case READ_PREF_TEST_SECONDARY:
        mock_server_auto_hello(server,
                               ...
                               mock_server_get_host_and_port(server));
        break;
    default: test_error("Invalid test_type: %d", (int)test_type);
    }

    return server;
}

static mongoc_uri_t *_get_uri(mock_server_t *server, read_pref_test_type_t test_type) {
    mongoc_uri_t *uri;
    ...
    switch (test_type) {
    case READ_PREF_TEST_PRIMARY:
    case READ_PREF_TEST_SECONDARY: mongoc_uri_set_option_as_utf8(uri, "replicaSet", "rs"); break;
    case READ_PREF_TEST_STANDALONE:
    case READ_PREF_TEST_MONGOS:
    default: break;
    }
...
static void _test_read_prefs_op_msg(read_pref_test_type_t test_type,
                                    mongoc_read_prefs_t *read_prefs,
                                    const char *query_or_cmd,
                                    const char *expected_cmd,
                                    const char *expected_find) {
...
static void _test_read_prefs(read_pref_test_type_t test_type,
                             mongoc_read_prefs_t *read_prefs,
                             const char *query_or_cmd,
                             const char *expected_cmd,
                             const char *expected_find) {
...
static void test_read_prefs_standalone_null(void) {
...
static void test_read_prefs_standalone_primary(void) {
    mongoc_read_prefs_t *read_prefs;
...
static void test_read_prefs_standalone_secondary(void) {
    mongoc_read_prefs_t *read_prefs;
...
static void test_read_prefs_standalone_tags(void) {
    bson_t b = BSON_INITIALIZER;
    mongoc_read_prefs_t *read_prefs;
...
static void test_read_prefs_primary_rsprimary(void) {
    mongoc_read_prefs_t *read_prefs;
...
static void test_read_prefs_secondary_rssecondary(void) {
    mongoc_read_prefs_t *read_prefs;
...
/* test that a NULL read pref is the same as PRIMARY */
static void test_read_prefs_mongos_null(void) {
...
static void test_read_prefs_mongos_secondary(void) {
    mongoc_read_prefs_t *read_prefs;
...
static void test_read_prefs_mongos_secondary_preferred(void) {
    mongoc_read_prefs_t *read_prefs;
...
static void test_read_prefs_mongos_tags(void) {
    bson_t b = BSON_INITIALIZER;
    mongoc_read_prefs_t *read_prefs;
...
static void test_read_prefs_mongos_max_staleness(void) {
    mock_server_t *server;
    mongoc_client_t *client;
    mongoc_collection_t *collection;
...
static void test_read_prefs_mongos_hedged_reads(void) {
    mock_server_t *server;
    mongoc_client_t *client;
    mongoc_collection_t *collection;
...
static void test_mongos_read_concern(void) {
    mock_server_t *server;
    mongoc_client_t *client;
    mongoc_collection_t *collection;
    mongoc_read_prefs_t *prefs;
...
static void _test_op_msg_direct_connection(bool is_mongos, test_op_msg_direct_fn_t fn, const char *expected_cmd) {
    mock_server_t *server;
    mongoc_client_t *client;
    mongoc_collection_t *collection;
...
static mongoc_cursor_t *find(mongoc_collection_t *collection, mongoc_read_prefs_t *prefs) {
    return mongoc_collection_find_with_opts(collection, tmp_bson("{}"), NULL /* opts */, prefs);
}

static mongoc_cursor_t *aggregate(mongoc_collection_t *collection, mongoc_read_prefs_t *prefs) {
    return mongoc_collection_aggregate(collection, MONGOC_QUERY_NONE, tmp_bson("{}"), NULL /* opts */, prefs);
}

/* direct connection to a secondary requires read pref primaryPreferred to
 * avoid "not primary" error from server */
static void test_op_msg_direct_secondary(void) {
...
static void test_op_msg_direct_mongos(void) {
...
void test_read_prefs_install(TestSuite *suite) {
