...
static void func_ctx_init(func_ctx_t *ctx,
...
static void func_ctx_cleanup(func_ctx_t *ctx) {
...
#define SET_OPT(_type)                                                                                                 \
    static void set_##_type##_opt(mongoc_##_type##_t *obj, opt_type_t opt_type) {                                      \
        SET_OPT_PREAMBLE(_type);                                                                                       \
                                                                                                                       \
        switch (opt_type) {                                                                                            \
        case OPT_READ_CONCERN: mongoc_##_type##_set_read_concern(obj, rc); break;                                      \
        case OPT_WRITE_CONCERN: mongoc_##_type##_set_write_concern(obj, wc); break;                                    \
        case OPT_READ_PREFS: mongoc_##_type##_set_read_prefs(obj, prefs); break;                                       \
        default: test_error("invalid opt_type: %d", (int)opt_type);                                                    \
        }                                                                                                              \
                                                                                                                       \
        SET_OPT_CLEANUP;                                                                                               \
    }
...
static void set_func_opt(bson_t *opts, mongoc_read_prefs_t **prefs_ptr, opt_type_t opt_type) {
    SET_OPT_PREAMBLE(function);

    switch (opt_type) {
    case OPT_READ_CONCERN: BSON_ASSERT(mongoc_read_concern_append(rc, opts)); break;
    case OPT_WRITE_CONCERN: BSON_ASSERT(mongoc_write_concern_append(wc, opts)); break;
    case OPT_READ_PREFS: *prefs_ptr = mongoc_read_prefs_copy(prefs); break;
    default: test_error("invalid opt_type: %d", (int)opt_type);
    }
...
void add_expected_opt(opt_source_t opt_source, opt_type_t opt_type, bson_t *cmd) {
    ...
    switch (opt_type) {
    case OPT_READ_CONCERN: opt = tmp_bson("{'readConcern': {'level': '%s'}}", source_name); break;
    case OPT_WRITE_CONCERN: opt = tmp_bson("{'writeConcern': {'w': '%s'}}", source_name); break;
    case OPT_READ_PREFS:
...
static const char *opt_type_name(opt_type_t opt_type) {
    switch (opt_type) {
    case OPT_READ_CONCERN: return "readConcern";
    case OPT_WRITE_CONCERN: return "writeConcern";
    case OPT_READ_PREFS: return "readPrefs";
    default: test_error("invalid opt_type: %d", (int)opt_type);
    }
}

static void cleanup_future(future_t *future) {
    future_value_t v;
...
static future_t *client_read_cmd(func_ctx_t *ctx, bson_t *cmd) {
...
static future_t *client_write_cmd(func_ctx_t *ctx, bson_t *cmd) {
...
static future_t *client_read_write_cmd(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "foo", "collection");
    return future_client_read_write_command_with_opts(ctx->client,
                                                      "db",
                                                      tmp_bson("{'foo': 'collection'}"),
                                                      ctx->prefs,
                                                      ctx->opts,
...
static future_t *client_watch(func_ctx_t *ctx, bson_t *cmd) {
...
static future_t *db_drop(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_INT32(cmd, "dropDatabase", 1);
    return future_database_drop_with_opts(ctx->db, ctx->opts, &ctx->error);
}

static future_t *db_read_cmd(func_ctx_t *ctx, bson_t *cmd) {
...
static future_t *db_write_cmd(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "foo", "db");
    return future_database_write_command_with_opts(ctx->db, tmp_bson("{'foo': 'db'}"), ctx->opts, NULL, &ctx->error);
}

static future_t *db_read_write_cmd(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "foo", "db");
    return future_database_read_write_command_with_opts(ctx->db,
                                                        tmp_bson("{'foo': 'db'}"),
                                                        ctx->prefs,
                                                        ctx->opts,
                                                        NULL,
...
static future_t *db_watch(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_INT32(cmd, "aggregate", 1);
...
static future_t *aggregate(func_ctx_t *ctx, bson_t *cmd) {
...
static future_t *aggregate_raw_pipeline(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "aggregate", "collection");
    ctx->cursor = mongoc_collection_aggregate(ctx->collection,
                                              MONGOC_QUERY_NONE,
                                              tmp_bson("[{'$out': 'foo'}]"),
...
static future_t *collection_drop(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "drop", "collection");
    return future_collection_drop_with_opts(ctx->collection, ctx->opts, &ctx->error);
}

static future_t *collection_read_cmd(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "foo", "collection");
    return future_collection_read_command_with_opts(ctx->collection,
                                                    tmp_bson("{'foo': 'collection'}"),
                                                    ctx->prefs,
                                                    ctx->opts,
                                                    NULL,
...
static future_t *collection_read_write_cmd(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "foo", "collection");
    return future_collection_read_write_command_with_opts(ctx->collection,
                                                          tmp_bson("{'foo': 'collection'}"),
                                                          NULL,
                                                          ctx->opts,
                                                          NULL,
...
static future_t *collection_write_cmd(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "foo", "collection");
    return future_collection_write_command_with_opts(ctx->collection,
                                                     tmp_bson("{'foo': 'collection'}"),
                                                     ctx->opts,
                                                     NULL,
                                                     &ctx->error);
...
static future_t *collection_watch(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "aggregate", "collection");
    return future_collection_watch(ctx->collection, tmp_bson("{}"), ctx->opts);
}

static future_t *count(func_ctx_t *ctx, bson_t *cmd) {
...
static future_t *count_documents(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "aggregate", "collection");
    return future_collection_count_documents(ctx->collection, tmp_bson("{}"), ctx->opts, ctx->prefs, NULL, &ctx->error);
}

static future_t *create_index(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "createIndexes", "collection");
    return future_collection_create_index_with_opts(ctx->collection,
                                                    tmp_bson("{}"),
                                                    NULL,
                                                    ctx->opts,
                                                    NULL,
...
static future_t *drop_index(func_ctx_t *ctx, bson_t *cmd) {
...
static future_t *estimated_document_count(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "count", "collection");
    return future_collection_estimated_document_count(ctx->collection, ctx->opts, ctx->prefs, NULL, &ctx->error);
}

static future_t *find(func_ctx_t *ctx, bson_t *cmd) {
...
static void find_and_modify_cleanup(void *data) {
    mongoc_find_and_modify_opts_destroy((mongoc_find_and_modify_opts_t *)data);
}

static future_t *find_and_modify(func_ctx_t *ctx, bson_t *cmd) {
...
static future_t *delete_many(func_ctx_t *ctx, bson_t *cmd) {
...
static future_t *delete_one(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "delete", "collection");
    BSON_ASSERT(!ctx->prefs);
    return future_collection_delete_one(ctx->collection, tmp_bson("{}"), ctx->opts, NULL, &ctx->error);
}

static future_t *insert_many(func_ctx_t *ctx, bson_t *cmd) {
...
static future_t *insert_one(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "insert", "collection");
    BSON_ASSERT(!ctx->prefs);
    return future_collection_insert_one(ctx->collection, tmp_bson("{}"), ctx->opts, NULL, &ctx->error);
}

static future_t *replace_one(func_ctx_t *ctx, bson_t *cmd) {
...
static future_t *update_many(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "update", "collection");
    BSON_ASSERT(!ctx->prefs);
    return future_collection_update_many(ctx->collection, tmp_bson("{}"), tmp_bson("{}"), ctx->opts, NULL, &ctx->error);
}

static future_t *update_one(func_ctx_t *ctx, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "update", "collection");
    BSON_ASSERT(!ctx->prefs);
...
static void bulk_operation_cleanup(void *data) {
    mongoc_bulk_operation_destroy((mongoc_bulk_operation_t *)data);
}

static future_t *bulk_exec(func_ctx_t *ctx, bson_t *cmd) {
...
static bool bulk_insert(mongoc_bulk_operation_t *bulk, bson_error_t *error, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "insert", "collection");
    return mongoc_bulk_operation_insert_with_opts(bulk, tmp_bson("{}"), NULL, error);
}

static bool bulk_remove_many(mongoc_bulk_operation_t *bulk, bson_error_t *error, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "delete", "collection");
    return mongoc_bulk_operation_remove_many_with_opts(bulk, tmp_bson("{}"), NULL, error);
}

static bool bulk_remove_one(mongoc_bulk_operation_t *bulk, bson_error_t *error, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "delete", "collection");
    return mongoc_bulk_operation_remove_one_with_opts(bulk, tmp_bson("{}"), NULL, error);
}

static bool bulk_replace_one(mongoc_bulk_operation_t *bulk, bson_error_t *error, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "update", "collection");
    return mongoc_bulk_operation_replace_one_with_opts(bulk, tmp_bson("{}"), tmp_bson("{}"), NULL, error);
}

static bool bulk_update_many(mongoc_bulk_operation_t *bulk, bson_error_t *error, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "update", "collection");
    return mongoc_bulk_operation_update_many_with_opts(bulk, tmp_bson("{}"), tmp_bson("{}"), NULL, error);
}

static bool bulk_update_one(mongoc_bulk_operation_t *bulk, bson_error_t *error, bson_t *cmd) {
    BSON_APPEND_UTF8(cmd, "update", "collection");
    return mongoc_bulk_operation_update_one_with_opts(bulk, tmp_bson("{}"), tmp_bson("{}"), NULL, error);
}

static void test_func_inherits_opts(void *ctx) {
...
static void install_inheritance_tests(TestSuite *suite, opt_inheritance_test_t *tests, size_t n) {
    size_t i;
    opt_inheritance_test_t *test;
    char *name;
...
void test_opts_install(TestSuite *suite) {
