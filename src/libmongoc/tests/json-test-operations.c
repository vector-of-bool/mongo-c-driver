...
mongoc_client_session_t *session_from_name(json_test_ctx_t *ctx, const char *session_name) {
...
static json_test_worker_thread_t *thread_from_name(json_test_ctx_t *ctx, const char *thread_name) {
...
json_test_worker_thread_t *worker_thread_new(json_test_ctx_t *ctx) {
...
void worker_thread_destroy(json_test_worker_thread_t *wt) {
    q_destroy(wt->queue);
...
static void start_thread(json_test_worker_thread_t *wt) {
    wt->shutdown_requested = false;
    mcommon_thread_create(&wt->thread, json_test_worker_thread_run, wt);
}

static void run_on_thread(json_test_worker_thread_t *wt, bson_t *op) {
    bson_mutex_lock(&wt->mutex);
...
static void wait_for_thread(json_test_worker_thread_t *wt) {
    bson_mutex_lock(&wt->mutex);
...
void json_test_ctx_init(json_test_ctx_t *ctx,
                        const bson_t *test,
...
void json_test_ctx_end_sessions(json_test_ctx_t *ctx) {
    int i;
...
static void append_session(mongoc_client_session_t *session, bson_t *opts) {
    if (session) {
        bool r;
        bson_error_t error;
...
static char *value_to_str(const bson_value_t *value) {
    bson_t doc;
...
static void convert_bulk_write_result(const bson_t *doc, bson_t *r) {
    bson_iter_t iter;
...
static void convert_spec_result(const char *op_name, const bson_value_t *spec_result, bson_value_t *converted) {
    bson_t doc;
    bson_t r;
    bson_iter_t iter;
...
static bool get_result(const bson_t *test, const bson_t *operation, bson_value_t *value) {
    const char *op_name;
...
static void check_success_expected(const bson_t *operation, bool succeeded, bool expected, const bson_error_t *error) {
...
static uint32_t error_code_from_name(const char *name) {
...
static void check_error_code_name(const bson_t *operation, const bson_error_t *error) {
...
static void check_error_contains(const bson_t *operation, const bson_error_t *error) {
    const char *msg;
...
static void check_error_labels_contain(const bson_t *operation, const bson_value_t *result) {
    bson_t reply;
...
static void check_error_labels_omit(const bson_t *operation, const bson_value_t *result) {
    bson_t reply;
...
void check_result(const bson_t *test,
...
static bool add_request_to_bulk(mongoc_bulk_operation_t *bulk, const bson_t *request, bson_error_t *error) {
    const char *name;
    bson_t args;
    bool r;
...
static bool bulk_write(mongoc_collection_t *collection,
                       const bson_t *test,
                       const bson_t *operation,
...
static bool single_write(mongoc_collection_t *collection,
                         const bson_t *test,
                         const bson_t *operation,
                         mongoc_client_session_t *session,
                         mongoc_write_concern_t *wc,
...
static mongoc_find_and_modify_opts_t *create_find_and_modify_opts(const char *name,
                                                                  const bson_t *args,
                                                                  mongoc_client_session_t *session,
                                                                  mongoc_write_concern_t *wc) {
...
static bool find_and_modify(mongoc_collection_t *collection,
                            const bson_t *test,
                            const bson_t *operation,
                            mongoc_client_session_t *session,
                            mongoc_write_concern_t *wc,
...
static bool insert_many(mongoc_collection_t *collection,
                        const bson_t *test,
                        const bson_t *operation,
                        mongoc_client_session_t *session,
                        mongoc_write_concern_t *wc,
...
static bool rename_op(mongoc_collection_t *collection,
                      const bson_t *test,
                      const bson_t *operation,
                      mongoc_client_session_t *session,
                      mongoc_write_concern_t *wc,
...
static bool drop(mongoc_collection_t *collection,
                 const bson_t *test,
                 const bson_t *operation,
                 mongoc_client_session_t *session,
                 mongoc_write_concern_t *wc,
...
static bool count(mongoc_collection_t *collection,
                  const bson_t *test,
                  const bson_t *operation,
                  mongoc_client_session_t *session,
...
static bool distinct(mongoc_collection_t *collection,
                     const bson_t *test,
                     const bson_t *operation,
                     mongoc_client_session_t *session,
                     const mongoc_read_prefs_t *read_prefs,
...
static void check_cursor(mongoc_cursor_t *cursor, const bson_t *test, const bson_t *operation) {
    const bson_t *doc;
    bson_error_t error;
...
static bool find(mongoc_collection_t *collection,
                 const bson_t *test,
                 const bson_t *operation,
                 mongoc_client_session_t *session,
                 const mongoc_read_prefs_t *read_prefs,
...
static bool find_one(mongoc_collection_t *collection,
                     const bson_t *test,
                     const bson_t *operation,
                     mongoc_client_session_t *session,
                     const mongoc_read_prefs_t *read_prefs,
...
static bool _is_aggregate_out(const bson_t *pipeline) {
    bson_iter_t iter;
    bson_iter_t stage;
...
static bool aggregate(mongoc_collection_t *collection,
                      const bson_t *test,
                      const bson_t *operation,
                      mongoc_client_session_t *session,
                      const mongoc_read_prefs_t *read_prefs,
...
static bool db_aggregate(mongoc_database_t *db,
                         const bson_t *test,
                         const bson_t *operation,
                         mongoc_client_session_t *session,
                         const mongoc_read_prefs_t *read_prefs,
...
static bool command(mongoc_database_t *db,
                    const bson_t *test,
                    const bson_t *operation,
                    mongoc_client_session_t *session,
                    const mongoc_read_prefs_t *read_prefs,
...
static bool list_databases(mongoc_client_t *client,
                           const bson_t *test,
                           const bson_t *operation,
                           mongoc_client_session_t *session,
                           bson_t *reply) {
...
static bool list_database_names(mongoc_client_t *client,
                                const bson_t *test,
                                const bson_t *operation,
                                mongoc_client_session_t *session,
                                bson_t *reply) {
...
static bool list_indexes(mongoc_collection_t *collection,
                         const bson_t *test,
                         const bson_t *operation,
                         mongoc_client_session_t *session,
                         bson_t *reply) {
...
static bool list_collections(mongoc_database_t *db,
                             const bson_t *test,
                             const bson_t *operation,
                             mongoc_client_session_t *session,
                             bson_t *reply) {
...
static bool list_collection_names(mongoc_database_t *db,
                                  const bson_t *test,
                                  const bson_t *operation,
                                  mongoc_client_session_t *session,
                                  bson_t *reply) {
...
static bool gridfs_download(mongoc_database_t *db,
                            const bson_t *test,
                            const bson_t *operation,
                            mongoc_client_session_t *session,
                            const mongoc_read_prefs_t *read_prefs,
...
static bool create_collection(mongoc_database_t *db,
                              const bson_t *test,
                              const bson_t *operation,
                              mongoc_client_session_t *session,
                              bson_t *reply) {
...
static bool drop_collection(mongoc_database_t *db,
                            const bson_t *test,
                            const bson_t *operation,
                            mongoc_client_session_t *session,
                            bson_t *reply) {
...
static bool create_index(mongoc_database_t *db,
                         mongoc_collection_t *collection,
                         const bson_t *test,
                         const bson_t *operation,
                         mongoc_client_session_t *session,
...
static bool collection_exists(mongoc_client_t *client, const bson_t *operation) {
    char **names;
    bson_t args;
...
static bool index_exists(mongoc_client_t *client, const bson_t *operation) {
    bson_t args;
    const char *database_name;
    const char *collection_name;
...
static uint32_t _get_total_pool_cleared_event(json_test_ctx_t *ctx) {
    uint32_t total = 0;
...
static void wait_for_event(json_test_ctx_t *ctx, const bson_t *operation) {
...
static void wait_for_primary_change(json_test_ctx_t *ctx, const bson_t *operation) {
    bool satisfied = false;
    int64_t measured = 0;
    int64_t total = 0;
...
static void assert_event_count(json_test_ctx_t *ctx, const bson_t *operation) {
    int64_t total = 0;
...
static void assert_session_transaction_state(json_test_ctx_t *ctx, const bson_t *operation) {
...
static bool op_error(const bson_t *operation) {
    return bson_has_field(operation, "error") && bson_lookup_bool(operation, "error");
}

bool json_test_operation(json_test_ctx_t *ctx,
                         const bson_t *test,
                         const bson_t *operation,
                         mongoc_collection_t *collection,
                         mongoc_client_session_t *session,
    ...
    if (!strcmp(obj_name, "collection")) {
        if (!strcmp(op_name, "bulkWrite")) {
            res = bulk_write(c, test, operation, session, wc, reply);
                   ...
                   || !strcmp(op_name, "replaceOne") || !strcmp(op_name, "updateOne")
...
static void one_operation(json_test_ctx_t *ctx, const bson_t *test, const bson_t *operation) {
    const char *op_name;
...
void json_test_operations(json_test_ctx_t *ctx, const bson_t *test) {
    bson_t operations;
    bson_t operation;
    bson_iter_t iter;
