...
static int64_t get_max_staleness(const mongoc_client_t *client) {
...
static void test_mongoc_client_max_staleness(void) {
    ...
    capture_logs(true);
    ASSERT(!test_framework_client_new("mongodb://a/?" MONGOC_URI_READPREFERENCE
    ...
    client = test_framework_client_new("mongodb://a/?" MONGOC_URI_MAXSTALENESSSECONDS "=120&" MONGOC_URI_READPREFERENCE
                                       "=secondary",
                                       NULL);
...
static void test_mongos_max_staleness_read_pref(void) {
    ...
    request = mock_server_receives_msg(server,
                                       MONGOC_MSG_NONE,
                                       tmp_bson("{'$db': 'db', '$readPreference': {'mode': 'secondaryPreferred'}}"));
...
static void _test_last_write_date(bool pooled) {
    mongoc_uri_t *uri;
...
static void test_last_write_date(void *ctx) {
    BSON_UNUSED(ctx);

    _test_last_write_date(false);
}

static void test_last_write_date_pooled(void *ctx) {
    BSON_UNUSED(ctx);
...
static void _test_last_write_date_absent(bool pooled) {
    mongoc_client_pool_t *pool = NULL;
    mongoc_client_t *client;
    bson_error_t error;
...
static void test_last_write_date_absent(void *ctx) {
    BSON_UNUSED(ctx);

    _test_last_write_date_absent(false);
}

static void test_last_write_date_absent_pooled(void *ctx) {
    BSON_UNUSED(ctx);
...
void test_client_max_staleness_install(TestSuite *suite) {
