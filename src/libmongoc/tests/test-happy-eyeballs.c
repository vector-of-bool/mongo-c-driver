 ...
 * response. */
static int _override_poll_response(he_testcase_server_t *server, mongoc_stream_poll_t *poller) {
 ...
 * exists). */
static he_testcase_server_t *_server_for_client(mongoc_stream_t *stream) {
    int i;
...
static ssize_t _mock_poll(mongoc_stream_poll_t *streams, size_t nstreams, int32_t timeout) {
...
static mongoc_stream_t *_mock_initiator(mongoc_async_cmd_t *acmd) {
...
static void _test_scanner_callback(uint32_t id,
                                   const bson_t *bson,
                                   int64_t rtt_msec,
                                   void *data,
...
static void _init_host(mongoc_host_list_t *host, uint16_t port, const char *type) {
...
static void _testcase_setup(he_testcase_t *testcase) {
...
static void _testcase_teardown(he_testcase_t *testcase) {
    mock_server_destroy(testcase->state.mock_server);
    mongoc_topology_scanner_destroy(testcase->state.ts);
}

static void _check_stream(mongoc_stream_t *stream, const char *expected, char *message) {
...
static void _testcase_run(he_testcase_t *testcase) {
...
static void _run_testcase(void *ctx) {
...
static void test_happy_eyeballs_dns_cache(void) {
#define E 1000
...
void test_happy_eyeballs_retirement(void) {
    /* test connecting to a retired node that fails to initiate a connection. */
}

void test_happy_eyeballs_install(TestSuite *suite) {
