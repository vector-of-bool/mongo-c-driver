...
static uint8_t *get_test_file(const char *filename, size_t *length) {
    ssize_t len;
    uint8_t *buf;
 ...
 */
static void assert_rpc_equal(const char *filename, mongoc_rpc_t *rpc) {
    mongoc_array_t ar;
    size_t length;
    size_t off = 0u;
...
static void test_mongoc_rpc_delete_gather(void) {
    mongoc_rpc_t rpc;
    bson_t sel;
...
static void test_mongoc_rpc_delete_scatter(void) {
    uint8_t *data;
    mongoc_rpc_t rpc;
    bool r;
    bson_t sel;
...
static void test_mongoc_rpc_get_more_gather(void) {
    mongoc_rpc_t rpc;
...
static void test_mongoc_rpc_get_more_scatter(void) {
    uint8_t *data;
    mongoc_rpc_t rpc;
    bool r;
    size_t length;
...
static void test_mongoc_rpc_insert_gather(void) {
    mongoc_rpc_t rpc;
...
static void test_mongoc_rpc_insert_scatter(void) {
...
static void test_mongoc_rpc_kill_cursors_gather(void) {
    mongoc_rpc_t rpc;
...
static void test_mongoc_rpc_kill_cursors_scatter(void) {
    uint8_t *data;
...
static void test_mongoc_rpc_query_gather(void) {
    mongoc_rpc_t rpc;
    bson_t b;
...
static void test_mongoc_rpc_query_scatter(void) {
    uint8_t *data;
    mongoc_rpc_t rpc;
    bool r;
    bson_t empty;
...
static void test_mongoc_rpc_reply_gather(void) {
...
static void test_mongoc_rpc_reply_scatter(void) {
    bson_reader_t *reader;
    uint8_t *data;
    mongoc_rpc_t rpc;
    const bson_t *b;
...
static void test_mongoc_rpc_reply_scatter2(void) {
    bson_reader_t *reader;
    uint8_t *data;
    mongoc_rpc_t rpc;
    bool r;
...
static void test_mongoc_rpc_update_gather(void) {
    mongoc_rpc_t rpc;
    bson_t sel;
    bson_t up;
...
static void test_mongoc_rpc_update_scatter(void) {
    uint8_t *data;
    mongoc_rpc_t rpc;
    bool r;
    bson_t b;
...
static void test_mongoc_rpc_buffer_iov(void) {
    mongoc_array_t ar;
    mongoc_rpc_t rpc;
    bson_t b;
    size_t allocate;
...

    {
        ...
        const size_t size = _mongoc_cluster_buffer_iovec((mongoc_iovec_t *)ar.data, ar.len, 0, full_opcode);
...
static void test_mongoc_rpc_msg_checksum_gather(mongoc_rpc_t *rpc) {
    ...
    for (size_t i = 0u; i < array.len; ++i) {
        ...
        const size_t actual = _mongoc_array_index(&array, mongoc_iovec_t, i).iov_len;

        ASSERT_WITH_MSG(expected == actual, "expected element %zu to have iov_len %zu, got %zu", i, expected, actual);
    }

    _mongoc_array_destroy(&array);
}

static void test_mongoc_rpc_msg_checksum(void) {
    // OP_MSG scatter should be able to handle absence of checksum.
    {
      ...
      unsigned char input[] = {
        ...
        ASSERT_WITH_MSG(_mongoc_rpc_scatter(&rpc, input, sizeof(input)), "failed to parse OP_MSG without checksum");
        _mongoc_rpc_swab_from_le(&rpc);
        ...
        ASSERT_CMPUINT32((uint32_t)rpc.msg.flags, ==, (uint32_t)MONGOC_MSG_NONE);
    ...
    // OP_MSG scatter should be able to handle presence of checksum.
    {
      ...
      unsigned char input[] = {
        ...
        mongoc_rpc_t rpc = {._init = 0};
        ASSERT_WITH_MSG(_mongoc_rpc_scatter(&rpc, input, sizeof(input)), "failed to parse OP_MSG with checksum");
        _mongoc_rpc_swab_from_le(&rpc);
        ...
        ASSERT_CMPINT32(rpc.msg.response_to, ==, 0);
        ASSERT_CMPINT32(rpc.msg.opcode, ==, MONGOC_OPCODE_MSG);
        ASSERT_CMPUINT32((uint32_t)rpc.msg.flags, ==, (uint32_t)MONGOC_MSG_CHECKSUM_PRESENT);
        ASSERT_CMPINT32(rpc.msg.n_sections, ==, 1);
        ASSERT_CMPINT(rpc.msg.sections->payload_type, ==, 0);
        {
            bson_t doc;
...
void test_rpc_install(TestSuite *suite) {
