...
static bool is_test_file_skipped(test_file_t *test_file) {
...
static bool is_test_skipped(test_t *test) {
    skipped_unified_test_t *skip;
...
failpoint_t *failpoint_new(char *name, char *client_id, uint32_t server_id) {
...
void failpoint_destroy(failpoint_t *fp) {
    if (!fp) {
        return;
    }
    bson_free(fp->name);
...
void register_failpoint(test_t *test, char *name, char *client_id, uint32_t server_id) {
    failpoint_t *fp = NULL;
...
static bool cleanup_failpoints(test_t *test, bson_error_t *error) {
    bool ret = false;
...
static bool test_has_operation(test_t *test, char *op_name) {
    bson_iter_t iter;
...
static const char *get_topology_type(mongoc_client_t *client);

static bool is_topology_type_sharded(const char *topology_type) {
    return 0 == strcmp("sharded", topology_type) || 0 == strcmp("sharded-replicaset", topology_type);
}

static bool is_topology_type_compatible(const char *test_topology_type, const char *server_topology_type) {
 ...
 * server.
 */
static void on_topology_changed(const mongoc_apm_topology_changed_t *event) {
 ...
 */
static void test_runner_get_all_server_ids(test_runner_t *test_runner, mongoc_array_t *out) {
    bson_error_t error;
    bool ret;
 ...
 */
static bool test_runner_terminate_open_transactions(test_runner_t *test_runner, bson_error_t *error) {
...
static test_runner_t *test_runner_new(void) {
    bson_error_t error;
...
static void test_runner_destroy(test_runner_t *test_runner) {
...
static test_file_t *test_file_new(test_runner_t *test_runner, bson_t *bson) {
...
static bool is_replset(bson_t *hello_reply) {
...
static bool is_sharded(bson_t *hello_reply) {
    const char *val;
...
static void check_schema_version(test_file_t *test_file) {
...
static bool check_run_on_requirement(test_runner_t *test_runner,
    ...
    BSON_FOREACH(run_on_requirement, req_iter) {
        ...
        if (0 == strcmp(key, "topologies")) {
            ...
            if (!found) {
                *fail_reason = bson_strdup_printf("Topology (%s) was not found among listed topologies: %s",
...
static bool check_run_on_requirements(test_runner_t *test_runner, bson_t *run_on_requirements, const char **reason) {
...
static bool test_setup_initial_data(test_t *test, bson_error_t *error) {
    test_runner_t *test_runner = NULL;
    test_file_t *test_file = NULL;
...
static bool test_create_entities(test_t *test, bson_error_t *error) {
    test_file_t *test_file = NULL;
    bson_iter_t iter;
...
static bool test_run_operations(test_t *test, bson_error_t *error) {
    bool ret = false;
    bson_iter_t iter;
...
static bool test_check_event(test_t *test, bson_t *expected, event_t *actual, bson_error_t *error) {
    bool ret = false;
    bson_iter_t iter;
...
static bool test_check_expected_events(test_t *test, bson_error_t *error) {
    bool ret = false;
    bson_iter_t iter;
...
static bool test_check_outcome_collection(test_t *test, bson_t *collection_data, bson_error_t *error) {
    bool ret = false;
    bson_parser_t *bp = NULL;
    char *database_name = NULL;
    char *collection_name = NULL;
...
static bool test_check_outcome(test_t *test, bson_error_t *error) {
    bool ret = false;
    bson_iter_t iter;
...
static void append_size_t(bson_t *doc, const char *key, size_t value) {
    BSON_ASSERT(bson_in_range_unsigned(int64_t, value));
    BSON_ASSERT(BSON_APPEND_INT64(doc, key, (int64_t)value));
}

static void append_bson_array(bson_t *doc, const char *key, const mongoc_array_t *array) {
    ...
    if (!array) {
        ...
        for (bson_t **iter = begin; iter != end; ++iter) {
            BSON_ASSERT(bson_uint32_to_string(index++, &index_key, buffer, sizeof(buffer)));
...
static bool test_generate_atlas_results(test_t *test, bson_error_t *error) {
...
static bool run_distinct_on_each_mongos(test_t *test, char *db_name, char *coll_name, bson_error_t *error) {
    bool ret = false;
    bson_t *cmd = NULL;
...
static bool test_run_distinct_workaround(test_t *test, bson_error_t *error) {
    bool ret = false;
    bson_iter_t iter;
    bson_parser_t *bp = NULL;
    mongoc_collection_t *coll = NULL;
 ...
 * disable failpoints).
 */
bool test_run(test_t *test, bson_error_t *error) {
    bool ret = false;
    test_runner_t *test_runner = NULL;
    test_file_t *test_file = NULL;
