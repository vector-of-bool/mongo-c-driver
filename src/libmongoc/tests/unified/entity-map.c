...
entity_client_new (entity_map_t *em, bson_t *bson, bson_error_t *error)
{
   ...
   bool observe_sensitive_commands = false;
   ...
   bsonParse ( //
      *bson,
      ...
      else(error ("An client 'id' string is required")),
      ...
      find (
         ...
         visitEach (cond (
            ...
            (true,
             do(test_error ("Unknown event type '%s'",
                            bson_iter_utf8 (&bsonVisitIter, NULL))))))),
      ...
      find (
         ...
         parse ( // The "version" string is required first:
            find (keyWithType ("version", utf8), do({
                     ...
                     if (!mongoc_server_api_version_from_string (
                            bson_iter_utf8 (&bsonVisitIter, NULL), &ver)) {
      ...
      find (key ("observeSensitiveCommands"),
            ...
            storeBool (observe_sensitive_commands)),
