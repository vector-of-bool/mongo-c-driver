...
static void entity_destroy(entity_t *entity);

entity_map_t *entity_map_new(void) {
    return bson_malloc0(sizeof(entity_map_t));
}

void entity_map_destroy(entity_map_t *entity_map) {
    entity_t *entity, *tmp;
...
static bool uri_apply_options(mongoc_uri_t *uri, bson_t *opts, bson_error_t *error) {
    bson_iter_t iter;
    bool ret = false;
    bool wcSet = false;
...
event_t *event_new(const char *type) {
    event_t *event = NULL;
...
static entity_t *entity_new(entity_map_t *em, const char *type) {
...
static bool is_sensitive_command(event_t *event) {
...
bool should_ignore_event(entity_t *client_entity, event_t *event) {
    bson_iter_t iter;
...
static void observe_event(entity_t *entity, command_callback_funcs_t funcs, const char *type, const void *apm_command) {
...
static void store_event_serialize_started(bson_t *doc, const mongoc_apm_command_started_t *apm_command) {
...
static void store_event_serialize_failed(bson_t *doc, const mongoc_apm_command_failed_t *apm_command) {
    ...
    BSON_APPEND_INT64(doc, "serverConnectionId", mongoc_apm_command_failed_get_server_connection_id_int64(apm_command));
...
static void store_event_serialize_succeeded(bson_t *doc, const mongoc_apm_command_succeeded_t *apm_command) {
    ...
    BSON_APPEND_INT64(doc,
                      "serverConnectionId",
                      mongoc_apm_command_succeeded_get_server_connection_id_int64(apm_command));
...
static void apm_command_callback(command_callback_funcs_t funcs, const char *type, const void *apm_command) {
    BSON_ASSERT_PARAM(type);
    BSON_ASSERT_PARAM(apm_command);
...
static void command_started(const mongoc_apm_command_started_t *started) {
...
static void command_failed(const mongoc_apm_command_failed_t *failed) {
    command_callback_funcs_t funcs = {
...
static void command_succeeded(const mongoc_apm_command_succeeded_t *succeeded) {
    command_callback_funcs_t funcs = {
...
static void set_command_callback(mongoc_apm_callbacks_t *callbacks, const char *type) {
...
static void add_observe_event(entity_t *entity, const char *type) {
    observe_event_t event = {.type = type};

    _mongoc_array_append_val(&entity->observe_events, event);
}

static void add_store_event(entity_t *entity, const char *type, const char *entity_id) {
    store_event_t event = {.type = type, .entity_id = entity_id};

    _mongoc_array_append_val(&entity->store_events, event);
}

entity_t *entity_client_new(entity_map_t *em, bson_t *bson, bson_error_t *error) {
    entity_t *entity = NULL;
...
static char *_entity_client_encryption_getenv(const char *name, bson_error_t *error) {
    char *res = NULL;
...
static bool _append_kms_provider_value_or_getenv(bson_t *bson,
                                                 const char *key,
                                                 const char *value,
                                                 const char *env_name,
                                                 bson_error_t *error) {
...
static bool _validate_string_or_placeholder(const bson_iter_t *iter, bson_error_t *error) {
...
static bool _parse_kms_provider_aws(bson_t *kms_providers,
                                    bson_t *tls_opts,
                                    const char *provider,
                                    bson_t *kms_doc,
                                    bson_error_t *error) {
    ...
    BSON_FOREACH(kms_doc, iter) {
        ...
        if (strcmp(key, "endpoint") == 0) {
            ...
            /* Configure tlsOptions to enable KMIP TLS connections. */
            {
                ...
                if (!_append_kms_provider_value_or_getenv(&tls_child,
                                                          "tlsCAFile",
                                                          NULL,
...
entity_t *entity_client_encryption_new(entity_map_t *entity_map, bson_t *bson, bson_error_t *error) {
...
static coll_or_db_opts_t *coll_or_db_opts_new(void) {
    return bson_malloc0(sizeof(coll_or_db_opts_t));
}

static void coll_or_db_opts_destroy(coll_or_db_opts_t *opts) {
    if (!opts) {
        return;
    }
...
static bool coll_or_db_opts_parse(coll_or_db_opts_t *opts, bson_t *in, bson_error_t *error) {
...
entity_t *entity_database_new(entity_map_t *entity_map, bson_t *bson, bson_error_t *error) {
    bson_parser_t *parser = NULL;
    entity_t *entity = NULL;
...
entity_t *entity_collection_new(entity_map_t *entity_map, bson_t *bson, bson_error_t *error) {
    bson_parser_t *parser = NULL;
    entity_t *entity = NULL;
...
mongoc_session_opt_t *session_opts_new(bson_t *bson, bson_error_t *error) {
    bool ret = false;
...
entity_t *entity_session_new(entity_map_t *entity_map, bson_t *bson, bson_error_t *error) {
    bson_parser_t *parser = NULL;
    entity_t *entity = NULL;
...
entity_t *entity_bucket_new(entity_map_t *entity_map, bson_t *bson, bson_error_t *error) {
    bson_parser_t *parser = NULL;
    entity_t *entity = NULL;
    mongoc_database_t *database = NULL;
    char *database_id = NULL;
 ...
 * object immediately.
 */
bool entity_map_create(entity_map_t *entity_map, bson_t *bson, bson_error_t *error) {
    bson_iter_t iter;
...
entity_t *entity_map_get(entity_map_t *entity_map, const char *id, bson_error_t *error) {
    entity_t *entity = NULL;
...
bool entity_map_delete(entity_map_t *em, const char *id, bson_error_t *error) {
...
mongoc_client_t *entity_map_get_client(entity_map_t *entity_map, const char *id, bson_error_t *error) {
...
mongoc_database_t *entity_map_get_database(entity_map_t *entity_map, const char *id, bson_error_t *error) {
...
mongoc_collection_t *entity_map_get_collection(entity_map_t *entity_map, const char *id, bson_error_t *error) {
...
mongoc_change_stream_t *entity_map_get_changestream(entity_map_t *entity_map, const char *id, bson_error_t *error) {
...
entity_findcursor_t *entity_map_get_findcursor(entity_map_t *entity_map, const char *id, bson_error_t *error) {
...
bson_val_t *entity_map_get_bson(entity_map_t *entity_map, const char *id, bson_error_t *error) {
...
mongoc_array_t *entity_map_get_bson_array(entity_map_t *entity_map, const char *id, bson_error_t *error) {
...
size_t *entity_map_get_size_t(entity_map_t *entity_map, const char *id, bson_error_t *error) {
...
mongoc_client_session_t *entity_map_get_session(entity_map_t *entity_map, const char *id, bson_error_t *error) {
...
static bson_t *entity_map_get_lsid(entity_map_t *em, char *session_id, bson_error_t *error) {
    entity_t *entity = NULL;
...
mongoc_gridfs_bucket_t *entity_map_get_bucket(entity_map_t *entity_map, const char *id, bson_error_t *error) {
...
static bool _entity_map_add(entity_map_t *em, const char *id, const char *type, void *value, bson_error_t *error) {
    bson_error_t tmperr;
    entity_t *entity = NULL;
...
bool entity_map_add_changestream(entity_map_t *em,
                                 const char *id,
...
void entity_findcursor_iterate_until_document_or_error(entity_findcursor_t *findcursor,
...
bool entity_map_add_findcursor(entity_map_t *em,
                               const char *id,
                               mongoc_cursor_t *cursor,
                               const bson_t *first_result,
                               bson_error_t *error) {
...
bool entity_map_add_bson(entity_map_t *em, const char *id, bson_val_t *val, bson_error_t *error) {
    return _entity_map_add(em, id, "bson", (void *)bson_val_copy(val), error);
}

bool entity_map_add_bson_array(entity_map_t *em, const char *id, bson_error_t *error) {
...
bool entity_map_add_size_t(entity_map_t *em, const char *id, size_t *value, bson_error_t *error) {
    return _entity_map_add(em, id, "size_t", value, error);
}

/* implement $$sessionLsid */
static bool special_session_lsid(bson_matcher_t *matcher,
...
bool special_matches_entity(bson_matcher_t *matcher,
                            const bson_t *assertion,
                            const bson_val_t *actual,
                            void *ctx,
                            const char *path,
...
bool entity_map_match(entity_map_t *em,
...
char *event_list_to_string(event_t *events) {
...
bool entity_map_end_session(entity_map_t *em, char *session_id, bson_error_t *error) {
    bool ret = false;
    entity_t *entity = NULL;
...
char *entity_map_get_session_client_id(entity_map_t *em, char *session_id, bson_error_t *error) {
    char *ret = NULL;
    entity_t *entity = NULL;
...
void entity_map_set_reduced_heartbeat(entity_map_t *em, bool val) {
    em->reduced_heartbeat = val;
}

void entity_map_disable_event_listeners(entity_map_t *em) {
    entity_t *eiter = NULL;
