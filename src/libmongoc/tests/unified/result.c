...
static void _result_init(result_t *result, const bson_val_t *value, const bson_t *reply, const bson_error_t *error) {
    bson_string_t *str;
...
bson_val_t *result_get_val(result_t *result) {
    return result->value;
}

bson_t *rewrite_upserted_ids(bson_t *mongoc_upserted_ids) {
    bson_t *upserted_ids;
    bson_iter_t iter;
...
bson_t *rewrite_bulk_write_result(const bson_t *bulk_write_result) {
...
void result_from_bulk_write(result_t *result, const bson_t *reply, const bson_error_t *error) {
...
void result_from_insert_one(result_t *result, const bson_t *reply, const bson_error_t *error) {
    bson_t *write_result;
    bson_val_t *val;
...
void result_from_insert_many(result_t *result, const bson_t *reply, const bson_error_t *error) {
    bson_t *write_result;
    bson_val_t *val;
...
void result_from_update_or_replace(result_t *result, const bson_t *reply, const bson_error_t *error) {
    bson_t *write_result;
    bson_val_t *val;
    bson_iter_t iter;
...
void result_from_delete(result_t *result, const bson_t *reply, const bson_error_t *error) {
    bson_t *write_result;
    bson_val_t *val;
...
void result_from_distinct(result_t *result, const bson_t *reply, const bson_error_t *error) {
    bson_val_t *val = NULL;
    bson_iter_t iter;
...
void result_from_cursor(result_t *result, mongoc_cursor_t *cursor) {
...
void result_from_val_and_reply(result_t *result,
...
void result_from_ok(result_t *result) {
    bson_error_t error = {0};
    _result_init(result, NULL, NULL, &error);
}

bool result_check(result_t *result,
                  entity_map_t *em,
    ...
    if (expect_error) {
        ...
        if (error_code) {
            if ((*error_code) != (int64_t)result->error.code) {
                test_set_error(error,
                               "expected error code %" PRIi64 ", but got: %" PRIi64,
                               *error_code,
...
static void test_resultfrombulkwrite(void) {
    bson_error_t error;
    result_t *result;
    bson_t *reply;
    bson_val_t *expect;
...
void test_result_install(TestSuite *suite) {
