...
bson_t *bson_copy_and_sort(const bson_t *in) {
    bson_t *out = bson_new();
    bson_iter_t iter;
    const char **keys;
...
bson_type_t bson_type_from_string(const char *in) {
    int i;
...
const char *bson_type_to_string(bson_type_t btype) {
    int i;
    for (i = 0; i < sizeof(bson_type_map) / sizeof(bson_type_map[0]); i++) {
...
static void test_copy_and_sort(void) {
...
void test_bson_util_install(TestSuite *suite) {
...
int64_t usecs_since_epoch(void) {
    struct timeval tv;
