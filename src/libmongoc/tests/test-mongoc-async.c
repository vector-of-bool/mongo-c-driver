...
static void test_hello_helper(mongoc_async_cmd_t *acmd,
...
static void test_hello_impl(bool with_ssl) {
...
static void test_hello_ssl(void) {
    test_hello_impl(true);
}
#else

static void test_large_hello_helper(mongoc_async_cmd_t *acmd,
                                    mongoc_async_cmd_result_t result,
                                    const bson_t *bson,
                                    int64_t duration_usec) {
    bson_iter_t iter;
...
static void test_large_hello(void *ctx) {
    mongoc_async_t *async;
...
static void test_hello_delay_callback(mongoc_async_cmd_t *acmd,
                                      mongoc_async_cmd_result_t result,
                                      const bson_t *bson,
                                      int64_t duration_usec) {
    BSON_UNUSED(result);
...
static mongoc_stream_t *test_hello_delay_initializer(mongoc_async_cmd_t *acmd) {
    return ((stream_with_result_t *)acmd->data)->stream;
}

static void test_hello_delay(void) {
...
void test_async_install(TestSuite *suite) {
