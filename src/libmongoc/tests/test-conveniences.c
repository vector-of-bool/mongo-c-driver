...
void test_conveniences_init(void) {
...
void test_conveniences_cleanup(void) {
    bson_t *doc;
...
void value_init_from_doc(bson_value_t *value, const bson_t *doc) {
    BSON_ASSERT(doc);
...
const char *tmp_str(const char *format, ...) {
    va_list args;
    char *str;
...
void bson_lookup(const bson_t *bson, const char *path, bson_iter_t *out) {
    bson_iter_t iter;
 ...
 *--------------------------------------------------------------------------
 */
void bson_iter_bson(const bson_iter_t *iter, bson_t *bson) {
    uint32_t len;
    const uint8_t *data;
 ...
 *--------------------------------------------------------------------------
 */
const char *bson_lookup_utf8(const bson_t *b, const char *key) {
    bson_iter_t iter;
 ...
 *--------------------------------------------------------------------------
 */
void bson_lookup_value(const bson_t *b, const char *key, bson_value_t *value) {
    bson_iter_t iter;
...
bson_t *bson_lookup_bson(const bson_t *b, const char *key) {
    bson_iter_t iter;
    bson_t tmp;
 ...
 *--------------------------------------------------------------------------
 */
void bson_lookup_doc(const bson_t *b, const char *key, bson_t *doc) {
    bson_iter_t iter;
...
void bson_lookup_doc_null_ok(const bson_t *b, const char *key, bson_t *doc) {
    bson_iter_t iter;
 ...
 *--------------------------------------------------------------------------
 */
bool bson_lookup_bool(const bson_t *b, const char *key) {
    bson_iter_t iter;
 ...
 *--------------------------------------------------------------------------
 */
int32_t bson_lookup_int32(const bson_t *b, const char *key) {
    bson_iter_t iter;
 ...
 *--------------------------------------------------------------------------
 */
int64_t bson_lookup_int64(const bson_t *b, const char *key) {
    bson_iter_t iter;
 ...
 *--------------------------------------------------------------------------
 */
mongoc_read_concern_t *bson_lookup_read_concern(const bson_t *b, const char *key) {
    bson_t doc;
 ...
 *--------------------------------------------------------------------------
 */
mongoc_write_concern_t *bson_lookup_write_concern(const bson_t *b, const char *key) {
 ...
 *--------------------------------------------------------------------------
 */
mongoc_read_prefs_t *bson_lookup_read_prefs(const bson_t *b, const char *key) {
    bson_t doc;
    const char *str;
 ...
 *--------------------------------------------------------------------------
 */
void bson_lookup_database_opts(const bson_t *b, const char *key, mongoc_database_t *database) {
    bson_t doc;
 ...
 *--------------------------------------------------------------------------
 */
void bson_lookup_collection_opts(const bson_t *b, const char *key, mongoc_collection_t *collection) {
    bson_t doc;
    mongoc_read_concern_t *rc;
    mongoc_write_concern_t *wc;
    mongoc_read_prefs_t *prefs;
 ...
 *--------------------------------------------------------------------------
 */
mongoc_transaction_opt_t *bson_lookup_txn_opts(const bson_t *b, const char *key) {
    bson_t doc;
 ...
 *--------------------------------------------------------------------------
 */
mongoc_session_opt_t *bson_lookup_session_opts(const bson_t *b, const char *key) {
    bson_t doc;
 ...
 *--------------------------------------------------------------------------
 */
mongoc_client_session_t *bson_lookup_session(const bson_t *b, const char *key, mongoc_client_t *client) {
    mongoc_session_opt_t *opts;
...
bool match_bson_value(const bson_value_t *doc, const bson_value_t *pattern, match_ctx_t *ctx) {
    ...
    case BSON_TYPE_DECIMAL128:
        ...
        break;
    default: test_error("unexpected value type %d: %s", doc->value_type, _mongoc_bson_type_to_str(doc->value_type));
    }
...
bool mongoc_write_concern_append_bad(mongoc_write_concern_t *write_concern, bson_t *command) {
...
static void init_huge_string(mongoc_client_t *client) {
...
static void init_four_mb_string(void) {
    test_conveniences_init();
...
const char *four_mb_string(void) {
    init_four_mb_string();
    return gFourMBString;
}

static bool find_key(void *current, void *key) {
    return !strcmp((const char *)current, (const char *)key);
}

static void key_dtor(void *item, void *ctx) {
    BSON_UNUSED(item);
    BSON_UNUSED(ctx);
    /* mongoc_set_t requires a dtor, there's nothing to destroy */
}

void assert_no_duplicate_keys(const bson_t *doc) {
    mongoc_set_t *keys;
    bson_iter_t iter;
    bson_t subdoc;
...
void match_in_array(const bson_t *doc, const bson_t *array, match_ctx_t *ctx) {
    bson_iter_t array_iter;
    bool found = false;
...
bson_t *bson_with_all_types(void) {
...
const char *json_with_all_types(void) {
    const char *json = "{\n"
                       "    \"double\": {\n"
                       "        \"$numberDouble\": \"1.0\"\n"
                       "    },\n"
...
void assert_wc_oob_error(bson_error_t *error) {
...
void semver_parse(const char *str, semver_t *out) {
    char *dot;
...
void server_semver(mongoc_client_t *client, semver_t *out) {
    bson_t reply;
    bson_error_t error;
...
int semver_cmp_str(semver_t *a, const char *str) {
    semver_t b;
...
const char *semver_to_string(semver_t *semver) {
