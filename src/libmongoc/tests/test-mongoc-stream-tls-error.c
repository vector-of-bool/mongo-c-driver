...
static BSON_THREAD_FUN(ssl_error_server, ptr) {
    ...
    case SSL_TEST_BEHAVIOR_STALL_BEFORE_HANDSHAKE: _mongoc_usleep(data->handshake_stall_ms * 1000); break;
    ...
    default: test_error("unimplemented ssl_test_behavior_t");
    }
...
static void test_mongoc_tls_hangup(void) {
...
static void test_mongoc_tls_handshake_stall(void) {
    mongoc_ssl_opt_t sopt = {0};
    mongoc_ssl_opt_t copt = {0};
    ssl_test_result_t sr;
    ssl_test_result_t cr;
...
static void test_mongoc_tls_load_files(void) {
...
void test_stream_tls_error_install(TestSuite *suite) {
#if !defined(MONGOC_ENABLE_SSL_SECURE_CHANNEL) && !defined(MONGOC_ENABLE_SSL_LIBRESSL)
