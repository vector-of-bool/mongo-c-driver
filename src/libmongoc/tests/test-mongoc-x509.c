...
static void test_extract_subject(void) {
    char *subject;
...
static void _expect_malformed(const char *data, int32_t len) {
    bool ret;
...
static void _expect_no_status_request(const char *data, int32_t len) {
    bool ret;
    ret = _mongoc_tlsfeature_has_status_request((const uint8_t *)data, len);
    BSON_ASSERT(!ret);
...
static void _expect_status_request(const char *data, int32_t len) {
    bool ret;
    ret = _mongoc_tlsfeature_has_status_request((const uint8_t *)data, len);
    BSON_ASSERT(ret);
    ASSERT_NO_CAPTURED_LOGS("mongoc");
...
static void test_tlsfeature_parsing(void) {
    capture_logs(true);
...
void test_x509_install(TestSuite *suite) {
#if defined(MONGOC_ENABLE_SSL) && !defined(MONGOC_ENABLE_SSL_LIBRESSL)
