...
void my_value_free(my_value *ptr) {
...
void widget_delete(void *w_) {
    struct widget *w = w_;
    *w->store_value_here = w->value;
    bson_free(w);
}

static void test_aliased(void) {
...
void test_shared_install(TestSuite *suite) {
