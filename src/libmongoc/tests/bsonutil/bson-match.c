...
static char *get_first_key(const bson_t *bson) {
    bson_iter_t iter;
...
static bool is_special_match(const bson_t *bson) {
...
static bool special_placeholder(bson_matcher_t *matcher,
...
/* implements $$exists */
static bool special_exists(bson_matcher_t *matcher,
                           const bson_t *assertion,
                           const bson_val_t *actual,
                           void *ctx,
                           const char *path,
...
/* implements $$type */
static bool special_type(bson_matcher_t *matcher,
                         const bson_t *assertion,
                         const bson_val_t *actual,
                         void *ctx,
                         const char *path,
...
static bool special_unset_or_matches(bson_matcher_t *matcher,
                                     const bson_t *assertion,
                                     const bson_val_t *actual,
                                     void *ctx,
                                     const char *path,
...
static bool special_matches_hex_bytes(bson_matcher_t *matcher,
                                      const bson_t *assertion,
                                      const bson_val_t *actual,
                                      void *ctx,
                                      const char *path,
...
static bool evaluate_special(bson_matcher_t *matcher,
                             const bson_t *assertion,
                             const bson_val_t *actual,
                             const char *path,
                             bson_error_t *error) {
...
bool bson_matcher_match(bson_matcher_t *matcher,
    ...
    if (bson_val_type(expected) == BSON_TYPE_ARRAY) {
        ...
        if (expected_keys != actual_keys) {
            MATCH_ERR("expected array of size %" PRIu32 ", but got array of size: %" PRIu32,
                      expected_keys,
                      actual_keys);
            goto done;
        }
...
void test_bson_match_install(TestSuite *suite) {
