...
bson_val_t *bson_val_from_value(const bson_value_t *value) {
    bson_val_t *val = NULL;
    bson_t tmp;
    char *as_string = NULL;
...
bson_val_t *bson_val_from_json(const char *single_quoted_json) {
    bson_val_t *val = NULL;
...
bson_val_t *bson_val_from_iter(const bson_iter_t *iter) {
    return bson_val_from_value(bson_iter_value((bson_iter_t *)iter));
}

bson_val_t *bson_val_from_bson(const bson_t *bson) {
    bson_value_t value;
    bson_iter_t iter;
...
bson_val_t *bson_val_from_array(const bson_t *bson) {
    bson_value_t value;
...
bson_val_t *bson_val_from_doc(const bson_t *bson) {
    bson_value_t value;
...
bson_val_t *bson_val_from_int64(int64_t val) {
    bson_value_t value;
...
bson_val_t *bson_val_from_bytes(const uint8_t *bytes, uint32_t len) {
    bson_value_t value;
...
bson_val_t *bson_val_copy(const bson_val_t *val) {
    if (!val) {
        return NULL;
    }
...
void bson_val_destroy(bson_val_t *val) {
    if (!val) {
        return;
    }
...
bool bson_val_eq(const bson_val_t *a, const bson_val_t *b, bson_val_comparison_flags_t flags) {
    bson_type_t vtype;
...
bson_type_t bson_val_type(const bson_val_t *val) {
    return val->type;
}

const bson_t *bson_val_to_document(const bson_val_t *val) {
...
const bson_t *bson_val_to_array(const bson_val_t *val) {
...
const bson_t *bson_val_to_bson(const bson_val_t *val) {
...
const uint8_t *bson_val_to_binary(const bson_val_t *val, uint32_t *len) {
...
const bson_value_t *bson_val_to_value(const bson_val_t *val) {
    return (bson_value_t *)&val->value;
}

const char *bson_val_to_utf8(const bson_val_t *val) {
    BSON_ASSERT(val->type == BSON_TYPE_UTF8);
    return val->value.value.v_utf8.str;
}

bool bson_val_is_numeric(const bson_val_t *val) {
    return (val->type == BSON_TYPE_INT32 || val->type == BSON_TYPE_INT64 || val->type == BSON_TYPE_DOUBLE);
}

int64_t bson_val_convert_int64(const bson_val_t *val) {
...
const char *bson_val_to_json(const bson_val_t *val) {
    if (!val) {
        return "(NULL)";
    }
