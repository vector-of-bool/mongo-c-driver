...
static mongoc_server_description_t *_force_scan(mongoc_client_t *client, mock_server_t *server, const char *hello) {
    bson_error_t error;
    request_t *request;
    future_t *future;
...
static void test_topology_version_update(void) {
...
static char *_single_to_double(const char *in) {
    char *in_copy;
    char *iter;
...
static void _assert_topology_version_compare(const char *tv1, const char *tv2, int expected) {
    int actual;
    bson_t *tv1_bson;
    bson_t *tv2_bson;
    bson_error_t error;
...
static void test_topology_version_compare(void) {
    ...
    _assert_topology_version_compare("{ }",
                                     "{ 'processId': { '$oid': 'AABBAABBAABBAABBAABBAABB' }, 'counter': 1 }",
                                     -1);
    _assert_topology_version_compare("{ 'counter': 2 }",
                                     "{ 'processId': { '$oid': 'AABBAABBAABBAABBAABBAABB' }, 'counter': 1 }",
                                     -1);
}

void test_streamable_hello_install(TestSuite *suite) {
