...
static void test_session_opts_clone(void) {
...
static void test_session_opts_causal_consistency_and_snapshot(void) {
    mongoc_session_opt_t *opts;
...
static void test_session_no_crypto(void *ctx) {
...
static void _test_session_pool_lifo(bool pooled) {
...
static void test_session_pool_lifo_single(void *ctx) {
    BSON_UNUSED(ctx);

    _test_session_pool_lifo(false);
}

static void test_session_pool_lifo_pooled(void *ctx) {
    BSON_UNUSED(ctx);
 ...
 */
static void _test_session_pool_timeout(bool pooled) {
    mongoc_client_pool_t *pool = NULL;
    mongoc_client_t *client;
    uint32_t server_id;
...
static void test_session_pool_timeout_single(void *ctx) {
    BSON_UNUSED(ctx);

    _test_session_pool_timeout(false);
}

static void test_session_pool_timeout_pooled(void *ctx) {
    BSON_UNUSED(ctx);
 ...
 */
static void _test_session_pool_reap(bool pooled) {
    mongoc_client_pool_t *pool = NULL;
    mongoc_client_t *client;
...
static void test_session_pool_reap_single(void *ctx) {
    BSON_UNUSED(ctx);

    _test_session_pool_reap(false);
}

static void test_session_pool_reap_pooled(void *ctx) {
    BSON_UNUSED(ctx);

    _test_session_pool_reap(true);
}

static void test_session_id_bad(void *ctx) {
    const char *bad_opts[] = {
        "{'sessionId': null}",
        "{'sessionId': 'foo'}",
...
static void _test_session_supported(bool pooled) {
    mongoc_client_pool_t *pool = NULL;
    mongoc_client_t *client;
    bson_error_t error;
...
static void test_session_supported_single(void *ctx) {
    BSON_UNUSED(ctx);

    _test_session_supported(false);
}

static void test_session_supported_pooled(void *ctx) {
    BSON_UNUSED(ctx);

    _test_session_supported(true);
}

static void _test_mock_end_sessions(bool pooled) {
...
static void test_mock_end_sessions_single(void) {
    _test_mock_end_sessions(false);
}

static void test_mock_end_sessions_pooled(void) {
...
static void test_mock_end_sessions_server_disconnect(void) {
    mock_server_t *server;
    mongoc_client_t *client;
    bson_error_t error;
...
static void endsessions_started_cb(const mongoc_apm_command_started_t *event) {
...
static void endsessions_succeeded_cb(const mongoc_apm_command_succeeded_t *event) {
    endsessions_test_t *test;
...
static void endsessions_test_init(endsessions_test_t *test, bool pooled) {
...
static void endsessions_test_destroy_client(endsessions_test_t *test) {
    if (test->pool) {
...
static void endsessions_test_get_ended_lsids(endsessions_test_t *test, size_t index, bson_t *ended_lsids) {
    bson_iter_t iter;
...
static void endsessions_test_cleanup(endsessions_test_t *test) {
    size_t i;
...
static void _test_end_sessions(bool pooled) {
    endsessions_test_t test;
    mongoc_client_t *client;
    bson_error_t error;
...
static void test_end_sessions_single(void *ctx) {
    BSON_UNUSED(ctx);

    _test_end_sessions(false);
}

static void test_end_sessions_pooled(void *ctx) {
    BSON_UNUSED(ctx);
...
static void send_ping(mongoc_client_t *client, mongoc_client_session_t *client_session) {
...
static void _test_end_sessions_many(bool pooled) {
    endsessions_test_t test;
    mongoc_client_t *client;
    int i;
...
static void test_end_sessions_many_single(void *ctx) {
    BSON_UNUSED(ctx);

    _test_end_sessions_many(false);
}

static void test_end_sessions_many_pooled(void *ctx) {
    BSON_UNUSED(ctx);
...
static void test_session_advance_cluster_time(void *ctx) {
    mongoc_client_t *client;
    bson_error_t error;
...
static void _test_advance_operation_time(mongoc_client_session_t *cs, uint32_t t, uint32_t i, bool should_advance) {
...
static void test_session_advance_operation_time(void *ctx) {
    mongoc_client_t *client;
    bson_error_t error;
    mongoc_client_session_t *cs;
    uint32_t t, i;
...
static void started(const mongoc_apm_command_started_t *event) {
    match_ctx_t ctx = {{0}};
    bson_iter_t iter;
...
static void succeeded(const mongoc_apm_command_succeeded_t *event) {
    bson_iter_t iter;
    bool has_cluster_time;
    bson_t cluster_time;
...
static void failed(const mongoc_apm_command_failed_t *event) {
    const char *cmd_name;
    bson_error_t error;
...
static void set_session_test_callbacks(session_test_t *test) {
    mongoc_apm_callbacks_t *callbacks;
...
static session_test_t *session_test_new(session_test_correct_t correct_client, session_test_causal_t causal) {
    session_test_t *test;
...
static int check_session_returned_visit(mongoc_server_session_t *ss, mongoc_topology_t *unused, void *check_state_) {
    match_ctx_t ctx = {{0}};
...
static void check_session_returned(session_test_t *test, const bson_t *lsid) {
    struct check_session_returned_t check_state;

    check_state.expect_lsid = lsid;
    check_state.found = false;

    mongoc_server_session_pool_visit_each(test->session_client->topology->session_pool,
                                          &check_state,
...
static const bson_t *first_cmd(session_test_t *test) {
    ASSERT_CMPSIZE_T(test->cmds.len, >, (size_t)0);
    return _mongoc_array_index(&test->cmds, bson_t *, 0);
}

static const bson_t *last_non_getmore_cmd(session_test_t *test) {
    ssize_t i;
    const bson_t *cmd;
...
static void session_test_destroy(session_test_t *test) {
    bson_t session_lsid;
    size_t i;
...
static void check_sessions_from_same_client_enforced(session_test_t *test) {
...
static void check_sessions_with_w0_prohibited(session_test_t *test) {
...
static void check_success(session_test_t *test) {
...
static void check_cluster_time(session_test_t *test) {
...
static void _test_explicit_session_lsid(session_test_fn_t test_fn) {
    session_test_t *test;
    bson_error_t error;
    int64_t start;
...
static void _test_session_from_wrong_client(session_test_fn_t test_fn) {
    session_test_t *test;
    bson_error_t error;
...
static void _test_implicit_session_lsid(session_test_fn_t test_fn) {
    session_test_t *test;
    int64_t start;
    mongoc_server_session_t *ss;
...
static void parse_read_concern_time(const bson_t *cmd, op_time_t *op_time) {
    bson_iter_t iter;
    bson_iter_t rc;
...
static void parse_reply_time(const bson_t *reply, op_time_t *op_time) {
    bson_iter_t iter;
...
static void _test_causal_consistency(session_test_fn_t test_fn, bool allow_read_concern) {
    session_test_t *test;
...
static void _run_session_test(session_test_fn_t test_fn, bool allow_read_concern) {
...
static void run_session_test(void *ctx) {
...
static void run_session_test_no_rc(void *ctx) {
...
static void run_session_test_bulk_operation(void *ctx) {
...
static void run_count_test(void *ctx) {
...
static void insert_10_docs(session_test_t *test) {
...
static void test_cmd(session_test_t *test) {
...
static void test_read_cmd(session_test_t *test) {
    test->succeeded = mongoc_client_read_command_with_opts(test->client,
                                                           "db",
                                                           tmp_bson("{'listCollections': 1}"),
                                                           NULL,
                                                           &test->opts,
...
static void test_write_cmd(session_test_t *test) {
...
static void test_read_write_cmd(session_test_t *test) {
    bson_t *cmd = tmp_bson("{"
...
static void test_db_cmd(session_test_t *test) {
    test->succeeded = mongoc_database_command_with_opts(test->db,
                                                        tmp_bson("{'listCollections': 1}"),
                                                        NULL,
                                                        &test->opts,
                                                        NULL,
...
static void test_count(session_test_t *test) {
    test->succeeded = (-1
                       != mongoc_collection_count_with_opts(test->collection,
                                                            MONGOC_QUERY_NONE,
                                                            NULL,
                                                            0,
                                                            0,
...
static void test_cursor(session_test_t *test) {
...
static void test_drop(session_test_t *test) {
...
static void test_drop_index(session_test_t *test) {
    bson_error_t error;
    bool r;
...
static void test_create_index(session_test_t *test) {
    BEGIN_IGNORE_DEPRECATIONS
    test->succeeded = mongoc_collection_create_index_with_opts(test->collection,
                                                               tmp_bson("{'a': 1}"),
                                                               NULL,
                                                               &test->opts,
                                                               NULL,
...
static void test_replace_one(session_test_t *test) {
...
static void test_update_one(session_test_t *test) {
    test->succeeded = mongoc_collection_update_one(test->collection,
                                                   tmp_bson("{}"),
                                                   tmp_bson("{'$set': {'x': 1}}"),
                                                   &test->opts,
                                                   NULL,
...
static void test_update_many(session_test_t *test) {
    test->succeeded = mongoc_collection_update_many(test->collection,
                                                    tmp_bson("{}"),
                                                    tmp_bson("{'$set': {'x': 1}}"),
                                                    &test->opts,
                                                    NULL,
...
static void test_insert_one(session_test_t *test) {
    test->succeeded = mongoc_collection_insert_one(test->collection, tmp_bson("{}"), &test->opts, NULL, &test->error);
}

static void test_insert_many(session_test_t *test) {
...
static void test_fam(session_test_t *test) {
...
static void test_db_drop(session_test_t *test) {
    test->succeeded = mongoc_database_drop_with_opts(test->db, &test->opts, &test->error);
}

static void test_gridfs_find(session_test_t *test) {
    mongoc_gridfs_t *gfs;
    bson_error_t error;
...
static void test_gridfs_find_one(session_test_t *test) {
    mongoc_gridfs_t *gfs;
    bson_error_t error;
    mongoc_gridfs_file_t *f;
...
static void test_watch(session_test_t *test) {
...
static void test_aggregate(session_test_t *test) {
    bson_t opts;
    mongoc_cursor_t *cursor;
    const bson_t *doc;
...
static void test_create(session_test_t *test) {
    mongoc_collection_t *collection;
...
static void test_database_names(session_test_t *test) {
    char **names;
...
static void test_find_databases(session_test_t *test) {
    mongoc_cursor_t *cursor;
    const bson_t *doc;
...
static void test_find_collections(session_test_t *test) {
    mongoc_cursor_t *cursor;
    const bson_t *doc;
...
static void test_collection_names(session_test_t *test) {
    char **strv;
...
static void test_find_indexes(session_test_t *test) {
    mongoc_cursor_t *cursor;
    const bson_t *doc;
...
static void _test_bulk(session_test_t *test, mongoc_bulk_operation_t *bulk) {
    uint32_t i;
    ...
    test->succeeded = mongoc_bulk_operation_update_one_with_opts(bulk,
                                                                 tmp_bson("{}"),
                                                                 tmp_bson("{'$set': {'x': 1}}"),
                                                                 NULL,
                                                                 &test->error);
...
static void test_bulk(session_test_t *test) {
    mongoc_bulk_operation_t *bulk;
...
static void test_bulk_set_session(session_test_t *test) {
    mongoc_bulk_operation_t *bulk;
    bson_iter_t iter;
    mongoc_client_session_t *cs;
    bson_error_t error;
...
static void test_bulk_set_client(session_test_t *test) {
    mongoc_bulk_operation_t *bulk;
    bson_iter_t iter;
    mongoc_client_session_t *cs;
    bson_error_t error;
...
static void test_cursor_implicit_session(void *ctx) {
    session_test_t *test;
...
static void test_change_stream_implicit_session(void *ctx) {
    session_test_t *test;
    mongoc_topology_t *topology;
    mongoc_client_session_t *cs;
    bson_error_t error;
...
static void test_cmd_error(void *ctx) {
    session_test_t *test;
    bson_error_t error;
...
static void test_read_concern(void *ctx) {
    session_test_t *test;
...
static void _test_unacknowledged(session_test_fn_t test_fn, bool explicit_cs, bool inherit_wc) {
    session_test_t *test;
...
static void test_unacknowledged_explicit_cs_inherit_wc(void *ctx) {
    _test_unacknowledged((session_test_fn_t)((TestFnCtx *)ctx)->test_fn, true, true);
}

static void test_unacknowledged_implicit_cs_explicit_wc(void *ctx) {
    _test_unacknowledged((session_test_fn_t)((TestFnCtx *)ctx)->test_fn, true, false);
}

static void test_unacknowledged_implicit_cs_inherit_wc(void *ctx) {
    _test_unacknowledged((session_test_fn_t)((TestFnCtx *)ctx)->test_fn, false, true);
}

static void test_unacknowledged_explicit_cs_explicit_wc(void *ctx) {
...
static bool _test_run_operation(json_test_ctx_t *ctx, const bson_t *test, const bson_t *operation) {
    bson_t reply;
...
static void test_sessions_spec_cb(bson_t *scenario) {
...
static void _test_session_dirty_helper(bool retry_succeeds) {
    mongoc_client_t *client;
...
static void test_session_dirty(void *unused) {
    BSON_UNUSED(unused);
...
void test_sessions_snapshot_prose_test_1(void *ctx) {
...
void test_session_install(TestSuite *suite) {
