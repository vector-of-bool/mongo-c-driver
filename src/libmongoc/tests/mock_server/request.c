...
static void request_from_op_msg(request_t *request, const mongoc_rpc_t *rpc);
...
request_t *request_new(const mongoc_buffer_t *buffer,
    ...
    case MONGOC_OPCODE_COMPRESSED: break;
    ...
    case MONGOC_OPCODE_KILL_CURSORS: request_from_killcursors(request, &request->request_rpc); break;
    ...
    case MONGOC_OPCODE_MSG: request_from_op_msg(request, &request->request_rpc); break;

    case MONGOC_OPCODE_REPLY:
    default: test_error("Unimplemented opcode %d", request->opcode);
    }

    return request;
}

const bson_t *request_get_doc(const request_t *request, size_t n) {
    BSON_ASSERT(request);
    return _mongoc_array_index(&request->docs, const bson_t *, n);
}

void assert_request_matches_flags(const request_t *request, mongoc_query_flags_t flags) {
...
/* TODO: take file, line, function params from caller, wrap in macro */
bool request_matches_bulk_insert(const request_t *request, const char *ns, mongoc_insert_flags_t flags, int n) {
    const mongoc_rpc_t *rpc;
...
/* TODO: take file, line, function params from caller, wrap in macro */
bool request_matches_getmore(const request_t *request, const char *ns, int32_t n_return, int64_t cursor_id) {
    const mongoc_rpc_t *rpc;
...
/* TODO: take file, line, function params from caller, wrap in macro */
bool request_matches_kill_cursors(const request_t *request, int64_t cursor_id) {
    const mongoc_rpc_t *rpc;
...
bool request_matches_msg(const request_t *request, uint32_t flags, const bson_t **docs, size_t n_docs) {
    const bson_t *doc;
...
bool request_matches_msgv(const request_t *request, uint32_t flags, va_list *args) {
    bson_t **docs;
    ...
    if (flags == MONGOC_QUERY_NONE) {
        ...
        while (flag <= MONGOC_QUERY_PARTIAL) {
            ...
            if (flags & flag) {
                ...
                switch (flag) {
                case MONGOC_QUERY_TAILABLE_CURSOR: bson_string_append(str, "TAILABLE"); break;
                case MONGOC_QUERY_SECONDARY_OK: bson_string_append(str, "SECONDARY_OK"); break;
                case MONGOC_QUERY_OPLOG_REPLAY: bson_string_append(str, "OPLOG_REPLAY"); break;
                case MONGOC_QUERY_NO_CURSOR_TIMEOUT: bson_string_append(str, "NO_TIMEOUT"); break;
                case MONGOC_QUERY_AWAIT_DATA: bson_string_append(str, "AWAIT_DATA"); break;
                case MONGOC_QUERY_EXHAUST: bson_string_append(str, "EXHAUST"); break;
                case MONGOC_QUERY_PARTIAL: bson_string_append(str, "PARTIAL"); break;
...
static uint32_t length_prefix(const uint8_t *data) {
    uint32_t len_le;
    ...
    if (flags == MONGOC_UPDATE_NONE) {
        ...
        while (flag <= MONGOC_UPDATE_MULTI_UPDATE) {
            ...
            if (flags & flag) {
                ...
                switch (flag) {
                case MONGOC_UPDATE_UPSERT: bson_string_append(str, "UPSERT"); break;
                case MONGOC_UPDATE_MULTI_UPDATE: bson_string_append(str, "MULTI"); break;
...
static void parse_op_msg_doc(request_t *request, const uint8_t *data, int32_t len, bson_string_t *msg_as_str) {
    int32_t data_len;
    int32_t doc_len;
    bson_t *doc;
    const uint8_t *pos;
...
static void request_from_op_msg(request_t *request, const mongoc_rpc_t *rpc) {
    ...
    for (section_no = 0; section_no < rpc->msg.n_sections; section_no++) {
        ...
        case 1:
            ...
            bson_string_append(msg_as_str, "]");
            break;
        default: test_error("Unimplemented payload type %d\n", section->payload_type);
        }
    }
