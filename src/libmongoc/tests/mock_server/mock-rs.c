...
mongoc_uri_t *make_uri(mongoc_array_t *servers) {
    const char *host_and_port;
...
static char *hello_json(mock_rs_t *rs, mongoc_server_description_type_t type, const bson_t *tags) {
    char *server_type;
    char *tags_json;
    char *hosts_str;
    char *json;
...
mock_rs_t *mock_rs_with_auto_hello(int32_t max_wire_version, bool has_primary, int n_secondaries, int n_arbiters) {
    int i;
...
void mock_rs_tag_primary(mock_rs_t *rs, const bson_t *tags) {
    bson_destroy(&rs->primary_tags);
    bson_copy_to(tags, &rs->primary_tags);
}

void mock_rs_tag_secondary(mock_rs_t *rs, int server_number, const bson_t *tags) {
    bson_destroy(rs->secondary_tags[server_number]);
    rs->secondary_tags[server_number] = bson_copy(tags);
}

static void mock_rs_auto_endsessions(mock_rs_t *rs) {
...
void mock_rs_set_request_timeout_msec(mock_rs_t *rs, int64_t request_timeout_msec) {
    rs->request_timeout_msec = request_timeout_msec;
}

static bool rs_q_append(request_t *request, void *data) {
...
request_t *mock_rs_receives_insert(mock_rs_t *rs, const char *ns, mongoc_insert_flags_t flags, const char *doc_json) {
    request_t *request;
...
request_t *mock_rs_receives_getmore(mock_rs_t *rs, const char *ns, int32_t n_return, int64_t cursor_id) {
    request_t *request;
...
request_t *_mock_rs_receives_msg(mock_rs_t *rs, uint32_t flags, ...) {
    request_t *request;
    va_list args;
    bool r;
...
request_t *mock_rs_receives_kill_cursors(mock_rs_t *rs, int64_t cursor_id) {
    request_t *request;
...
static mongoc_server_description_type_t _mock_rs_server_type(mock_rs_t *rs, uint16_t port) {
...
void mock_rs_replies_simple(request_t *request, const char *docs_json) {
...
bool mock_rs_request_is_to_primary(mock_rs_t *rs, request_t *request) {
    BSON_ASSERT(request);
...
bool mock_rs_request_is_to_secondary(mock_rs_t *rs, request_t *request) {
    BSON_ASSERT(request);
...
void mock_rs_stepdown(mock_rs_t *rs) {
    char *json;
...
void mock_rs_elect(mock_rs_t *rs, size_t id) {
    char *json;
    mock_server_t **ptrs;
