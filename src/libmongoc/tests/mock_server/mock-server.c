...
static void _mock_server_reply_with_stream(mock_server_t *server, reply_t *reply, mongoc_stream_t *client);
...
mock_server_t *mock_server_new(void) {
...
mock_server_t *mock_server_with_auto_hello(int32_t max_wire_version) {
...
static bool hangup(request_t *request, void *ctx) {
    BSON_UNUSED(ctx);
 ...
 *--------------------------------------------------------------------------
 */
void mock_server_set_ssl_opts(mock_server_t *server, mongoc_ssl_opt_t *opts) {
...
int mock_server_autoresponds(mock_server_t *server, autoresponder_t responder, void *data, destructor_t destructor) {
...
void mock_server_remove_autoresponder(mock_server_t *server, int id) {
    size_t i;
...
static bool auto_hello_generate_response(request_t *request, void *data, bson_t *hello_response) {
...
static bool auto_hello(request_t *request, void *data) {
...
static void hello_callback_free(void *data) {
    hello_callback_t *callback = (hello_callback_t *)data;
...
int mock_server_auto_hello_callback(mock_server_t *server,
                                    hello_callback_func_t callback_func,
                                    void *data,
                                    destructor_t destructor) {
...
int mock_server_auto_hello(mock_server_t *server, const char *response_json, ...) {
...
static bool auto_endsessions(request_t *request, void *data) {
    BSON_UNUSED(data);
...
int mock_server_auto_endsessions(mock_server_t *server) {
...
void mock_server_set_request_timeout_msec(mock_server_t *server, int64_t request_timeout_msec) {
    bson_mutex_lock(&server->mutex);
...
void mock_server_set_rand_delay(mock_server_t *server, bool rand_delay) {
    bson_mutex_lock(&server->mutex);
...
sync_queue_t *mock_server_get_queue(mock_server_t *server) {
    sync_queue_t *q;
...
static void request_assert_no_duplicate_keys(request_t *request) {
 ...
 *--------------------------------------------------------------------------
 */
request_t *mock_server_receives_bulk_msg(mock_server_t *server,
                                         uint32_t flags,
...
request_t *_mock_server_receives_msg(mock_server_t *server, uint32_t flags, ...) {
    request_t *request;
    va_list args;
    bool r;
...
request_t *_mock_server_receives_single_msg(mock_server_t *server, uint32_t flags, const bson_t *doc) {
    request_t *request;
    bool r;
...
request_t *mock_server_matches_legacy_hello(request_t *request, const char *match_json);

request_t *mock_server_matches_any_hello_with_json(request_t *request,
 ...
 *--------------------------------------------------------------------------
 */
request_t *mock_server_receives_any_hello(mock_server_t *server) {
...
request_t *mock_server_receives_legacy_hello(mock_server_t *server, const char *match_json) {
...
request_t *mock_server_receives_hello(mock_server_t *server) {
...
request_t *mock_server_receives_any_hello_with_match(mock_server_t *server,
                                                     const char *match_json_op_msg,
                                                     const char *match_json_op_query) {
...
request_t *mock_server_receives_hello_op_msg(mock_server_t *server) {
    return _mock_server_receives_single_msg(server,
                                            0,
...
request_t *mock_server_receives_bulk_insert(mock_server_t *server, const char *ns, mongoc_insert_flags_t flags, int n) {
    request_t *request;
...
request_t *mock_server_receives_getmore(mock_server_t *server, const char *ns, int32_t n_return, int64_t cursor_id) {
    request_t *request;
...
request_t *mock_server_receives_kill_cursors(mock_server_t *server, int64_t cursor_id) {
    request_t *request;
...
void mock_server_replies_simple(request_t *request, const char *docs_json) {
    mock_server_replies(request, MONGOC_REPLY_NONE, 0, 0, 1, docs_json);
}

/* To specify additional flags for OP_MSG replies. */
void mock_server_replies_opmsg(request_t *request, mongoc_op_msg_flags_t flags, const bson_t *doc) {
    reply_t *reply;
...
static bool _mock_server_stopping(mock_server_t *server) {
    bool stopped;
...
static BSON_THREAD_FUN(main_thread, data) {
    ...
    for (;;) {
        ...
        if (client_sock) {
            test_suite_mock_server_log("%5.2f  %hu -> server port %hu (connected)",
...
static void _reply_destroy(reply_t *reply) {
    int i;
...
void mock_server_set_bind_opts(mock_server_t *server, mock_server_bind_opts_t *opts) {
    server->bind_opts = *opts;
}

void rs_response_to_hello(mock_server_t *server, int max_wire_version, bool primary, int has_tags, ...) {
    va_list ap;
    bson_string_t *hosts;
    bool first;
    mock_server_t *host;
