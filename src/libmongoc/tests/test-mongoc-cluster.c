...
static uint32_t server_id_for_reads(mongoc_cluster_t *cluster) {
    ...
    server_stream = mongoc_cluster_stream_for_reads(cluster, NULL, NULL, NULL, &error);
...
static void test_get_max_bson_obj_size(void) {
...
static void test_get_max_msg_size(void) {
    mongoc_server_description_t *sd;
    mongoc_cluster_node_t *node;
    mongoc_client_pool_t *pool;
    mongoc_client_t *client;
...
static void _test_cluster_node_disconnect(bool pooled) {
...
static void test_cluster_node_disconnect_single(void *ctx) {
    BSON_UNUSED(ctx);

    _test_cluster_node_disconnect(false);
}

static void test_cluster_node_disconnect_pooled(void *ctx) {
    BSON_UNUSED(ctx);

    _test_cluster_node_disconnect(true);
}

static void _test_cluster_command_timeout(bool pooled) {
    mock_server_t *server;
    mongoc_uri_t *uri;
    mongoc_client_pool_t *pool = NULL;
    mongoc_client_t *client;
...
static void test_cluster_command_timeout_single(void) {
    _test_cluster_command_timeout(false);
}

static void test_cluster_command_timeout_pooled(void) {
    _test_cluster_command_timeout(true);
}

static void _test_write_disconnect(void) {
    mock_server_t *server;
    char *hello;
    mongoc_client_t *client;
    mongoc_collection_t *collection;
...
static void test_write_command_disconnect(void *ctx) {
    BSON_UNUSED(ctx);
...
static void test_cluster_time_cmd_started_cb(const mongoc_apm_command_started_t *event) {
    const bson_t *cmd;
...
static void test_cluster_time_cmd_succeeded_cb(const mongoc_apm_command_succeeded_t *event) {
    const bson_t *reply;
    cluster_time_test_t *test;
    bson_iter_t iter;
    uint32_t len;
...
static void _test_cluster_time(bool pooled, command_fn_t command) {
...
static bool command_simple(mongoc_client_t *client, bson_error_t *error) {
    return mongoc_client_command_simple(client, "test", tmp_bson("{'ping': 1}"), NULL, NULL, error);
}

static void test_cluster_time_command_simple_single(void) {
    _test_cluster_time(false, command_simple);
}

static void test_cluster_time_command_simple_pooled(void) {
    _test_cluster_time(true, command_simple);
}

/* test the deprecated mongoc_client_command function with $clusterTime */
static bool client_command(mongoc_client_t *client, bson_error_t *error) {
    mongoc_cursor_t *cursor;
    const bson_t *doc;
    bool r;
...
static void test_cluster_time_command_single(void) {
    _test_cluster_time(false, client_command);
}

static void test_cluster_time_command_pooled(void) {
    _test_cluster_time(true, client_command);
}

/* test modern mongoc_client_read_command_with_opts with $clusterTime */
static bool client_command_with_opts(mongoc_client_t *client, bson_error_t *error) {
...
static void test_cluster_time_command_with_opts_single(void) {
    _test_cluster_time(false, client_command_with_opts);
}

static void test_cluster_time_command_with_opts_pooled(void) {
    _test_cluster_time(true, client_command_with_opts);
}

/* test aggregate with $clusterTime */
static bool aggregate(mongoc_client_t *client, bson_error_t *error) {
    mongoc_collection_t *collection;
    mongoc_cursor_t *cursor;
    const bson_t *doc;
    bool r;
...
static void test_cluster_time_aggregate_single(void) {
    _test_cluster_time(false, aggregate);
}

static void test_cluster_time_aggregate_pooled(void) {
    _test_cluster_time(true, aggregate);
}

/* test queries with $clusterTime */
static bool cursor_next(mongoc_client_t *client, bson_error_t *error) {
    mongoc_collection_t *collection;
    mongoc_cursor_t *cursor;
    const bson_t *doc;
    bool r;
...
static void test_cluster_time_cursor_single(void) {
    _test_cluster_time(false, cursor_next);
}

static void test_cluster_time_cursor_pooled(void) {
    _test_cluster_time(true, cursor_next);
}

/* test inserts with $clusterTime */
static bool insert(mongoc_client_t *client, bson_error_t *error) {
    mongoc_collection_t *collection;
    bool r;
...
static void test_cluster_time_insert_single(void) {
    _test_cluster_time(false, insert);
}

static void test_cluster_time_insert_pooled(void) {
    _test_cluster_time(true, insert);
}

static void replies_with_cluster_time(request_t *request, int t, int i, const char *docs_json) {
    ...
    BSON_APPEND_DOCUMENT(&doc,
                         "$clusterTime",
                         tmp_bson("{'clusterTime': {'$timestamp': {'t': %d, 'i': %d}}, 'x': 'y'}", t, i));
...
static void assert_ok(future_t *future, const bson_error_t *error) {
    bool r = future_get_bool(future);
    ASSERT_OR_PRINT(r, (*error));
    future_destroy(future);
}

static future_t *future_ping(mongoc_client_t *client, bson_error_t *error) {
    return future_client_command_simple(client, "test", tmp_bson("{'ping': 1}"), NULL, NULL, error);
}

static void _test_cluster_time_comparison(bool pooled) {
...
static void test_cluster_time_comparison_single(void) {
    _test_cluster_time_comparison(false);
}

static void test_cluster_time_comparison_pooled(void) {
...
/* a "not primary" or "node is recovering" error marks server Unknown.
   ...
   message. */
static void _test_not_primary(bool pooled, run_command_fn_t run_command, cleanup_fn_t cleanup_fn) {
...
static future_t *future_command_simple(mongoc_client_t *client) {
    return future_client_command_simple(client, "test", tmp_bson("{'cmd': 1}"), NULL, NULL, NULL);
}

static void function_command_simple_cleanup(future_t *future) {
    future_destroy(future);
}

static void test_not_primary_single(void) {
    _test_not_primary(false, future_command_simple, function_command_simple_cleanup);
}

static void test_not_primary_pooled(void) {
...
static future_t *future_command_private(mongoc_client_t *client) {
    bson_error_t error;
    mongoc_server_stream_t *server_stream;
...
static bool auto_hello_callback(request_t *request, void *data, bson_t *hello_response) {
...
static void _test_dollar_query(void *ctx) {
    dollar_query_test_t *test;
    mock_server_t *server;
    mongoc_client_t *client;
    mongoc_collection_t *collection;
...
static void _test_cluster_hello_fails(bool hangup) {
...
static void test_cluster_hello_fails(void) {
    _test_cluster_hello_fails(false);
}

static void test_cluster_hello_hangup(void) {
    _test_cluster_hello_fails(true);
}

static void test_cluster_command_error(void) {
    mock_server_t *server;
    mongoc_client_t *client;
    bson_error_t err;
    request_t *request;
...
static void test_advanced_cluster_time_not_sent_to_standalone(void) {
    mock_server_t *server;
    mongoc_client_t *client;
 ...
 * to everything else. */
static bool _responder(request_t *req, void *data) {
    char *hello;
...
void test_hello_on_unknown(void) {
    mock_server_t *mock_server;
    mongoc_client_pool_t *pool;
    mongoc_client_t *client;
    bson_error_t error;
 ...
 */
void _test_cmd_on_unknown_serverid(bool pooled) {
    mongoc_client_pool_t *pool;
    mongoc_client_t *client;
    bson_error_t error;
    bool ret;
...
void test_cmd_on_unknown_serverid_pooled(void) {
    _test_cmd_on_unknown_serverid(true /* pooled */);
}

void test_cmd_on_unknown_serverid_single(void) {
    _test_cmd_on_unknown_serverid(false /* pooled */);
}

/* Test that server streams are invalidated as expected. */
static void test_cluster_stream_invalidation_single(void) {
    mongoc_client_t *client;
    mongoc_server_description_t *sd;
    bson_error_t error;
    ...
    tdmod = mc_tpld_modify_begin(client->topology);
    _mongoc_topology_description_clear_connection_pool(tdmod.new_td, mongoc_server_description_id(sd), &kZeroServiceId);
    mc_tpld_modify_commit(tdmod);
    BSON_ASSERT(!mongoc_cluster_stream_valid(&client->cluster, stream));
...
/* Test that server streams are invalidated as expected. */
static void test_cluster_stream_invalidation_pooled(void) {
    mongoc_client_pool_t *pool;
    mongoc_client_t *client;
    mongoc_server_description_t *sd;
    bson_error_t error;
...
void test_cluster_install(TestSuite *suite) {
