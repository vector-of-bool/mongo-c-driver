...
static void test_auth(mongoc_database_t *db, bool expect_failure) {
    bson_error_t error;
...
static bool creds_eq(_mongoc_aws_credentials_t *a, _mongoc_aws_credentials_t *b) {
    BSON_ASSERT_PARAM(a);
    BSON_ASSERT_PARAM(b);
...
static bool _mongoc_setenv(const char *name, const char *value) {
#ifdef _WIN32
    char *envstring;
...
static bool caching_expected(const mongoc_uri_t *uri) {
...
static bool do_find(mongoc_client_t *client, bson_error_t *error) {
...
static void test_cache(const mongoc_uri_t *uri) {
    bson_error_t error;
...
static void test_cache_with_env(const mongoc_uri_t *uri) {
    bson_error_t error;
...
static void test_multithreaded(const mongoc_uri_t *uri) {
