...
static __inline int bson_open(const char *filename, int flags, ...) {
    int fd = -1;
...
void _test_error(const char *format, ...) BSON_GNUC_PRINTF(1, 2);
...
#define ASSERT_STARTSWITH(a, b)                                                                                        \
    do {                                                                                                               \
        ...
        const char *_b = b;                                                                                            \
        if ((_a) != strstr((_a), (_b))) {                                                                              \
            MONGOC_STDERR_PRINTF("%s:%d %s(): : [%s] does not start with [%s]\n",                                      \
                                 __FILE__,                                                                             \
                                 __LINE__,                                                                             \
                                 BSON_FUNC,                                                                            \
                                 _a,                                                                                   \
...
#define ASSERT_NO_CAPTURED_LOGS(_info)                                                                                 \
    do {                                                                                                               \
        if (has_captured_logs()) {                                                                                     \
            MONGOC_STDERR_PRINTF("%s:%d %s(): testing %s shouldn't have logged:\n",                                    \
                                 __FILE__,                                                                             \
                                 __LINE__,                                                                             \
                                 BSON_FUNC,                                                                            \
                                 _info);                                                                               \
...
void TestSuite_Add(TestSuite *suite, const char *name, TestFunc func);
int TestSuite_CheckLive(void);
void TestSuite_AddLive(TestSuite *suite, const char *name, TestFunc func);
...
void TestSuite_AddWC(TestSuite *suite, const char *name, TestFuncWC func, TestFuncDtor dtor, void *ctx);
Test *
_V_TestSuite_AddFull(TestSuite *suite, const char *name, TestFuncWC func, TestFuncDtor dtor, void *ctx, va_list ap);
void _TestSuite_AddFull(TestSuite *suite, const char *name, TestFuncWC func, TestFuncDtor dtor, void *ctx, ...);
...
void _process_skip_file(const char *, mongoc_array_t *);
