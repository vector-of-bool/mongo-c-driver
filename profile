...
parser.add_argument('--thread-frame-base',
    help='Include the thread ID as a stack frame', action='store_true')
...
int do_perf_event(struct bpf_perf_event_data *ctx) {
    ...
    // create map key
    struct key_t key = {.pid = tgid, .thread_id = id & 0xffffffff};
...
print('Compiling BPF program...', file=sys.stderr)
b = BPF(text=bpf_text)
if args.pid:
    b.sym(0, args.pid)
print('Attaching BPF program...', file=sys.stderr)
...
try:
    ...
    while remain > 0:
        part = min(remain, 5)
        print('{} seconds remaining...'.format(remain), file=sys.stderr)
        sleep(part)
        remain -= part
...
for k, v in sorted(counts.items(), key=lambda counts: counts[1].value):
    ...
    if args.folded:
        ...
        line = [k.name]
        if args.thread_frame_base:
            line.append('[thread {k.thread_id}]'.format(k=k))
